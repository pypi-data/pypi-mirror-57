...
class Item_viewQuerySet(TreeAuditModelQuerySet):
    def get_range_rows(self, start=None, end=None, function=None, json=None, distinct_field_names=None, *args):
        ...
        # criteria = json.get('data') and json.get('data').get('criteria') and len([item for item in json.get('data').get('criteria')]) > 1
        # if criteria:
        #     queryResult = Item_flat_view.objects.filter()._get_range_rows(*args, start=start, end=end, function=function, json=json, distinct_field_names=distinct_field_names)
        #     ids = tuple([record.id for record in queryResult])
        #     queryResult = Item_refs.objects.filter().get_descendants(id=ids, start=start, end=end)
        #     ids = [record.child_id for record in queryResult]
        #     queryResult = self.filter(id__in=ids).distinct()[start:end]
        # else:
