...
def instalong(input_file,output_folder='INSTA_LONG'):
    try :
        os.mkdir(output_folder)
    except FileExistsError:
        pass
    img = Image.open(input_file)
    W,H = img.size
    num_strips = np.ceil(W/H)
    WW = np.linspace(0,num_strips*H,num_strips+1,dtype=int)
    for i in range(len(WW)-1):
        tmp = img.crop((WW[i],0,WW[i+1],H))
        tmp.save(output_folder+'/'+str(i+1)+'.png')
    return(output_folder)

def carousal(input_file,output_folder='INSTA_LONG'):
    return(instalong(input_file,output_folder))

instacarousal = instalong
