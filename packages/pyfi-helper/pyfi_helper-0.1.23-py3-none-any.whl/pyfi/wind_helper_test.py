...
def case10():
    ...
    print(w.edb(begin_date=begin_date, end_date=end_date, codes=["brent"]))
...
def case11():
    ...
    code = "050004.IB"
    paras = ['windcode', 'yield_cnbd', 'net_cnbd', 'dirty_cnbd', 'modidura_cnbd', 'matu_cnbd', 'vobp_cnbd',
             'cnvxty_cnbd', 'accrueddays', 'nxcupn', 'accruedinterest', 'day', 'volume', 'dealnum']
...
def case19():
    ...
    df = w.edb(codes=["gz1y", "gz2y", "gz3y", "gz4y", "gz5y", "gz6y", "gz7y", "gz8y", "gz9y", "gz10y"],
               begin_date=begin_date,
               end_date=end_date).ffill().dropna()
    print(df)


def case_wsd():
    from pyfi import WindHelper
    WindHelper.wsd(code=["TF1406.CFE","F1406.CFE"], begin_date="20131216", end_date="20140101", paras=["settle"], options=None)
...
def case20():
    ...
    """
    from pyfi import WindHelper as w
    df = w.wsd(code=["180210.IB","180205.IB"], paras="yield_cnbd", begin_date="2018-10-01", end_date="2018-12-01")
    print(df)
...
def case21():
    ...
    多代码单因子测试
    :return:
    Created on Sun Mar  3 16:08:39 2019
    ...
    w.start()
    data = w.wset("sectorconstituent", "date=2019-03-03;sectorid=a001010100000000")
    df = pd.DataFrame(index=data.Codes)
    for i in range(len(data.Data)):
        df.loc[:, data.Fields[i]] = data.Data[i]
    from pyfi import WindHelper as w, get_end_date
    # 获得股票的列表
    # 提取的数据
    paras = ["west_avgroe_FY3", "west_eps_FY3", "west_sales_FY3"]
    panel = pd.DataFrame()
    for i in range(len(paras)):
        begin_date = get_end_date()
        end_date = get_end_date()
        tmp_df = w.wsd(code=df.wind_code.values.tolist()[:], paras=paras[i], begin_date=begin_date, end_date=end_date)
        print(tmp_df)
