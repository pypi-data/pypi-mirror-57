...
class Parameters:
    ...
    def parse_command_line(self):
        ...
        parser = argparse.ArgumentParser(prog='svist4get', description='svist4get commmand line format:', usage= short_help,
                                         add_help=False)


        parser.add_argument('-tl', dest='transcript_labels', action='append', nargs='*', default=[])

        parser.add_argument('-tls', dest='transcript_label_style', choices=['none', 'name', 'id', 'both', 'empty', 'auto'], default= 'empty')
        ...
        parser.add_argument("--version", "-v", action='version', version='%(prog)s 1.2.15',
                ...
                for j in range(len(self.adjustable_parameters)):
                    if j < k:
                        ...
                        if self.config['transcript_label_style'] == 'both':
                            if self.adjustable_parameters[j]['transcript_name'] != '' and self.name1 not in self.adjustable_parameters[j]['transcript_name']:
                                self.name1 = self.name1 + ' (' +  self.adjustable_parameters[j]['transcript_name'] + ')'

                            if self.adjustable_parameters[k]['transcript_name'] != '' and self.name2 not in self.adjustable_parameters[k]['transcript_name']:
