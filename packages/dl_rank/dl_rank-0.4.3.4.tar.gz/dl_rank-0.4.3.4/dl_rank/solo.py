...
class EstimatorManager(object):
    ...
    def build_estimator(self):
        ...
        distribution_strategy = self.run_conf.pop("distribute")
        if self.args['ps'] > 0 and self.args['tw'] > 0:
            if distribution_strategy == 'ParameterServerStrategy':
                self.run_conf.update({'distribute': tf.contrib.distribute.ParameterServerStrategy()})
                self.run_conf.update({'distribute': tf.contrib.distribute.ParameterServerStrategy()})
            elif distribution_strategy == 'AllReduce':
                self.run_conf.update({'distribute': tf.contrib.distribute.CollectiveAllReduceStrategy()})
                self.run_conf.update({'distribute': tf.contrib.distribute.ParameterServerStrategy()})
        else:
            self.run_conf['distribute'] = None
        # else:
