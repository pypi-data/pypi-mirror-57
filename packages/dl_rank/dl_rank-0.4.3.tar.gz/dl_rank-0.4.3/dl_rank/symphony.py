...
import subprocess
import argparse
from dl_rank.file.fileUtils import *
from dl_rank.file import wfile, pathStr
import time
try:
...
def transportFile(ssh, file_path_list, target_path):
    ...
    for file_path in file_path_list:
        target_path_ = os.path.join(target_path, file_path.rsplit('/', 1)[1])
        if not wfile.IsDirectory(file_path):
            sftp.put(file_path, target_path_)
        else:
            sftp.mkdir(target_path_)
            sftp.put_dir(file_path, target_path_)
...
def distributeTrain(confDir, date, retrain, ps_num, ev_num, emrName, keyFile, logDir=emr_home_path, hdfspath=None, dirs2issue=None):
    if not wfile.Exists(logDir):
        wfile.MakeDirs(logDir)
    confDir = pathStr(confDir)
    ...
    cmd_dir = os.path.join(logDir, 'cmd')
    if not wfile.Exists(cmd_dir):
        wfile.MakeDirs(cmd_dir)
    def run_remote(wi, TF_CONFIG, UserName, hostname, is_eval):
        ...
        cmd = r'''
        python3 {trainpath} --mode train --tw {tw} --wi {wi} --ps {ps} {date} --conf {conf} --tfconfig {tfconfig} --logpath {logpath} --hdfspath {hdfs}
        '''.format(trainpath=trainpath, tw=total_worker if not is_eval else 0, wi=wi, ps=ps_num,
                   date=date, conf=confDir, tfconfig=TF_CONFIG, logpath=logDir, hdfs=hdfspath)
        cmd = cmd.strip() + ' > {}'.format(os.path.join(logDir, 'train.log'))
        with wfile.Open(cmd_dir+'/{}'.format(hostname), 'w+') as f:
            f.writelines(cmd)
    ...
    def run_local(wi, TF_CONFIG):
        ...
        '''.format(trainpath=trainpath, tw=total_worker, wi=wi, ps=ps_num, date=date,
                   conf=confDir, tfconfig=TF_CONFIG, logpath=logDir, hdfs=hdfspath)
        # nohup_cmd = '/home/hadoop/nohup' + cmd + ' > {} 2>&1 &'.format(os.path.join(logDir, 'dl_rank_run.log'))
        cmd = cmd.strip() + ' > {}'.format(os.path.join(logDir, 'train.log'))
        with wfile.Open(cmd_dir+'/local', 'w+') as f:
            f.writelines(cmd)
...
def run_tensorboard(conf='', model_dir='', logpath=emr_home_path, port=6006):
    ...
    if model_dir != '':
        ...
        import yaml
        conf_path = copy_conf2logpath(conf, logpath)
        with wfile.Open(conf_path + '/mission.yaml', 'r') as f:
            train_conf = yaml.load(f)
            model_dir = train_conf['train']['model_dir']
...
def distributeInfer(conf, date, ps, emrName, logpath=emr_home_path, hdfspath=None, use_TFoS=True, dirs2issue=None, executer_num=0):
    _args_useSpark = True
    if dirs2issue is None:
        dirs2issue = []
    conf = pathStr(conf)
    if __name__=='__main__':
        ...
        package_names = ['model', 'utils', 'conf', 'layer', 'file']
    ...
    '''.format(py_file=py_file, archive= '', TFoS=use_TFoS_param, date=date, executor=executer_num, exec_file=os.path.join(_project_path, _exec_file),
               tf_log_path=logpath, hdfs=hdfspath, ps=ps, num_executor=executer_num, use_TFoS='--use_TFoS' if use_TFoS else '', conf=conf,
               inferlogpath=os.path.join(logpath, 'infer.log')
               )
