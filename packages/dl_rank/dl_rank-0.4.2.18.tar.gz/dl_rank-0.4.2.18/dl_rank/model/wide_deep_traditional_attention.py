...
class wide_and_deep(baseModel):
    ...
    def build_columns_information(self, params):
        ...
        wide = dict()
        multi_size = dict()
        partitioner = tf.fixed_size_partitioner(self.num_shards) if self.num_shards > 1 else None
        ...
        for feature in feature_list:
            ...
            embed_dim = conf['parameter'].get('embed_dim', 1)
            name = conf['parameter'].get('name', feature)
            multi_size.update({feature: abs(conf['multi']['num']) if 'multi' in conf else 1})
            with tf.variable_scope(scope, reuse=tf.AUTO_REUSE, partitioner=partitioner):
                ...
                if scope.startswith('deep') and self.use_deep:
                    deep.update({feature: tf.get_variable(initializer=get_initializer(**initializer),
                                                          shape=[size, embed_dim],
                                                          name='{}_embedding'.format(name))})
