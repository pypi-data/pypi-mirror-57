        ...
        for f in deep:
            ...
            if f_type == 'category':
                if combiner == 'none':
                    ...
                    deep_f = deep[f]
                    if f in table.keys():
                        features_f_indicator = table[f].lookup(features[f])

                        features_f_indicator, f_mask = layers.to_dense(features_f_indicator, default_value=null_value)
                        # f_mask = tf.not_equal(features_f_indicator, null_value)
                    else:
                        features_f = layers.sparse_cast(features[f], tf.int32)
                        features_f_indicator = layers.replace2default(features_f, size, default_value)
                        features_f_indicator, f_mask = layers.to_dense(features_f_indicator, default_value=null_value, out_type=tf.int32)

                    dense_shape = list(map(lambda x: -1 if x is None else x, features_f_indicator.shape.as_list()))

                    features_f_indicator_reshape = tf.contrib.layers.dense_to_sparse(tf.reshape(features_f_indicator, [-1]), eos_token=-404)
                    features_f_emb_reshape = tf.nn.embedding_lookup_sparse(deep_f, features_f_indicator_reshape, None)
                    features_f_emb = tf.reshape(features_f_emb_reshape, dense_shape+[features_f_emb_reshape.shape.as_list()[-1]])
