...
def embedding_lookup(params, ids):
    dense_shape = list(map(lambda x: -1 if x is None else x, ids.shape.as_list()))
    ids_reshape = tf.contrib.layers.dense_to_sparse(tf.reshape(ids, [-1]))
    ids_embed_reshape = tf.nn.embedding_lookup_sparse(params, ids_reshape, None)
    features_f_emb = tf.reshape(ids_embed_reshape, dense_shape + [ids_embed_reshape.shape.as_list()[-1]])
