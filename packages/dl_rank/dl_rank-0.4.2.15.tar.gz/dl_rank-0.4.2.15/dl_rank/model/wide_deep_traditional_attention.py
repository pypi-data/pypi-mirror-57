...
class wide_and_deep(baseModel):
    ...
    def forward(self, indexed_input, params, is_training):
        columns, _ = self.build_columns_information(params)
        sparse_emb, deep_emb, mask = self._first_parse_fn(indexed_input, columns, ',')
    ...
    def _forward_model(self, is_training, wide_emb, deep_emb, mask):
        ...
        with tf.variable_scope('trunk'):
            ...
            if self.mode == 'train':
                ...
                sequence_emb = deep_emb['click']

                itemid_token_mask = mask['pid']
                sequence_token_mask = mask['click']
            else:
                self.sequence = tf.identity(deep_emb['click'], name="seq_input_node")                    # placeholder: embedding_lookup_2/Reshape_1
    ...
    def _first_parse_fn(features, params, teriary_delim):
        ...
        deep_emb = dict()
        mask = dict()
        embeddings = params['deep']
        feature_len = {name: abs(info[1]) for name, info in params['multi'].items()}
        for feature_name, feature_record in features.items():
            ...
            if feature_name == "click":
                sequence = feature_f
                sequence_emb = tf.nn.embedding_lookup(embeddings['pid'], sequence)
                ...
                itemid = feature_f
                itemid_emb = tf.nn.embedding_lookup(embeddings['pid'], itemid)
                ...
                w_sp = tf.SparseTensor(values=tf.gather_nd(u_emb, indicies), indices=indicies,
                                       dense_shape=[feature_string_split.dense_shape[0], feature_len[feature_name]])
                feature_emb = tf.nn.embedding_lookup_sparse(embeddings[feature_name], w_sp, None, combiner='sum', name="SparseSum")
                add_sum = tf.pad(feature_emb, [
                    [0, tf.cast(feature_string_split.dense_shape[0], tf.int32) - tf.shape(feature_emb)[0]], [0, 0]],
