...
class ExperimentBuilder(object):
    ...
    def get_model_param(self, param):
        ...
        """
        if param in self._model_args.args:
            return self._model_args.args[param]
        elif param in self._model_args.default_args:
    ...
    def build(self, **kwargs):
        xp = self._recursive_build_(Experiment, self._model_args.args, self._model_args.default_args, kwargs)
    ...
    def _recursive_build_(protobuf_class, args, default_args, kwargs):
        ...
        for field in protobuf_class.DESCRIPTOR.fields:
            if field.name in args:
                assert field.name not in kwargs, "You cannot pass arguments that are set by ExperimentBuilder"
                ...
                if field.message_type is None:
                    ...
                    value.update(args)  # args may have nested values necessary to build this protobuf
                    # This fields is a protobuf message, we build it recursively
                    real_args[field.name] = ExperimentBuilder._recursive_build_(class_helpers.load_proto_class_from_protobuf_descriptor(field.message_type), value, default_args, kwargs)
                ...
                if field.label == FieldDescriptor.LABEL_REQUIRED:  # we build the message if it is required
                    # This fields is a protobuf message, we build it recursively
                    real_args[field.name] = ExperimentBuilder._recursive_build_(class_helpers.load_proto_class_from_protobuf_descriptor(field.message_type), args, default_args, kwargs)
