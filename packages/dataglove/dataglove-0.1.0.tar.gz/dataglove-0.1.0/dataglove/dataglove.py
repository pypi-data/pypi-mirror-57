...
path = ""
pythonType = ctypes.sizeof(ctypes.c_voidp)
if pythonType == 8:
	path = os.path.join(dir, "DataGloveWindows_64.dll")
else:
	path = os.path.join(dir, "DataGloveWindows_32.dll")
...
# Data Glove Core API                                                     #
#-------------------------------------------------------------------------#
def Forte_CreateDataGloveIO(handType, bluetoothName="", scanForOnlyPairedDevices=1):
	bluetoothName = bluetoothName.encode('utf-8')
	return __func(__lib.Forte_CreateDataGloveIO, c_void_p, [c_byte, ctypes.c_char_p, c_byte])(handType, bluetoothName, scanForOnlyPairedDevices)
