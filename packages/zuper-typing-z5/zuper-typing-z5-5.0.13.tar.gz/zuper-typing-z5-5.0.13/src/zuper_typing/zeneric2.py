...
from .annotations_tricks import (get_ClassVar_arg, get_Type_arg, is_ClassVar, is_NewType, is_Type, is_TypeLike,
                                 name_for_type_like)
from .constants import (BINDINGS_ATT, cache_enabled, DEPENDS_ATT, enable_type_checking, enable_type_checking_difficult,
...
def type_check(self, k, v, val):
    try:
        T = type(val)
        simple = T in [int, float, bool, str, bytes, Decimal, datetime]
        do_it = enable_type_checking_difficult or simple
        if do_it:
            fail = T.__name__ != v.__name__ and not isinstance(val, v)
            if fail:  # pragma: no cover
                msg = (f'The field is not of the expected value')
                # warnings.warn(msg, stacklevel=3)
                raise ZValueError(msg, field=k, expected_type=v, found_value=val, )
    except TypeError as e:  # pragma: no cover
        ...
        def __post_init__(self):
            ...
            for k, v in new_annotations.items():
                ...
                if isinstance(v, type):
                    val = getattr(self, k)
                    type_check(self, k=k, val=val, v=v)
