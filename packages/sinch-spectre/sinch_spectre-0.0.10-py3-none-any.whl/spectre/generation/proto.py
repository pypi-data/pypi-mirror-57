...
def generate(entities, config):
    output = []
    output.append(create_header(config))
    output.append(create_services(entities))
    output.append(create_messages(entities))
    write(''.join(output), config)
...
package __PACKAGE__;
'''
    output = output.replace('__JAVA_PACKAGE__', config.namespace + '.' + name + '.grpc')
    output = output.replace('__PACKAGE__', config.namespace + '.' + name)
    return output
