...
def MultinomialNB_classifier_score(X, y, **nb_parameters):
    ...
    if x_min<0:
        x_shift = -x_min
    classifier = MultinomialNB(**nb_parameters)
...
def ComplementNB_classifier(X, y, **nb_parameters):
    """
    ...
    if x_min<0:
        x_shift = -x_min
    classifier = ComplementNB(**nb_parameters)
    classifier.fit(X+x_shift, y)
    # coef_ = array of shape (nb_classes, nb_TFs)
...
def bagging_classifier(X, y, **bagging_parameters):
    """
    ...
    Examples:
        ...
        >>> bagging_parameters = {"base_estimator":svc,
                                  "n_estimators":5,
                                  "max_samples":0.9}
        >>> scores = bagging_classifier(tfs,tg,**bagging_parameters)
        >>> scores
    ...
    def get_score(classifier):
        ...
        if hasattr(classifier, 'feature_importances_'):
            ...
            if classifier.coef_.shape[0] > 1:
                ...
                scores = np.abs(classifier.coef_[0,:])
        return(scores)
    bc = BaggingClassifier(**bagging_parameters)#BaggingClassifier
    bc.fit(X,y)
