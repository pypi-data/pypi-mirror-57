...
class AuxiliaryBinaryInterface( object ):
	...
	@classmethod
	def Run( cls, args='', stdin=None, verbose=None ):
		execpath = cls.GetExecutablePath()
		if verbose is None: verbose = cls.verbose
		if verbose: print( ( ( "%s" % execpath ) if ' ' in execpath else execpath ) + ' ' + args )
		try: process = subprocess.Popen( [ execpath ] + shlex.split( args ), stdin=subprocess.PIPE )
		except OSError:
	...
	def __init__( self, destination, source=None, format=None, nChannels=None, fs=None, transform=None, verbose=None ):
		...
		self.fs = fs
		if self.verbose: print( '%r has been initialized' % self )
	...
	def Process( cls, snd, destination=None, **kwargs ):
		...
		You can direct its output either to the file specified by
...
class ffmpeg( AuxiliaryBinaryInterface ):
	...
	#   https://bitbucket.org/snapproject/audiomath-hg/issues/3
