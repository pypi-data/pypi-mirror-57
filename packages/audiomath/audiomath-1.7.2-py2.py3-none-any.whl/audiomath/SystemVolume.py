...
# third-party Python package `pcaw`, released under its original
# license (see `audiomath/pcaw_fork.py`).
# 
# $END_AUDIOMATH_LICENSE$

"""
Utilities for manipulating the operating-system master volume
and/or mixer. On Windows, this requires the third-party packages
`comtypes` and `psutil` (via the included fork of Andre Miras'
`pycaw`).  On macOS, it uses Applescript.  Currently there is no
