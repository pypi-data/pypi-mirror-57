...
class Synth( object ):
	"""
	This is a duck-typed substitute for the numpy array `s.y`
	at the heart of a `Sound` instance `s`, but instead of
	containing static data it allows on-the-fly functional
	computation of the sound signal.
	...
	You can also use a pre-configured *instance* of `Synth` as the decorator,
	...
	def __getitem__( self, subs ):
		...
		if len( subs ) == 1: subs.append( slice )
