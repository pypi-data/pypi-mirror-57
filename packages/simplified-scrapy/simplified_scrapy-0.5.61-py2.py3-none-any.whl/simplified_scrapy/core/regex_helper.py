...
def replaceReg(html, regex, new, count = 0):
  return re.sub(regex, new, html, count)
...
def _getTag(html, end, attr, start=None):
  if not start: start = html.rfind('<',0,end)
...
def removeElement(tag,attr='class',value=None,html=None,start=None,end=None,before=None):
  obj = _getElement(tag,attr,value,html,start,end,before)
  if(obj):
    return html[0:obj[1]]+html[obj[2]:]
  return html
...
def getParent4Ele(html,e,tag=None):
  if(e):
    start = e._start
    start = _getStart4Tag(html,start,tag)
    if not tag:
      tag = _getTag(html,e._start,None,start)
    end = e._end
    end = _getEnd4Tag(html,end,tag)
    if e.tag == tag:
...
def _checkSingle(html,start=None):
  ...
  flag = html[end-1:end]=='/'
  if flag: return True
  tag = _getTag(html,end,None,start)
...
def getPrevious4Ele(html,ele,tag=None):
  if(ele):
    ...
    start = _getStart4Tag(html,end,tag)
    if(start != None and end != None):
