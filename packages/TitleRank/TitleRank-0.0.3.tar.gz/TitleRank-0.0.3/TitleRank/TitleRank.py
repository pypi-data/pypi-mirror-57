...
print(os.path.join(os.getcwd(), current_path, 'data/stopwords.txt'))

class TitleRank():
    def __init__(self, stopwords_path=os.path.join(os.getcwd(), current_path, 'data/stopwords.txt'), userstopwords_path=os.path.join(os.getcwd(), current_path, 'data/user_stopwords.txt')):
        #导入停用词
        STOPWORDS = set(map(str.strip, open(stopwords_path).readlines()))
        self.stopwords = set(STOPWORDS)
        self.title_list=[]
        self.sp = spm.SentencePieceProcessor()
        self.sp.load(os.path.join(os.getcwd(), current_path, 'models/news_title_8w.model'))
        self.sp_emb = self.load_models(os.path.join(os.getcwd(), current_path, "embeddings/sp_title.emb"))
