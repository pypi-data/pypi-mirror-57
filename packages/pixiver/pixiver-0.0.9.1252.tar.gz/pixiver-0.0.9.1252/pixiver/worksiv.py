...
class Discussion(basiciv.BasicConfig):
    ...
    def __init__(self, illust_id: int or str, comments_count, **kwargs):
...
class Works(basiciv.BasicConfig):
    ...
    def __init__(self, illust_id: int or str = None, **kwargs):
    ...
    def view_tags(self) -> basiciv.Queue:
    ...
    def view_comments(self) -> Discussion:
    ...
    def view_thumb_image(self) -> None:
        print('Press key "s" save, non-"s" key page turn, key "Enter" determine, key "q" quit.')
    ...
    def view_small_image(self) -> None:
        print('Press key "s" save, non-"s" key page turn, key "Enter" determine, key "q" quit.')
    ...
    def view_regul_image(self) -> None:
        print('Press key "s" save, non-"s" key page turn, key "Enter" determine, key "q" quit.')
    ...
    def save(self, kind: str = 'original', page_num: int = 1) -> True or None:
        """
        :param kind: [mini, thumb, small, regular, original]
        :param page_num: >=0 and < pages
        :return: returns True when param == valid and image == successfully downloaded, otherwise False.
        """
        if 0 <= page_num < self.pages():
            ...
            try:
                ...
                raise basiciv.exceptions.PixivIoError('Save original image failed!')
            return True
        else:
            raise basiciv.exceptions.PixivError('Page code error!')

    def like(self) -> True or None:
    ...
    def bookmark(self) -> True or None:
        if 'cookies' not in self.sess.headers:
            raise basiciv.exceptions.PixivError('You must be logged in before using this feature!')
