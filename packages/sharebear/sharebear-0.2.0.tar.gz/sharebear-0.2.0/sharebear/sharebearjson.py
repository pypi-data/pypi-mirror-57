...
globals_yaml = os.path.join(this_script_dir, user + '.json')


def make_pickle(key, object, user=user):
    with open(os.path.join(this_script_dir, str(key) + str(user) + '.pickle'), 'w') as picklefile:
        pickle.dump(object, file=picklefile, protocol=pickle.HIGHEST_PROTOCOL)


def read_pickle(path_to_pickled):
    try:
        with open(path_to_pickled, 'r') as picklefile:
...
def get_var(global_var, text_file=globals_yaml, fail_silently=False):
    ...
    try:
        with open(text_file, 'r') as infile:
            collected_globals = json.load(infile)
        global_var_value = pickle.load(collected_globals[global_var])
...
def set_var(global_var, update_value, text_file=globals_yaml, delete=False):
    ...
    try:
        ...
        with open(text_file, 'w') as outfile:
            if not delete:
                global_vars = {}
                global_vars[global_var] = str(global_var) + '.pickle'
                with open(str(global_var) + '.pickle', 'w') as pickle_file:
                    pickle.dump(update_value, pickle_file, protocol=pickle.HIGHEST_PROTOCOL)
