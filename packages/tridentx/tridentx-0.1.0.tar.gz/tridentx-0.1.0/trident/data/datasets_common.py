...
class DataProvider(object):
    ...
    def __getitem__(self, index):
        ...
        if len(self.data[self.current_scenario])>max(index) and self.current_scenario in self.labels and len(self.labels[self.current_scenario])>max(index) :
            return self.image_transform(self.data[self.current_scenario][index]),self.labels[self.current_scenario][index]
        return self.image_transform(self.data[self.current_scenario][index]),
    ...
    def get_all_data(self,is_shuffle=False):
        if is_shuffle==False:
            return self.data[self.current_scenario]
        else:
            idxes=np.random.shuffle(np.arange(len(self)))
            return self.data[self.current_scenario][idxes]
    ...
    def image_transform(self, img_data):
        if len(self.image_transform_funcs)==0:
            return backend_adaptive(img_data)
            ...
            for im in img_data:
                if im.ndim>=2:
                    for fc in self.image_transform_funcs:
                        im=fc(im)
                return_img_data.append(backend_adaptive(im))
            return np.asarray(return_img_data)
        else:
            return img_data
