...
__all__ = ['to_numpy','to_tensor','print_network','plot_tensor_grid','summary','calculate_flops','Modulex','Sequential','Input','get_device','load']
...
class Modulex(nn.Module):
    ...
    def regist_shape_infer_hook(self):
        def hook(module, input):
            if module._is_built == False and callable(getattr(module, 'build_once')):
                module.build_once(input[0].size())

        if (not isinstance(self, Sequential) and self._is_built== False):
    ...
    def shape_infer(self,input_size):
        def regist_shape_infer_hook(module):
            def hook(module, input):
                class_name = str(module.__class__).split(".")[-1].split("'")[0]
                if class_name not in name_dict:
                    name_dict[class_name]=1
                else:
                    name_dict[class_name] += 1
                module.name = "%s-%i" % (class_name, name_dict[class_name])
                if module._is_built== False and callable(getattr(module, 'build_once')):
                    module.build_once(input[0].size())

            if (not isinstance(module, nn.Sequential) and not isinstance(module, Sequential) and not isinstance(module, nn.ModuleList) and not ( module == self)):
                if isinstance(module, Modulex):
        ...
        x =torch.from_numpy(np.zeros((2,)+input_size,dtype=np.float32)).to(device)
        # p    rint(type(x[0]))
        ...
        # register hook
        self.apply(regist_shape_infer_hook)
        # make a forward pass
        # print(x.shape)
        self.forward(x)
...
class Sequential(Modulex):
    ...
    def build_once(self, input_shape):
        if self._is_built == False :
            ...
            self.shape_infer(self.input_shape[1:])
            self._is_built =True
    # def build_once(self, input_shape):
    #     if self._is_built==False:
    #         for module in self._modules.values():
    #             module.build_once(input_filters)
    #             input_filters=module.num_filters
    #         self.to(_device)
    #         self._is_built = True
