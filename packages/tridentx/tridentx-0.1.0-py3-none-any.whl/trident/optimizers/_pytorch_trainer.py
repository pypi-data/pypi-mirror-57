...
class Events(Enum):
    ...
    BEFORE_ZERO_GRAD="before_zero_grad"
...
class RemovableEventHandle(object):
    ...
    def __init__(self, event_name, handler, trainer):
    ...
    def remove(self):
        ...
        if handler is None or trainer is None:
            return

        if trainer.has_event_handler(handler, self.event_name):
            trainer.remove_event_handler(handler, self.event_name)
    ...
    def _run_once_on_dataset(self):
        ...
        try:
            for batch in self.state.dataloader:
                ...
                self._fire_event(Events.MINIBATCH_START)
                self.state.output = self._process_function(self, self.state.batch)
                self._fire_event(Events.MINIBATCH_END)
