...
class TrainingPlan(object):
    def __init__(self, callbacks=None,gradient_accumulation_steps=1,):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.training_context = {'_results_history': [], # loss_wrapper
            'losses': None, # optimizer
            'optimizer': None, # stop training
            'stop_training': False, # current_epoch
            '_current_epoch': -1, # current_batch
            'current_batch': None, # current output
            'current_output': None, # current loss
            'current_loss': None,
        ...
        for epoch in range(self.num_epochs):
            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    # optimizer clean the gradient
                    optimizer.before_batch_train()
                    ...
                    #optimizer update the gradient
                    optimizer.updates(current_loss,self.training_context)
