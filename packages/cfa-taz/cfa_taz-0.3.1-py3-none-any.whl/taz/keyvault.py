...
from azure.identity import DefaultAzureCredential, ChainedTokenCredential, ClientSecretCredential, ManagedIdentityCredential, InteractiveBrowserCredential
import os
...
class KeyVault:
    ...
    attributes:
        ...
        - mode (int): connection mode based on environment vars
        AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET and AZURE_CLI_USER
            - 0: service principal authentication if AZURE_CLIENT_SECRET is set
            - 1: user MSI authentication if AZURE_CLIENT_ID is set
            - 2: CLI based authentication if AZURE_CLI_USER var is set
            - 3: system MSI authentication else
    """
    ...
    def __init__(self, name):
        ...
        self.client = SecretClient(
            "https://{}.vault.azure.net/".format(self.name),
            credentials
        )
