...
usage:
    ia undark <identifier>... --comment=<comment>
    ia undark --help
    ...
    for identifier in args['<identifier>']:
        ...
        if not item.exists:
            print('warning: item does not exist - {}'.format(item.identifier),
                  file=sys.stderr)
        r = item.undark(args['--comment'])
        if r is None:
            print('warning: no task submitted, item already undark - {}'.format(item.identifier),
                  file=sys.stderr)
        elif r.status_code != 200:
            print('error: task failed, item not undark - {}'.format(item.identifier),
                  file=sys.stderr)
            errors = True
        else:
            print('success: item undarked - {}'.format(item.identifier))
