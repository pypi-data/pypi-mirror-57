...
class SenseDataService(object):
    ...
    # 23广彬用
    @catch_except_log
    def get_multi_market_info_by_name(self, names_list: list, stub=None):
        _names_list = json.dumps(names_list, ensure_ascii=False)
        _response = stub.get_multi_market_info_by_name(stock_pb2.Request(entity_name=_names_list))
        return json.loads(_response.txt)

    # 24广彬用
    @catch_except_log
    def get_main_market_info_by_name(self, names_list: list, stub=None):
        _names_list = json.dumps(names_list, ensure_ascii=False)
        _response = stub.get_main_market_info_by_name(stock_pb2.Request(entity_name=_names_list))
        return json.loads(_response.txt)

    # 25广彬用
    @catch_except_log
    def get_market_info_by_stock_code(self, names_list: list, stub=None):
        _names_list = json.dumps(names_list, ensure_ascii=False)
        _response = stub.get_market_info_by_stock_code(stock_pb2.Request(entity_name=_names_list))
        return json.loads(_response.txt)
