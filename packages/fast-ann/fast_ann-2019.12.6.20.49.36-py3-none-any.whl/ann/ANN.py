...
class ANN(object):
    ...
    def search(self, data, topK=10, nprobe=1, k_factor=1):
        ...
        :return:
        """
        self.index.k_factor = k_factor  # 搜索阶段会首    先选取 k_factor*topK，重排序
    ...
    def write_index(self, file_name="index_file.index"):
        _ = faiss.index_gpu_to_cpu(self.index) if faiss.get_num_gpus() > 0 else self.index
        faiss.write_index(_, file_name)
