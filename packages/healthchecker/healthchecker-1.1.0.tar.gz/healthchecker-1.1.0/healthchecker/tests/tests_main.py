...
class TestValidInputs(TestCase):
    ...
    def test_notify_ok(self, mock_notify, mock_logger):
        ...
        mock_notify.assert_called_with('https://test.local', 'http://test.local',
                                       {}, log=True)
    ...
    def test_notify_with_headers(self, mock_notify, mock_logger):
        ...
        __main__._notify('https://test.local', svclist,
                         headers={'x-freedom': '#FreeOlaBini'})
        mock_notify.assert_called_with('https://test.local', 'http://test.local',
                                       {'x-freedom': '#FreeOlaBini'}, log=True)
    ...
    def test_notify_with_payload(self, mock_notify, mock_logger):
        ...
        mock_notify.assert_called_with('https://test.local',
                                       'payload: http://test.local', {}, log=True)
    ...
    def test_notify_json(self, mock_notify, mock_logger):
        ...
        mock_notify.assert_called_with('https://test.local', '["http://test.local"]',
                                       {'content-type': 'application/json'}, log=True)
    ...
    def test_notify_all_options(self, mock_notify, mock_logger):
        ...
        mock_notify.assert_called_with(
            ...
            {'x-freedom': '#FreeOlaBini', 'content-type': 'application/json'},
            log=True,
        )
    ...
    def test_to_github_create_file(self, mock_github, mock_logger):
        ...
        mock_logger.info.assert_called_with('File %s %s: %s', 'ftest', 'created',
                                            'abc123')
        self.assertTrue(result)
    ...
    def test_to_github_update_file_no_changes(self, mock_github, mock_logger):
        ...
        mock_github.InputGitAuthor = lambda name, email: name + ' ' + email
        id_, sec = 'abc', '123'
        result = __main__._to_github('healthchecker/test', f'{id_},{sec}', 'ftest',
                                     svclist)
        mock_github.Github.assert_called_once_with(client_id=id_, client_secret=sec)
        gh.get_repo.assert_called_once_with('healthchecker/test')
        repo.get_contents.assert_called_once_with('ftest')
        mock_logger.info.assert_called_with('File %s %s: %s', 'ftest', 'kept unchanged',
                                            'no changes detected')
        self.assertTrue(result)
    ...
    def test_to_github_update_file(self, mock_github, mock_logger):
        ...
        repo = mock.MagicMock()
        repo.get_contents.return_value = file
        repo.update_file.return_value = {'commit': commit}
        gh = mock.MagicMock()
        gh.get_repo.return_value = repo
        mock_github.Github.return_value = gh
        mock_github.InputGitAuthor = lambda name, email: name + ' ' + email
        ...
        mock_logger.info.assert_called_with('File %s %s: %s', 'ftest', 'updated',
                                            'abc123')
        self.assertTrue(result)
    ...
    def test_to_github_repo_not_found(self, mock_github, mock_logger):
        ...
        mock_github.UnknownObjectException = Exception
        mock_github.Github.return_value = gh
        result = __main__._to_github('healthchecker/test', 'abc123', 'ftest',
                                     ServiceStatusList())
        gh.get_repo.assert_called_once_with('healthchecker/test')
        mock_logger.error.assert_called_once_with('Repository %s not found',
                                                  'healthchecker/test')
        self.assertFalse(result)
    ...
    def test_to_github_bad_creds(self, mock_github, mock_logger):
        ...
        result = __main__._to_github('healthchecker/test', 'abc123', 'ftest',
                                     ServiceStatusList())
        gh.get_repo.assert_called_once_with('healthchecker/test')
        mock_logger.error.assert_called_once_with('Bad credentials for repository %s',
                                                  'healthchecker/test')
        self.assertFalse(result)

    @mock.patch('healthchecker.__main__.logger')
    def test_to_github_no_filename(self, mock_logger):
        result = __main__._to_github('healthchecker/test', 'abc123', '',
                                     ServiceStatusList())
        mock_logger.error.assert_called_once_with('Repository %s indicated to store '
    ...
    def test_to_github_no_token(self, mock_logger):
        result = __main__._to_github('healthchecker/test', '', 'ftest',
                                     ServiceStatusList())
        mock_logger.error.assert_called_once_with('Repository %s indicated to store '
                                                  'results but Github API Token is '
                                                  'missing', 'healthchecker/test')
        self.assertFalse(result)
    ...
    def test_main_arg_url(self, mock_argparser, mock_healthcheckurls):
        ...
        result = asyncio.run(__main__.main())
        parser.parse_args.assert_called_once()
        mock_healthcheckurls.assert_called_once_with(['http://test.local'], [],
                                                     timeout=10, log=True)
        self.assertEqual(result, 0)
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_github_ok(self, mock_argparser, mock_healthcheckurls,
                                mock_to_github):
        ...
        mock_to_github.assert_called_once_with('healthchecker/test', 'abc123',
                                               'ftest', 'statuses', 'branch',
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_github_failed(self, mock_argparser, mock_healthcheckurls,
                                    mock_to_github):
        mock_healthcheckurls.return_value = 'statuses'
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_notify_ok(self, mock_argparser, mock_healthcheckurls,
                                mock_notify):
        ...
        parser.parse_args.assert_called_once()
        mock_healthcheckurls.assert_called_once()
        mock_notify.assert_called_once_with('https://test.local', False, 'payload',
                                            {'header': 'test'}, True)
        self.assertEqual(result, 0)
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_notify_failed(self, mock_argparser, mock_healthcheckurls,
                                    mock_notify):
        mock_healthcheckurls.return_value = False
        mock_notify.return_value = False
        args = mock.MagicMock()
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_output_file_ok(self, mock_argparser, mock_healthcheckurls,
                                     mock_to_file):
        mock_healthcheckurls.return_value = 'statuses'
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_output_file_failed(self, mock_argparser, mock_healthcheckurls,
                                         mock_to_file):
        mock_healthcheckurls.return_value = 'statuses'
...
class TestInvalidInputs(TestCase):
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_arg_notify_wrong_headers(self, mock_argparser, mock_healthcheckurls,
                                           mock_notify, mock_logger):
        mock_healthcheckurls.return_value = False
        mock_notify.return_value = True
        args = mock.MagicMock()
        args.version = False
        ...
        mock_notify.assert_called_once_with('https://test.local', False, 'payload',
                                            {}, True)
        mock_logger.error.assert_called_with('At least one header is invalid, '
                                             'skipping...')
    ...
    @mock.patch('healthchecker.__main__.healthcheck_urls', new_callable=AsyncMock)
    @mock.patch('healthchecker.__main__._get_arguments_parser')
    def test_main_wrong_timeout(self, mock_argparser, mock_healthcheckurls,
                                mock_logger):
        args = mock.MagicMock()
        args.version = False
        args.url = (
        ...
        parser.parse_args.assert_called_once()
        mock_healthcheckurls.assert_called_once()
        mock_logger.error.assert_called_with('HEALTHCHECKER_REQUESTS_TIMEOUT '
