...
class TestValidInputs(TestCase):
    ...
    def test_cli_option_version(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--version'),
        )
        expected = (
            'HealthChecker v',
        )
    ...
    def test_cli_options_github(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--gh-repo', 'healthchecker/test',
             'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--gh-repo', 'healthchecker/test',
             '--gh-token', 'test', 'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--gh-repo', 'healthchecker/test',
             '--gh-token', 'test', '--gh-filename', 'test', 'http://127.0.0.1:1'),
        )
        expected = (
    ...
    def test_cli_options_notify(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             '--notify-header', 'x-auth:abc123', 'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             '--notify-payload', 'Failed URLs: ', 'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             '--notify-payload', 'This URLs HEALTHCHECKER_FAILED_URLS failed.',
             'http://127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             '--notify-json', 'http://127.0.0.1:1'),
        )
        expected = (
    ...
    def test_cli_options_output(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--output', self.temp_filename,
             '127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '-o', self.temp_filename,
             '127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '--output', '-',
             '127.0.0.1:1'),
            ('python3', '-m', 'healthchecker', '-o', '-',
             '127.0.0.1:1'),
        )
        expected = (
            'Result stored as',
...
class TestInvalidInputs(TestCase):
    ...
    def test_cli_inexistent_option(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--inexistent'),
        )
        expected = (
    ...
    def test_cli_invalid_param_notify_header(self):
        cmds = (
            ('python3', '-m', 'healthchecker', '--notify-url', 'https://127.0.0.1:1',
             '--notify-header', 'invalid', 'https://127.0.0.1:1'),
        )
        expected = (
            'At least one header is invalid, skipping...',
        )
        self._test_base(cmds, expected)
