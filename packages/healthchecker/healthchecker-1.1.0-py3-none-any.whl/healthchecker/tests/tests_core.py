...
class TestValidInputs(TestCase):
    ...
    def test_notify(self, mock_http_request):
        ...
        mock_http_request.assert_called_with('POST', 'https://test.local', log=False,
                                             timeout=5, headers={}, data=b'payload')
        self.assertTrue(result)
        mock_http_request.return_value = resp_ok, 0, True
        result = notify('https://test.local')
        mock_http_request.assert_called_with('POST', 'https://test.local', log=False,
                                             timeout=5, headers=None, data=None)
    ...
    def test_notify_additional_params_to_requests(self, mock_http_request):
        ...
        mock_http_request.assert_called_once_with('POST', 'https://test.local',
                                                  timeout=5, log=False, data=None,
                                                  headers=None, some='kword', more=True)
    ...
    def test_notify_log(self, mock_http_request):
        ...
        mock_http_request.assert_called_with('POST', 'https://test.local', log=True,
                                             timeout=5, headers=None, data=None)
    ...
    def test_healthcheck_url_with_data_correct(self, mock_http_request, mock_logger):
        ...
        mock_http_request.assert_called_with('GET', 'https://test.local', timeout=5,
                                             log=False)
    ...
    @mock.patch('healthchecker.core.http_request')
    def test_healthcheck_url_log(self, mock_http_request, mock_logger_warning,
                                 mock_logger_info):
        resp = mock.MagicMock()
        resp.status_code = 200
        resp.ok = True
        ...
        mock_logger_warning.assert_called_with('Request to %s timed out taking '
                                               '%.2f seconds', 'https://test.local',
                                               5.555)
        mock_http_request.return_value = resp, 4, False
        healthcheck_url('https://test.local', timeout=5, log=True)
        mock_logger_warning.assert_called_with('Request to %s took too long: '
                                               '%.2f seconds', 'https://test.local', 4)
    ...
    @mock.patch('healthchecker.core.logger')
    @mock.patch('healthchecker.core.http_request')
    def test_healthcheck_url_additional_params_to_requests(self, mock_http_request,
                                                           mock_logger):
        # mock_logger just to mute output
        resp = mock.MagicMock()
        resp.status_code = 200
        ...
        mock_http_request.assert_called_with('GET', 'https://test.local', timeout=5,
                                             log=False, some='kword', more=True)
    ...
    def test_healthcheck_urls_additional_params_to_requests(self, mock_healthcheck):
        ...
        validations = ['']
        mock_healthcheck.return_value = ServiceStatus()
        asyncio.run(healthcheck_urls(urls, validations, timeout=5, some='kword',
                                     more=True))
        mock_healthcheck.assert_has_calls((
            mock.call('https://test.local', '', timeout=5, log=False, some='kword',
                      more=True),
        ))
