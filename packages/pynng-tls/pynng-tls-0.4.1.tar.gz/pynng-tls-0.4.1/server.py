from pynng import Rep0, TLSConfig
...
with Rep0() as sock:
    # c = TLSConfig(TLSConfig.MODE_SERVER, cert_key_file="key_pair.pem")
    c = TLSConfig(TLSConfig.MODE_SERVER, own_key_string=SERVER_KEY, own_cert_string=SERVER_CRT)
    sock.tls_config = c
    sock.listen("tls+tcp://localhost:5555")

    while True:
        msg = sock.recv()
        print("Got request, length={}".format(len(msg)))
        msg *= 2
        sock.send(msg)
        print("Sent reply, length={}".format(len(msg)))
