...
from fds.model.fds_storage_class import FDSRestoreResult
...
def parse_argument(args):
  global method, bucket_name, region, object_name, \
    ...
    restore_archive, storage_class, list_trash, is_public, restore_trash, outside_access
  ...
  restore_trash = args.restore_trash
  print_config('restore trash object', restore_trash)

  storage_class = args.storage_class
  print_config('storage_class', storage_class)

  list_trash = args.list_trash
  print_config('list_trash', list_trash)

  outside_access = args.outside_access
  print_config('outside access', outside_access)
...
def get_object(data_file, bucket_name, object_name, metadata, offset, length,
    webp_quality=False, gif_extract_type=False):
  check_bucket_name(bucket_name)
  check_object_name(object_name)
  if webp_quality:
...
def put_bucket(bucket_name, storage_class=None):
  check_bucket_name(bucket_name=bucket_name)
  fds_client.create_bucket(bucket_name, storage_class)
...
def set_public(bucket_name, object_name=None):
  check_bucket_name(bucket_name)
  fds_client.set_public(bucket_name, object_name=object_name)
  if object_name:
...
def restore_archived_object(bucket_name, object_name):
  check_bucket_name(bucket_name=bucket_name)
  check_object_name(object_name=object_name)
  result = fds_client._restore_archived_object_(bucket_name, object_name)

  if result == FDSRestoreResult.Restore:
    logger.info('restore archived object [%s/%s] success; this may takes several hours', bucket_name, object_name)
  elif result == FDSRestoreResult.Restoring:
    logger.info('object [%s/%s] is restoring, please wait', bucket_name, object_name)
  else:
    logger.info('object [%s/%s] is already restored', bucket_name, object_name)

def restore_trash_object(bucket_name, object_name):
  check_bucket_name(bucket_name=bucket_name)
  check_object_name(object_name=object_name)
  fds_client.restore_object(bucket_name, object_name)
  logger.info('object [%s/%s] is restored', bucket_name, object_name)
...
def head_object(bucket_name, object_name):
  res = fds_client.get_object_metadata(bucket_name=bucket_name,
                                      object_name=object_name)
  for k in sorted(res.metadata.keys()):
    print("%s:%s" % (k, res.metadata[k]))
...
def set_bucket_outside_access(bucket_name, outside_access):
  outside_access = outside_access.lower() == 'true'
  fds_client.set_bucket_outside_access(bucket_name, outside_access)
  logger.info("Set [%s] outside-access: [%s]" % (bucket_name, str(outside_access)))

def set_object_outside_access(bucket_name, object_name, outside_access):
  outside_access = outside_access.lower() == 'true'
  fds_client.set_object_outside_access(bucket_name, object_name, outside_access)
  logger.info("Set [%s/%s] outside-access: [%s]" % (bucket_name, object_name, str(outside_access)))
...
def main():
  ...
  parser.add_argument('--storage-class',
                      ...
                      const=None,
                      default=None,
                      help='Create bucket or object with specified storage class')
  ...
  parser.add_argument('--outside-access',
    nargs='?',
    dest='outside_access',
    default=None,
    type=str,
    help='Set or reset bucket/object outside access')

  parser.add_argument('--restore-trash',
    action='store_true',
    dest='restore_trash',
    default=False,
    help='''If toggled, restore trash object.''')
      ...
      if method == 'put':
        if src_bucket_name and src_object_name and dst_bucket_name and dst_object_name:
          ...
          if data_dir:
            ...
            set_public(bucket_name=bucket_name, object_name=object_name)
            ...
            restore_trash_object(bucket_name, object_name)
          elif outside_access:
            set_object_outside_access(bucket_name, object_name, outside_access)
          else:
            put_object(data_file=data_file,
                       bucket_name=bucket_name,
                       object_name=object_name,
          ...
          set_bucket_outside_access(bucket_name, outside_access)
        else:
          put_bucket(bucket_name, storage_class=storage_class)
        pass
      elif method == 'get':
        if object_prefix:
