...
from fds.model.fds_storage_class import FDSStorageClass, FDSRestoreResult
import os
import sys
...
class GalaxyFDSClient(object):
  ...
  def create_bucket(self, bucket_name, storage_class=None):
    '''
    ...
    params = None
    if storage_class:
      params = {"storageClass": str(storage_class)}
  ...
  def migrate_bucket_v2(self, bucket_name, org_id, team_id):
    '''
    tansfer owner id of bucket to org_id and team_id, this method can not be invoke by common user
    :param bucket_name: The name of the bucket to whom the object is put
    :param org_id:      The target org id
    :param team_id:   The target team id
    '''
    uri = '%s%s' % (self._config.get_base_uri(), quote(bucket_name))
    params = {
      'teamId': team_id,
    ...
    if response.status_code != requests.codes.ok:
      ...
      if self._config.debug:
        headers = ' header=%s' % response.headers
      message = 'Failed to migrate bucket: %s to org %s with %s, ' \
                'status=%s, reason=%s%s' % \
                (bucket_name, org_id, team_id, response.status_code, response.content, headers)
      raise GalaxyFDSClientException(message)
  ...
  def put_object(self, bucket_name, object_name, data, metadata=None):
    ...
    if filen < threshold_size:
      ...
      upload_token = self.init_multipart_upload(bucket_name, object_name, metadata)
      upload_list = []
      part_number = 1
      while (content):
  ...
  def get_object_with_uri(self, uri, position=0, size=4096):
    ...
    '''
    bucket_name, object_name = uri_to_bucket_and_object(uri)
    return self.get_object(bucket_name, object_name, position, size, True)
  ...
  def get_object_metadata(self, bucket_name, object_name):
    ...
    :param bucket_name: The name of the bucket
    :param object_name: The name of the object to check
    :return: metadata of specified object
    '''
    uri = '%s%s/%s' % (self._config.get_base_uri(), quote(bucket_name), quote(object_name))
    response = self._request.head(uri, auth=self._auth)
    if response.status_code == requests.codes.ok:
      return self._parse_object_metadata_from_headers(response.headers)
    elif response.status_code == requests.codes.not_found:
      raise GalaxyFDSClientException("Object [%s/%s] not exists" % (bucket_name, object_name))
    else:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
      message = 'Head object failed, status=%s, reason=%s%s' % (
        response.status_code, response.content, headers)
      raise GalaxyFDSClientException(message)
  ...
  def set_object_metadata(self, bucket_name, object_name, metadata):
    """
    Set metadata of object_name
    :param bucket_name: bucket name
    :param object_name: object name
    :param metadata: a FDSObjectMetadata type object or a python dict type object
    """
    uri = '%s%s/%s' % (self._config.get_base_uri(), quote(bucket_name), quote(object_name))

    if isinstance(metadata, dict):
      metadataBean = {"rawMeta": metadata}
    elif isinstance(metadata, FDSObjectMetadata):
      metadataBean = {"rawMeta": metadata.metadata}
    else:
      raise GalaxyFDSClientException("the type of metadata should be FDSObjectMetadata or dict")
    params = {"setMetaData": ''}
    response = self._request.put(uri, auth=self._auth, params=params,
                                 data=json.dumps(metadataBean, default=lambda x: x.to_string()))
    if response.status_code != requests.codes.ok:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
      message = 'Set object metadata failed, status=%s, reason=%s%s' % (
        response.status_code, response.content, headers)
      raise GalaxyFDSClientException(message)
  ...
  def set_public(self, bucket_name, object_name=None):
    """
    ...
    if object_name is None:
      ...
      self._set_object_public(bucket_name, object_name)

  def _set_object_public(self, bucket_name, object_name):
  ...
  def set_private(self, bucket_name, object_name=None):
    if object_name is None:
      self._set_bucket_private(bucket_name)
    else:
      self._set_object_private(bucket_name, object_name)
  ...
  def _set_object_private(self, bucket_name, object_name):
    acl = AccessControlList()
    grant = Grant(Grantee(UserGroups.ALL_USERS), Permission.READ)
    grant.type = GrantType.GROUP
    acl.add_grant(grant)
    self.delete_object_acl(bucket_name, object_name, acl)

  def init_multipart_upload(self, bucket_name, object_name, metadata=None):
    '''
  ...
  def _restore_archived_object_(self, src_bucket_name, src_object_name):
    ...
    if response.status_code == 200:
      return FDSRestoreResult.Restored
    elif response.status_code == 202:
      return FDSRestoreResult.Restore
    elif response.status_code == 409:
      return FDSRestoreResult.Restoring
    else:
      headers = ""
      if self._config.debug:
        headers = ' header=%s' % response.headers
