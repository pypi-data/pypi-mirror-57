...
Based on jdutil by Matt Davis (http://github.com/jiffyclub)
"""
...
MJD_ONE_MINUTE = 1.0 / 24 / 60
MJD_THIRTY_SECONDS = MJD_ONE_MINUTE / 2.0
...
def date_time_string_to_mjd(date_time_string,
                            format=u'%Y-%m-%d %H:%M'):
    """
    :param date_time_string:
    :param format:
    :return:mjd time adjusted for local time (e.g. BST)
    """
    try:
        date_time = datetime.strptime(date_time_string, format)
        # Adjust from local time to UTC
        date_time = date_time + daylight_savings.TIME_OFFSET
        return date_time.to_mjd()
    except Exception:
