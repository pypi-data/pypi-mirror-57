...
"""
    ...
    def do_authorize(self):
        ...
        has authority (has the right) to access the given target asset (``self.connection.target_user``,
        ``self.connection.target_server``, etc). For details consult the *Creating custom Authentication and
    ...
    def _extract_username(self):
        ...
        return (
            ...
            or self.connection.target_username
    ...
    def authenticate(self, cookie, session_cookie, **kwargs):
        """
        **Should not be modified.**
        ...
        self._setup_self(cookie, session_cookie, **kwargs)
    ...
    def authorize(self, cookie, session_cookie, **kwargs):
        """
        **Should not be modified.**
    ...
    def session_ended(self, session_id, cookie, session_cookie):
        ...
        **Should not be modified.**
        ...
        self._setup_self(cookie, session_cookie, session_id=session_id)
        return self.finalize_hook(
            self._execute_steps(checkpoints_key="session_ended_checkpoints", steps=self._session_ended_steps())
        )

    def _setup_self(self, cookie, session_cookie, **kwargs):
        self.connection = ConnectionInfo(**kwargs)
        self.cookie = cookie
