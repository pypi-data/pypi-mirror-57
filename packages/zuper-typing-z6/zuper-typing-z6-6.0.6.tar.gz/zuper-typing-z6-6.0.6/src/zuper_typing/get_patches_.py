...
def assert_equivalent_types(T1: TypeLike, T2: TypeLike, assume_yes: set = None):
    ...
    try:
        ...
        if is_dataclass(T1):
            ...
            if T1i.bindings != T2i.bindings:
                msg = "Different bindings"
                raise NotEquivalentException(msg, T1i=T1i, T2i=T2i)

            if T1i.orig != T2i.orig:
                msg = "Different orig"
                raise NotEquivalentException(msg, T1i=T1i, T2i=T2i)

            n1 = list(map(get_TypeVar_name, T1i.generic_att))
            n2 = list(map(get_TypeVar_name, T2i.generic_att))
            if n1 != n2:
                msg = "Different generic_att"
                raise NotEquivalentException(msg, n1=n1, n2=n2, T1i=T1i, T2i=T2i)
