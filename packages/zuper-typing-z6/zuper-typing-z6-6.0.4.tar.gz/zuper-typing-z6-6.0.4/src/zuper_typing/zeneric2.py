...
from .exceptions import ZTypeError, ZValueError
...
class ZenericFix(metaclass=ZMeta):
    ...
    def __class_getitem__(cls0, params):
        ...
        gp = type(name, (GenericProxy,), {GENERIC_ATT2: types0})
        setattr(gp, GENERIC_ATT2, types0)
        setattr(gp, BINDINGS_ATT, {})
        setattr(gp, GENERIC_BOUND_ATT, bound_att)
        return gp
...
class MyABC(StructuralTyping, ABCMeta):
    def __new__(mcs, name_orig: str, bases, namespace, **kwargs):
        try:
            ...
            d = "-"
        # logger.info(f"----\nCreating name: {name_orig}\n{d}")
        # logger.info('namespace: %s' % debug_print(namespace))
        # logger.info('bases: %s' % debug_print(bases))
        # if bases:
        ...
        if spec:
            ...
            name = name_orig
        #
        # if GENERIC_BOUND_ATT in namespace:
        #     bound_att = namespace[GENERIC_BOUND_ATT]
        # elif bases and GENERIC_BOUND_ATT in bases[0].__dict__:
        #     bound_att = bases[0].__dict__[GENERIC_BOUND_ATT]
        # else:
        #     bound_att = ()
        ...
        setattr(cls, GENERIC_BOUND_ATT, spec)
        # setattr(cls, GENERIC_BOUND_ATT, getattr(mcs, GENERIC_BOUND_ATT, ()))
        return cls
...
if PYTHON_36:  # pragma: no cover
    ...
    class FakeGenericMeta(MyABC):
        def __getitem__(self, params2):
            types = getattr(self, GENERIC_ATT2, ())
...
class GenericProxy(metaclass=FakeGenericMeta):
    ...
    def __class_getitem__(cls, params2) -> type:
        ...
        types = getattr(cls, GENERIC_ATT2, ())
        types2 = as_tuple(params2)

        if len(types) != len(types2):
            msg = "Cannot match type length"
            raise ZValueError(msg, types=types, types2=types2)
        ...
        for T, U in zip(types, types2):
            ...
            if T.__bound__ is not None and isinstance(T.__bound__, type):
                ...
                if not issubclass(U, T.__bound__):
                    ...
                    raise ZTypeError(msg, T=T, T_bound=T.__bound__, U=U)
...
def make_type(
    ...
    for tu in list(bindings):
        if tu in clsi.bindings:
            bindings.pop(tu)

    logger.debug(
        "make_type: " + debug_print(dict(cls=cls, bindings=bindings))  # symbols=symbols
    )
    ...
    # prev_bindings = getattr(cls, BINDINGS_ATT, {})
    # assert isinstance(generic_att2, tuple), generic_att2

    # if set(clsi.generic_att) != set(bindings):
    #     msg = "Mismatch with params"
    #     raise ZValueError(msg, di=clsi, bindings=bindings)

    recur = lambda _: replace_typevars(_, bindings=bindings, symbols=symbols)
    new_bindings = {}
    prev_bindings_updated = {k: recur(v) for k, v in clsi.bindings.items()}
    new_bindings.update(prev_bindings_updated)
    new_bindings.update(bindings)

    # orig2 = tuple(recur(v) for v in clsi.orig)
    # orig2 = clsi.orig
    annotations = getattr(cls, "__annotations__", {})
    name_without = get_name_without_brackets(cls.__name__)

    def param_name(x: type) -> str:
        # if x in new_bindings:
        #     tx = new_bindings[x]
        # else:
        #     tx = x
        # x2 = recur(tx)
    ...
    if clsi.orig:
        name2 = "%s[%s]" % (
            name_without,
            ",".join(param_name(_) for _ in clsi.orig),  # generic_att
        )
        # name2 = "%s[%s]" % (name_without, ",".join(param_name(_) for _ in clsi.orig))
    else:
        name2 = name_without

    # logger.info(
    #     "obtained name" + debug_print(dict(cls=cls, bindings=bindings, name2=name2))
    # )
    try:
    ...
    class Fake2:
        def __getitem__(self, item):
            logger.info(f"Fake2:getitem {item}")
            n = name_for_type_like(item)
    ...
    ts = ()
    for U in bindings.values():
        ts += find_typevars_inside(U)

    generic_att2_new = tuple(unique_preserve_order(ts))
    ...
    setattr(cls2, BINDINGS_ATT, new_bindings)
    setattr(cls2, GENERIC_ATT2, generic_att2_new)
    setattr(cls2, GENERIC_BOUND_ATT, clsi.orig)
    ...
    logger.debug(
        "make_type: "
        + debug_print(dict(cls=cls, bindings=bindings, result=cls2))  # symbols=symbols,
    )
...
@dataclass
class DataclassInfo:
    generic_att: Tuple[TypeLike, ...]
    bindings: Dict[TypeLike, TypeLike]
    orig: Tuple[TypeLike, ...]


def get_dataclass_info(T: Type[dataclass]) -> DataclassInfo:
    g = getattr(T, GENERIC_ATT2, ())
    b = getattr(T, BINDINGS_ATT, {})
    c = getattr(T, GENERIC_BOUND_ATT, ())
    return DataclassInfo(g, b, c)


def unique_preserve_order(t: Sequence[X]) -> List[X]:
    res = []
    for _ in t:
        if _ not in res:
            res.append(_)
