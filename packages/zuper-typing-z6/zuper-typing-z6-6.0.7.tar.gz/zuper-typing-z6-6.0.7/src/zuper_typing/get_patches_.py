...
def check_dataclass_info_same(d1, d2):
    ...
    if len(d1.orig) != len(d2.orig):
        msg = "Different orig"
        raise NotEquivalentException(msg, d1=d1, d2=d2)
    if len(d1.generic_att) != len(d2.generic_att):
        msg = "Different generic_att"
        raise NotEquivalentException(msg, d1=d1, d2=d2)

    for t1, t2 in zip(d1.orig, d2.orig):
        assert_equivalent_types(t1, t2)
    for t1, t2 in zip(d1.generic_att, d2.generic_att):
        assert_equivalent_types(t1, t2)
...
def assert_equivalent_types(T1: TypeLike, T2: TypeLike, assume_yes: set = None):
    ...
    try:
        ...
        if is_dataclass(T1):
            ...
            check_dataclass_info_same(T1i, T2i)
