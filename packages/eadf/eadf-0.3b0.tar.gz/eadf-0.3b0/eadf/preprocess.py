...
def setCompressionFactor(
    ...
    arrInFeasibleSizes = arrCumulative <= numValue - 1e-7
...
def periodifyFreq(arrData: np.ndarray, paddingSize: int) -> np.ndarray:
    ...
    t_inner = np.linspace(0, 1, arrData.shape[2], endpoint=False)

    t_upper = np.linspace(
        1,
        1 + (1 / t_inner.shape[0]) * paddingSize,
        paddingSize,
        endpoint=False,
    )

    t_lower = np.linspace(
        -(1 / t_inner.shape[0]) * paddingSize, 0, paddingSize, endpoint=False
    )

    la = 0.5 * (np.cos(np.linspace(0, 1, paddingSize) * np.pi) + 1)

    for ii0 in range(arrData.shape[0]):
        print("EADF.preprocess.periodifyFreq: Elevation %d" % (ii0), end="\r")
            ...
            for ii3 in range(arrData.shape[3]):
                for ii4 in range(arrData.shape[4]):
                    ...
                    splUpperR = InterpolatedUnivariateSpline(
                        t_inner[-numVals:], np.real(y[-numVals:])
                    )
                    splLowerR = InterpolatedUnivariateSpline(
                        t_inner[:numVals], np.real(y[:numVals])
                    )
                    splUpperI = InterpolatedUnivariateSpline(
                        t_inner[-numVals:], np.imag(y[-numVals:])
                    )
                    splLowerI = InterpolatedUnivariateSpline(
                        t_inner[:numVals], np.imag(y[:numVals])
                    )

                    # now we write the extrapolated values and multiply
                    # it with the windowing function.
                    arrRes[ii0, ii1, -paddingSize:, ii3, ii4] = (
                        la * splUpperR(t_upper) + (1 - la) * splLowerR(t_lower)
                    ) + 1j * (
                        la * splUpperI(t_upper) + (1 - la) * splLowerI(t_lower)
                    )

                    # #     # print(ii0, ii1, ii3, ii4)
                    # import matplotlib.pyplot as plt
                    # plt.subplot(211)
                    # plt.plot(t_inner, np.real(y))
                    # plt.plot(t_inner, np.imag(y))
                    # plt.plot(t_upper,  la * splUpperR(t_upper)
                    #         + (1 - la) * splLowerR(t_lower))
                    # plt.plot(t_upper,
                    #     la * splUpperI(t_upper)
                    #         + (1 - la) * splLowerI(t_lower)
                    # )
                    # plt.subplot(212)
                    # plt.semilogy(
                    #     np.fft.fftshift(
                    #         np.fft.fft(
                    #             arrData[ii0, ii1, :, ii3, ii4]
                    #         )
                    #     )
                    # )
                    # plt.semilogy(
    ...
    print()
    print("EADF.preprocess.periodifyFreq: Done.")
    return arrRes
