...
class EADF(object):
    ...
    def useGPU(self, flag: int) -> None:
        ...
        # now copy everything to the GPU (False -> True)
    ...
    def _eval(
        ...
        if self.isNarrowBand:
            logging.error("This array is narroband. Cannot use this function")
            return
        ...
        arrFreq *= (
            ...
            / (
                self._arrRawData.shape[2] * self._arrFreq[-1]
                - self._arrFreq[0]
            )
            * 2
            * np.pi
    ...
    def _evalNarrowBand(
        ...
        if self._isSingleFreq:
            ...
            a1 = (numFreq - self._arrFreq[numInd]) / (
                self._arrFreq[numInd + 1] - self._arrFreq[numInd]
            )
            a2 = (self._arrFreq[numInd + 1] - numFreq) / (
                self._arrFreq[numInd + 1] - self._arrFreq[numInd]
            )
    ...
    def __init__(
        ...
        **options : kwargs
        ...
        # the user might have set the flag.
        if self._isSingleFreq:
            self._isNarrowBand = options.get(
                "keepNarrowBand", self._isSingleFreq
            )
        else:
            self._isNarrowBand = True
        ...
        if not self._isNarrowBand:
            self._arrRawData = periodifyFreq(
                arrData[:, arrAziTrunc], int(arrData.shape[2] / 2)
            )
