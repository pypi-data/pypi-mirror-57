...
>>> arrAzi, arrEle = eadf.core.toGrid(
>>>     *eadf.core.sampleAngles(240, 120)
>>> )
>>> A.pattern(arrAzi, arrEle, 1)
...
Additionally we automatically run a just in time compiler if the numba
package is correctly installed. This is used to speed up the core routines
that are involved with the actual beam pattern evaluation.
...
>>> def B():
>>>     A.pattern(arrAzi, arrEle, 1)
>>> print(timeit.timeit(B, number=3)/3)

For the above code, simply installing numba via your favourite package
manager yields the following speedup.

>>> [user@pc EADF]$ intelpython3 bench.py # no numba
>>> 4.40465442603454
>>> [user@pc EADF]$ intelpython3 bench.py # with numba
>>> 3.7546293360646814

Note that in this example we evaluate the beampattern at 28800 distinct
angular locations. (YMMV) If you are using Intel Python, the numba package
is already installed in your distribution and the package plays nicely with
the rest of the optimizations done with the MKL.
...
from .arrays import generateURA
from .arrays import generateULA
from .arrays import generateUCA
from .arrays import generateStackedUCA
from .arrays import generateUniformArbitrary

from .auxiliary import toGrid
from .auxiliary import columnwiseKron
...
from .importers import fromAngleListData
from .importers import fromFourierData
