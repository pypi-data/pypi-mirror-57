...
def fromFourierData(
    ...
    ----------
    arrFourierData : np.ndarray
        co-ele x azi x pol x freq x elem
    arrAzi : np.ndarray
        Sampled azimuth angles in radians
    arrCoEle : np.ndarray
        Sampled co-elevation angles in radians
    ...
    fftShift: bool
        Should we do FFT shifts in the first two data dimensions?
    ...
    """
    if arrFreq.shape[0] != arrFourierData.shape[2]:
        logging.error(
            (
                "fromFourierData:"
                + "freqency samples %d must equal data dimension 3 %d"
            )
            % (arrFreq.shape[0], arrFourierData.shape[2])
        )
        return
    if arrPos.shape[1] != arrFourierData.shape[4]:
        logging.error(
            (
                "fromFourierData:"
                + "num of positions %d doesnt match elem number %d"
            )
    ...
    if fftShift:
        arrCoEleShift = np.fft.fftshift(np.arange(arrFourierData.shape[0]))
        arrAziShift = np.fft.fftshift(np.arange(arrFourierData.shape[1]))
    else:
        arrCoEleShift = np.arange(arrFourierData.shape[0])
        arrAziShift = np.arange(arrFourierData.shape[1])

    # this does the inverse FFT on 2*co-ele x azi
    arrDataRaw, _, _ = fourierToSampled(
        arrFourierData[arrCoEleShift, ...][:, arrAziShift, ...]
    )

    return EADF(
        arrDataRaw[: arrCoEle.shape[0], : arrAzi.shape[0], ...],
        arrAzi,
        arrCoEle,
        arrFreq,
        arrPos,
