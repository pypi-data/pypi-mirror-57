...
from tracwikiextras.util import render_table
...
class Entities(Component):
    """Use HTML entities in wiki text.

    The HTML entity `&hearts;` in a wiki text, for example, renders a heart.
    Use the `ShowEntities` macro to display a list of all entities.
    ...
    def expand_macro(self, formatter, name, content, args=None):
        return render_table(['&%s;' % e for e in ENTITIES], content,
                            lambda e: self._format_entity(formatter, e, None))
