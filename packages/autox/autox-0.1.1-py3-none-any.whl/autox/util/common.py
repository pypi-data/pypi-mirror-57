...
class MyRegression:
    ...
    def execute_case(self, scenario_list):
        """
        :param filename: 用例文件名称
        :return: 测试结果的报告模板

        """
        # global result, pass_result, fail_result, skip_result, case_all_count
        global result, pass_result, fail_result, skip_result, case_all_count
        host = cs.DB_HOST
        user = cs.DB_USER
                        ...
                        if actual_response is not None:
                            ...
                            if actual_status_code is not None:
                                ...
                                if assert_len > 1:
                                    ...
                                    summary_report = self.excReport.sum_result(scenario_name, url, method, name, run_time,
    ...
    def build_report(self, scenario_list):
        try:
            test_report = self.execute_case(scenario_list)
