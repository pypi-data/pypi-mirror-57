    ...
    for kss in range(int(max(IDX))+1):
        ...
        if BuildModel:
            for knn in range(enum):
                ...
                print('object numbers:',len(xax))
                ax922.plot(xax,yax,'r-',linewidth=1)
                ax922.axis('off')
    bdst = bdst[1:]
    if BuildModel :
        fig922.savefig(os.path.join(figdst,"registered objects.png"),format='png',transparent=True)
        fig922.savefig(os.path.join(figdst,"registered objects.svg"),format='svg',transparent=True) 
        fig289.savefig(os.path.join(figdst,"eigen-shape dendrogram.png"),format='png',transparent=True)
        fig289.savefig(os.path.join(figdst,"eigen-shape dendrogram.svg"),format='svg',transparent=True)
    crap=np.sort(IDX)
    sid=np.argsort(IDX)
    BMsort=cmsn[sid]
