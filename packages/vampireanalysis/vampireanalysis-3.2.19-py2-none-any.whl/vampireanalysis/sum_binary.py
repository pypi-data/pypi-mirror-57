...
def sum_binary(entries,CP_subfolder,progress_bar):
	labeledimfolder = os.path.join(CP_subfolder,'segmented images')
	setfolders = [_ for _ in os.listdir(CP_subfolder) if (os.path.isdir(os.path.join(CP_subfolder,_)) and _ != 'segmented images')]
	setfolderpaths = [os.path.join(CP_subfolder,_) for _ in setfolders]
	progress = 0
