...
from ..utils.ViewUtil import ViewUtil
import re
...
class Tag:
    ...
    def getTagExpression(self,alias = ''):
        ...
        for tag in self.tags:
            ...
            if onTag:
                tagAttrs = attrs.copy()
                # 起始符号
                tagStart = '{0}{1}(.*?){2}'.format(self.template.tagStart, tagAttrs['name'], self.template.tagEnd)
    ...
    def parseVar(self,varBody):
        ...
        if  firstChar == '$':
            # 变量
            result = varBody[1:]
            funcName = '{0}|safe'.format(varBody[1:])
            result = self.parseFunc(funcName)
        elif firstChar == ':':
            #方法
            func = varBody[1:];
            funcFirstChar = func[0];
            if funcFirstChar == '$':
                funcName = '{0}|safe'.format(func[1])
                result = self.parseFunc(funcName)
            else:
