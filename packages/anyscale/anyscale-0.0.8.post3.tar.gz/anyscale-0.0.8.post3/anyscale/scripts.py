...
from anyscale.snapshot import (
    ...
    describe_snapshot, download_snapshot, get_snapshot_uuid,
    list_snapshots)
from anyscale.util import (
    confirm, deserialize_datetime,
...
def session_start(command, args, shell, snapshot, name):
    project_definition = load_project_or_throw()
    print("YYY", project_definition.config)
    if not name:
    ...
    try:
        ...
        raise click.ClickException(e)
    print("YYY2", project_definition.config)
...
@click.option(
    "--snapshot",
    help="The snapshot the session should be synchronized with.",
    default=None)
@click.option(
    "--name",
...
def session_sync(snapshot, name, yes):
    ...
    for session in sessions:
        send_json_request(
            "session_sync",
            {"session_id": session["id"], "snapshot_id": snapshot},
            post=True)
        ...
        for session in sessions:
            ...
            for command in resp["commands"]:
                ...
                command_table.append([
                    command["command"], command["session_command_id"],
                    created_at])
            print(tabulate.tabulate(
                command_table,
                headers=[
