...
def push(workbooks, *, path=None, owner=None, label=None, datasource_map_folder=None, use_full_path=False,
         access_control=None, override_max_interp=False, errors='raise',
    ...
    owner_identity = None
    if owner not in [None, _workbook.ORIGINAL_OWNER, _workbook.FORCE_ME_AS_OWNER]:
        ...
        for index, workbook in remaining_workbooks.copy():  # type: (int, Workbook)
            ...
            try:
                ...
                if isinstance(e, ApiException) and e.status == 504:
                    # Gateway Timeout errors occur from time to time, ignore them and retry
                    status.put('Result', '504 Gateway Timeout, will retry...')
                    at_least_one_thing_pushed = True
                    continue
