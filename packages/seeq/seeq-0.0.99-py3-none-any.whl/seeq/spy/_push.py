...
def _process_push_row(asset_batch_input, cache, condition_batch_input, status, datasource_output, flush_now, index,
    ...
    if 'Signal' in d['Type']:
        ...
        if _common.present(d, 'Statistic'):
            threshold_metric_input.aggregation_function = _statistic_to_aggregation_function(d['Statistic'])
...
def _statistic_to_aggregation_function(statistic):
    fns = {
        # Signals
        'average': 'average()',
        'count': 'count()',
        ...
        'sum': 'sum()',
        'totalized': 'totalized()',
        'value at end': 'startValue()',
        'value at start': 'endValue()',
...
def _flush(put_signals_input, put_scalars_input, condition_batch_input, threshold_metric_inputs, asset_batch_input,
    ...
    def _set_push_result_string(dfi, iuo):
        ...
        if hasattr(iuo, 'datasource_class'):
            push_results_df.at[dfi, 'Datasource Class'] = iuo.datasource_class
        else:
            push_results_df.at[dfi, 'Datasource Class'] = items_api.get_property(id=iuo.id, property_name='Datasource '
                                                                                                          'Class').value
        if hasattr(iuo, 'datasource_id'):
            push_results_df.at[dfi, 'Datasource ID'] = iuo.datasource_id
        else:
            push_results_df.at[dfi, 'Datasource ID'] = items_api.get_property(id=iuo.id, property_name='Datasource '
                                                                                                       'ID').value
        if hasattr(iuo, 'data_id'):
            push_results_df.at[dfi, 'Data ID'] = iuo.data_id
        else:
            push_results_df.at[dfi, 'Data ID'] = items_api.get_property(id=iuo.id, property_name='Data ID').value

        if hasattr(iuo, 'item') and iuo.item is not None:
