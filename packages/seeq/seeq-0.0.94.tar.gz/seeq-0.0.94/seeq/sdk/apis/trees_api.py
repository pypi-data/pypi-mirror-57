...
class TreesApi(object):
    ...
    def get_tree_root_nodes(self, **kwargs):
        """
        Get a collection of root nodes for the specified tree
        Pagination is enabled for this query
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        to be invoked when receiving the response.
    ...
    def remove_all_nodes_from_tree(self, **kwargs):
        """
        Remove all items from the specified tree. (These items will still exist but not as part of this tree.)
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.remove_all_nodes_from_tree(callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.remove_all_nodes_from_tree_with_http_info(**kwargs)
        else:
            (data) = self.remove_all_nodes_from_tree_with_http_info(**kwargs)
            return data

    def remove_all_nodes_from_tree_with_http_info(self, **kwargs):
        """
        Remove all items from the specified tree. (These items will still exist but not as part of this tree.)
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.remove_all_nodes_from_tree_with_http_info(callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method remove_all_nodes_from_tree" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        return self.api_client.call_api('/trees/assets', 'DELETE',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
