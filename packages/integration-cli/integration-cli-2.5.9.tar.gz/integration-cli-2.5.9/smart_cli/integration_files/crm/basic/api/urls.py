...
from .views import CredentialModelViewSet, FilterAPIView, SelectionAPIView, CalculationAPIView
from integration_utils.views import HomeAPIView
from methods.views import BaseMethodModelViewSet, CustomAPIMethodViewSet

urlpatterns = [
    # auth methods
    path('get_credentials/', CredentialModelViewSet.as_view({"post": "create"}), name='get-credentials'),

    # steps
    path('filter/', FilterAPIView.as_view(), name='filter'),
    path('selection/', SelectionAPIView.as_view(), name='selection'),
    path('calculation/', CalculationAPIView.as_view(), name='selection'),
