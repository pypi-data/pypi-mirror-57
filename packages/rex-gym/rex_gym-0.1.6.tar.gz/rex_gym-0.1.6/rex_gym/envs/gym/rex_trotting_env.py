"""Implements the gym environment of Rex moving with trotting style.
WORK IN PROGRESS!
"""
import math
...
class RexTrottingEnv(rex_gym_env.RexGymEnv):
    """The trotting gym environment for Rex.
  ...
  of legs will move according to the reference trajectory:
      extension = extension_amplitude * cos(2 * pi * step_frequency * t + phi)
  ...
  """
    ...
    def __init__(self,
                 ...
                 env_randomizer=None,
                 log_path=None,
                 init_extension=INIT_FOOT_POS,
                 init_swing=INIT_LEG_POS,
                 step_frequency=2.0,
                 extension_amplitude=0.2,
                 swing_amplitude=-0.2):
    ...
    """
        ...
        # The reset position. Shoulder fixed to 0.
        self._init_pose = [
            0.0, init_swing, init_extension, 0.0, init_swing, init_extension,
            0.0, init_swing, init_extension, 0.0, init_swing, init_extension
        ]
        ...
        action_dim = 12
        action_high = np.array([0.4] * action_dim)
    ...
    def reset(self):
        ...
        super(RexTrottingEnv, self).reset(initial_motor_angles=initial_motor_angles,
                                               reset_duration=0.5)
    ...
    """
        ...
        for i in range(NUM_LEGS):
            ...
            leg_action = leg_pose[int(3 * i) + 1]
            motor_pose[int(3 * i + 1)] = max(min(leg_action, INIT_LEG_POS + 0.60), INIT_LEG_POS - 0.60)
            foot_pose = leg_pose[int(3 * i) + 2] + leg_pose[int(3 * i)]
            motor_pose[int(3 * i + 2)] = max(min(foot_pose, INIT_FOOT_POS + 0.60), INIT_FOOT_POS - 0.60)
            # leg_action = INIT_LEG_POS + leg_pose[int(3 * i)]
            # motor_pose[int(3 * i + 1)] = max(min(leg_action, INIT_LEG_POS + 0.45), INIT_LEG_POS - 0.78)
            # foot_pose = INIT_FOOT_POS + leg_pose[int(3 * i)]
            # motor_pose[int(3 * i + 2)] = max(min(foot_pose, INIT_FOOT_POS + 0.63), INIT_FOOT_POS - 0.23)
            # motor_pose[int(3 * i + 1)] = max(min(leg_action, INIT_LEG_POS + 0.45), INIT_LEG_POS - 0.78)

            # motor_pose[int(3 * i + 2)] = max(min(foot_pose, INIT_FOOT_POS + 0.63), INIT_FOOT_POS - 0.63)
            # if i==0 or i==3:
            #     motor_pose[int(3 * i + 1)] = leg_pose[int(3 * i + 1)]
    ...
    """
        ...
        trotting_signal = np.array([
            0.0, -sw_first_pair, ext_first_pair, 0.0, -sw_second_pair, ext_second_pair,
            0.0, -sw_second_pair, ext_second_pair, 0.0, -sw_first_pair, ext_first_pair
        ])
    ...
    """
        ...
        is_fallen = math.fabs(roll) > 0.3 or math.fabs(pitch) > 0.5
        return is_fallen
    ...
    """
        ...
        if self._use_signal_in_observation:
            observation.extend(self._transform_action_to_motor_command([0] * 12))
