...
class BaseExperiment(object):
    ...
    def log_asset_folder(self, folder, step=None, log_file_name=False, recursive=False):
        ...
        try:
            for file_name, file_path in log_asset_folder(folder, recursive):
                if log_file_name:
                    asset_url = self.log_asset(file_data=file_path, file_name=file_path)
                else:
            ...
            try:
                ...
                if os_packages_list is not None:
                    if self.config["comet.override_feature.sdk_os_packages_http"]:
                        ...
                        self.streamer.put_messge_in_q(os_message)
                    else:
