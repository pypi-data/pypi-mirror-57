...
class PluginSelector():
    ...
    def get_only_selected_graph_plugins_names(name_list):
        repository = get_plugin_repository()
        graph_plugin_names = [plugin.name for plugin in PluginSelector.get_all_graph_visualization_plugins()]
        return [repository.get_plugin(name) for name in name_list if repository.get_plugin(name) in graph_plugin_names]
