...
class ImageTag(Tag):
    def _to_html(self):
        ...
        return '<a href="{}">'.format(attributes['src']), '<img {}>'.format(self.renderer.html_attributes(attributes)), '</a>'
...
class CenterTag(Tag):
    def _to_html(self):
        return u'<div class="center">', self.get_content(), u'</div>',

class RightTag(Tag):
    def _to_html(self):
        return u'<div class="right">', self.get_content(), u'</div>',
...
class QuoteTag(Tag):
    ...
    def _to_html(self):
        pieces = ['<div class="alert alert-quote">']

        citation = self.params.get('quote')
        if citation:
            pieces.append('<strong>')
            pieces.append('<a href="/community/profile/{}">{}</a> said:'.format(citation, citation))
            pieces.append('</strong>')
            pieces.append('<br>')

        pieces.append(self.get_content())
        pieces.append('</div>')
