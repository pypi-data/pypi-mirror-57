...
class Updater:
    ...
    def save_updated_package(self, update_information: UpgradeInfo):
    ...
    def is_update_required(self) -> bool:
        ...
        :return:
        """
        response = requests.post(self.config.api_base_url + "/update", json={"package": self.package},
    ...
    def get_update_information(self) -> UpgradeInfo:
        """
        Gets information about the update from the server
        :return:
        """
        response = requests.post(self.config.api_base_url + "/update/details", json={"package": self.package},
                                 headers=self.headers)

        if response.status_code == 200:
            return UpgradeInfo(**response.json())
    ...
    def get_file_list(self) -> FileMetas:
        ...
        response = requests.post(self.config.api_base_url + "/update/files", json={"package": self.package},
                                 headers=self.headers)
    ...
    def get_updated_file(self, file: FileMeta) -> File:
        ...
        :return:
        """
        response = requests.post(self.config.api_base_url + "/update/file", json={"file_meta": file.dict()},
                                 headers=self.headers)
    ...
    def create_backup_readme(self, location, update_information: UpgradeInfo):
...
{update_information}""".format(update_information=update_information_str)
    ...
    def preview(self) -> Union[bool, UpgradeInfo]:
        """
    ...
    def update(self) -> Union[bool, UpgradeInfo]:
        """
        ...
        if self.is_update_required():
            ...
            update_information: UpgradeInfo = self.get_update_information()
