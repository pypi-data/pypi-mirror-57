...
def extracted_array_2d_from_array_2d(array_2d, y0, y1, x0, x1):
    """Resize an arrays to a new size by extracting a sub-set of the arrays.

    The extracted input coordinates use NumPy convention, such that the upper values should be specified as +1 the \
    dimensions of the extracted arrays.

    In the example below, an arrays of size (5,5) is extracted using the coordinates y0=1, y1=4, x0=1, x1=4. This
    extracts an arrays of dimensions (3,3) and is equivalent to array_2d[1:4, 1:4].

    This function is necessary work with numba jit tags and is why a standard Numpy arrays extraction is not used.

    Parameters
    ----------
    array_2d : ndarray
        The 2D arrays that an arrays is extracted from.
    y0 : int
        The top row number (e.g. the higher y-coodinate) of the arrays that is extracted for the resize.
    y1 : int
        The bottom row number (e.g. the lower y-coodinate) of the arrays that is extracted for the resize.
    x0 : int
        The left column number (e.g. the lower x-coodinate) of the arrays that is extracted for the resize.
    x1 : int
        The right column number (e.g. the higher x-coodinate) of the arrays that is extracted for the resize.

    Returns
    -------
    ndarray
        The extracted 2D arrays from the input 2D arrays.
...
def resized_array_2d_from_array_2d(
    ...
    """Resize an arrays to a new size around a central pixel.

    If the origin (e.g. the central pixel) of the resized arrays is not specified, the central pixel of the arrays is \
    calculated automatically. For example, a (5,5) arrays's central pixel is (2,2). For even dimensions the central \
    pixel is assumed to be the lower indexed value, e.g. a (6,4) arrays's central pixel is calculated as (2,1).
    ...
    pad_value : float
        If the reszied arrays is bigger in size than the input arrays, the value the padded edge values are filled in \
        using.
...
def replace_noise_map_2d_values_where_image_2d_values_are_negative(
    ...
    """If the values of a 2D image arrays are negative, this function replaces the corresponding 2D noise-map arrays \
    ...
    ----------
    image_2d : ndarray
        The 2D image arrays used to locate the pixel indexes in the noise-map which are replaced.
    noise_map_2d : ndarray
        The 2D noise-map arrays whose values are replaced.
...
def numpy_array_1d_to_fits(array_1d, file_path, overwrite=False):
    """Write a 1D NumPy arrays to a .fits file.

    Before outputting a NumPy arrays, the arrays is flipped upside-down using np.flipud. This is so that the structures \
    appear the same orientation as .fits files loaded in DS9.

    Parameters
    ----------
    array_2d : ndarray
        The 2D arrays that is written to fits.
    file_path : str
...
def numpy_array_1d_from_fits(file_path, hdu):
    """Read a 2D NumPy arrays to a .fits file.

    After loading the NumPy arrays, the arrays is flipped upside-down using np.flipud. This is so that the structures \
    appear the same orientation as .fits files loaded in DS9.
    ...
    hdu : int
        The HDU extension of the arrays that is loaded from the .fits file.

    Returns
    -------
    ndarray
        The NumPy arrays that is loaded from the .fits file.
...
def numpy_array_2d_to_fits(array_2d, file_path, overwrite=False):
    """Write a 2D NumPy arrays to a .fits file.
...
def index_2d_for_index_1d_from_shape(indexes_1d, shape):
    """For pixels on a 2D arrays of shape (rows, columns), map an arrays of 1D pixel indexes to 2D pixel indexes.

    Indexing is defined from the top-left corner rightwards and downwards, whereby the top-left pixel on the 2D arrays
    corresponds to index 0, the pixel to its right pixel 1, and so on.

    For a 2D arrays of shape (3,3), 1D pixel indexes are converted as follows:
    ...
    --------
    ndarray
        An arrays of 2d pixel indexes with dimensions (total_indexes, 2).

    Examples
    --------
    indexes_1d = np.arrays([0, 1, 2, 5])
...
def index_1d_for_index_2d_from_shape(indexes_2d, shape):
    """For pixels on a 2D arrays of shape (rows, colums), map an arrays of 2D pixel indexes to 1D pixel indexes.

    Indexing is defined from the top-left corner rightwards and downwards, whereby the top-left pixel on the 2D arrays
    corresponds to index 0, the pixel to its right pixel 1, and so on.

    For a 2D arrays of shape (3,3), 2D pixel indexes are converted as follows:
    ...
    --------
    ndarray
        An arrays of 1d pixel indexes with dimensions (total_indexes).

    Examples
    --------
    indexes_2d = np.arrays([[0,0], [1,0], [2,0], [2,2]])
...
def sub_array_1d_from_sub_array_2d(sub_array_2d, mask, sub_size):
    """For a 2D sub arrays and mask, map the values of all unmasked pixels to a 1D sub-arrays.

    A sub-arrays is an arrays whose dimensions correspond to the hyper arrays (e.g. used to make the grid) \
    multiplid by the sub_size. E.g., it is an arrays that would be generated using the sub-grid and not binning \
    up values in sub-pixels back to the grid.

    The pixel coordinate origin is at the top left corner of the 2D arrays and goes right-wards and downwards,
    with sub-pixels then going right and downwards in each pixel. For example, for an arrays of shape (3,3) and a \
    sub-grid size of 2 where all pixels are unmasked:

    - pixel [0,0] of the 2D arrays will correspond to index 0 of the 1D arrays.
    - pixel [0,1] of the 2D arrays will correspond to index 1 of the 1D arrays.
    - pixel [1,0] of the 2D arrays will correspond to index 2 of the 1D arrays.
    - pixel [2,0] of the 2D arrays will correspond to index 4 of the 1D arrays.
    - pixel [1,0] of the 2D arrays will correspond to index 12 of the 1D arrays.

    Parameters
    ----------
    sub_array_2d : ndarray
        A 2D arrays of values on the dimensions of the sub-grid.
    mask : ndarray
        A 2D arrays of bools, where *False* values mean unmasked and are included in the util.
    array_2d : ndarray
        The 2D arrays of values which are mapped to a 1D arrays.

    Returns
    --------
    ndarray
        A 1D arrays of values mapped from the 2D arrays with dimensions (total_unmasked_pixels).

    Examples
    --------

    sub_array_2d = np.arrays([[ 1.0,  2.0,  5.0,  6.0],
                             [ 3.0,  4.0,  7.0,  8.0],
                             [ 9.0, 10.0, 13.0, 14.0],
                             [11.0, 12.0, 15.0, 16.0])

    mask = np.arrays([[True, False],
                     [False, False]])
...
def sub_array_2d_from_sub_array_1d(sub_array_1d, mask, sub_size):
    ...
    values back to the original 2D arrays where masked values are set to zero.
    ...
    sub_array_1d : ndarray
        The 1D arrays of values which are mapped to a 2D arrays.
    shape : (int, int)
        The shape of the 2D arrays which the pixels are defined on.
    ...
    --------
    ndarray
        A 2D arrays of values mapped from the 1D arrays with dimensions shape.

    Examples
    --------
    one_to_two = np.arrays([[0,1], [1,0], [1,1], [1,2], [2,1]])

    array_1d = np.arrays([[2.0, 4.0, 5.0, 6.0, 8.0])
