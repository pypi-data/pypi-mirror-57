...
def select_pr(project):
    if github.user_is_admin(project):
        ...
        logger.warning("Sei admin del repository, puoi fare il merge skippando i check (CI, review, ecc...)\
        ...
        prs = list(filter(lambda pr: pr.mergeable and pr.mergeable_state == "clean", github.get_list_pr(project)))

    if prs:
        choices = [{"name": "{} {}".format(i.number, i.title), "value": i} for i in prs]
        return prompt_utils.ask_choices('Seleziona PR: ', choices)
    else:
        logger.warning("Non esistono pull request pronte per il merge, per favore controlla su https://github.com/primait/{}/pulls".format(project))
        sys.exit(-1)
