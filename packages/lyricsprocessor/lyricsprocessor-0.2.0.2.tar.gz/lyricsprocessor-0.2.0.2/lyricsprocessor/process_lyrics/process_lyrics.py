...
def get_verse_keys(lyrics:str):
    ...
    return verse_keys
########################################################################################################################
def remove_short_songs(df, min_length_song):
    rows_to_keep = []
    for index, row in df.iterrows():
        if len(row['lyrics'].split(' ')) > min_length_song:
...
def clean_df(df:DataFrame,
             ...
             min_length_song:int=min_length_song):
    artist_id = df['artist_id']
    df[id_col] = df[id_col].apply(lambda x: x['id'])
    df = df[df[id_col] == artist_id] # only keep rows where primary artist is the one we are after
    ...
    df.dropna(inplace=True, subset=['lyrics', 'title'])
    df = remove_short_songs(df, min_length_song)
    return df
...
def main(api_key:str,
    ...
    df = clean_df(df,
                  ...
                  min_length_song=min_length_song)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_length_song', default=30, type=int,
                        help='what should the minimum length song be?')
    args = parser.parse_args()

    logging.basicConfig(format='%(process)d-%(levelname)s-%(message)s')

    df = load_df(args.df_path, args.hdf_key)
    main(args.api_key, df, args.hdf_key, args.check_lyrics_state, args.save_path, args.min_length_song)