...
class QalxJob:
    ...
    @property
    def log(self):
        """An instance of `worker_logger` to enable logging from step functions"""
    ...
    def publish_status(self, status="", status_string="", status_dict=None):
        ...
        """
        if status_string:  # pragma: no cover
            the_warning = deprecation.DeprecatedWarning(
                function="status_string",
                deprecated_in="0.8",
                removed_in="1.0",
                details="`status_string` will be renamed to `status` in future "
                        "versions to maintain consistency with Bots"
            )
            warnings.warn(the_warning, category=DeprecationWarning,
                          stacklevel=2)
            # If the user supplies both arguments through the deprecation
            # period we won't know which to use
            assert not all([status, status_string]), \
                'Ensure only `status` or `status_string` is specified'
            if status_string and not status:
                # Ensure that we use the status they passed through as a kwarg
                status = status_string

        if status_dict is not None:  # pragma: no cover
            the_warning = deprecation.DeprecatedWarning(
                function="status_dict",
                deprecated_in="0.8",
                removed_in="1.0",
                details="Data stored in `status_dict` is not saved by the API."
                        "The argument will be removed in future versions."
            )
            warnings.warn(the_warning, category=DeprecationWarning,
                          stacklevel=2)
    ...
    def last_step_result(self):
        """
        The last :class:`~pyqalx.bot.StepResult` object (or None if no previous steps)
        for this job
        """
    ...
    def add_item_data(self, item_key, data, meta=None):
        ...
        :return: The specific :class:`~pyqalx.core.entities.item.Item` instance that has been added
        """
    ...
    def save_entity(self):
        ...
        :return: a child of :class:`~pyqalx.core.entities.entity.QalxEntity` of the correct
                 type based on the type of `self.entity`
        """
    ...
    def delete_message(self):
        ...
        """
        return self._worker_entity._delete_message()


class StepResult(NamedTuple):
    """
    An NamedTuple representing the result of a single step function.
    """
    step_name: str = ""  #: The name of the step function that this instance refers to
    success: bool = False  #: Was this step successful or not?
    result_data: dict = {}  #: A dict of data for the result of the step
...
class Bot:
    __slots__ = (
        ...
        "ontermination_function", "bot_entity", "bot_adapter", "qalx_session_class", "user_config_class", "bot_config_class",
        "entity_classes"
    )
    ...
    def __init__(self,
        ...
        :type: list
        """
        self.name = f'{bot_name}-{platform.node()}'
        ...
        # to the exception.
        # TODO:Figure out a nicer way to get the step_name in here
        self.process_function = partial(cannot_be_undefined,
                                        step_name='process')
        ...
        self.qalx_session_class = qalx_session_class
        self.user_config_class = user_config_class
        self.bot_config_class = bot_config_class
        self.entity_classes = entity_classes

        if self.entity_classes is None:
            self.entity_classes = []
            ...
            if fn.endswith("_function"):
                ...
                if fn_attr not in [do_nowt, do_nowt_right]:
                    if getattr(fn_attr, 'func', None) == cannot_be_undefined:
                        ...
                        # an exception pre fork.  Only functions using `partial`
                        # will have the `func` attr
                        fn_attr()
    ...
    def start(self,
        ...
        :type processes: int
        :param add_queue: Should the queue be created if it doesn't already
                          exist
        :type add_queue: bool
        """

        if self.qalx_session_class is None:
            self.qalx_session_class = QalxSession

        user_session = self.qalx_session_class(profile_name=user_profile_name,
                                               config_class=self.user_config_class,
                                               skip_ini=skip_ini)

        if self.bot_config_class is None:
            self.bot_config_class = BotConfig

        # This configures logging to use the config settings specified
        # on the bot
        bot_session = self.qalx_session_class(
            profile_name=bot_profile_name,
            config_class=self.bot_config_class,
            skip_ini=skip_ini)
        if add_queue:
            # We use `get_or_create` to stop the bot erroring because
            # the user didn't know the queue already existed
            the_warning = deprecation.DeprecatedWarning(
                function="add_queue",
                deprecated_in="0.7",
                removed_in="1.0",
                details="Bots now use `get_or_create` when given a "
                        "`queue_name` so the user no longer has the "
                        "responsibility to create the queue before starting "
                        "the bot"
            )
            warnings.warn(the_warning, category=DeprecationWarning,
                          stacklevel=2)
            ...
            # We create the queue if it doesn't already exist
            queue = user_session.queue.get_or_create(name=self.queue_name)
    ...
    def initialisation(self, func):
        ...
        authenticated againt the `self.user_config_class`
...
class Worker(Process):
    ...
    def __init__(self, steps, queue, bot_entity, api_entity,
    ...
    def _check_queue_credentials(self):
        ...
        if self.queue._credentials_expiring_soon():
            self.logger.debug(f'refreshing security credentials for queue '
                              f'`{self.queue["name"]}`')
    ...
    def _run_functions(self):
        ...
        while True:
            if self.stop:
                ...
                q_msgs = self.queue_adapter.get_messages(queue=self.queue)
