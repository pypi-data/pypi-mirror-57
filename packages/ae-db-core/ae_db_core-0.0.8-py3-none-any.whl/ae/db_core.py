...
and an easy-to-use interface, that allows your application to switch
between database dynamically - simply by selecting another :ref:`configuration file <config-files>`.
...
:pep:`Python DB API <249>` compatible connection object::
    ...
    class SpecificDb(DbBase):
        def connect(self) -> str:
            self.last_err_msg = ''
            try:
               self.conn = SpecificConnection(**self.connect_params())
...
The helper method :meth:`~DbBase.connect_params` is merging all database credentials and features
into a dict for to be passed as connection parameters to the database-specific connection
class/function/factory of the database driver.
...
Your class SpecificDb does also inherit from :class:`DbBase` useful methods
...
for to :meth:`SELECT <DbBase.select>` queries and
...
from ae.core import DEBUG_LEVEL_ENABLED, try_eval               # type: ignore  # for mypy
from ae.lockname import NamedLocks                              # type: ignore  # for mypy
from ae.sys_core import SystemBase, SystemConnectionBase        # type: ignore  # for mypy
...
class DbBase(SystemConnectionBase, ABC):
    ...
    def __init__(self, system: SystemBase):
        ...
        :param system:      SystemBase instance. Providing useful attributes, like e.g.:

                            **console_app**: instance of the application using this database.

                            **credentials**: dict with database driver specific account credentials, like e.g.:
                            ...
                            **features**: list of features. Features will also be passed to the database driver.
        ...
        credentials = self.system.credentials
        dsn = credentials.get('DSN') or credentials.get('dsn')
        self.dsn = dsn if dsn and isinstance(dsn, str) else f"DbBase{system.sys_id}{DbBase.instances}"  #: db src name
    ...
    @abstractmethod
    def connect(self) -> str:
        """ sub-class has to overwrite this connect method """
    ...
    def connect_params(self) -> Dict[str, Any]:
        """ merges self.credentials with self.features into a database driver connection parameters dict.
