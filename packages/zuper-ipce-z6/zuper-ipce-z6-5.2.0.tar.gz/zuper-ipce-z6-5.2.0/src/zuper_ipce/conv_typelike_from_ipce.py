...
from zuper_typing.constants import BINDINGS_ATT, GENERIC_BOUND_ATT, PYTHON_36
...
def typelike_from_ipce_dataclass(
    ...
    setattr(T, BINDINGS_ATT, bindings)
    orig = res.get(X_ORIG, [])  # iji

    setattr(T, GENERIC_BOUND_ATT, tuple(make_TypeVar(_) for _ in orig))
    return SRE(T, used)
