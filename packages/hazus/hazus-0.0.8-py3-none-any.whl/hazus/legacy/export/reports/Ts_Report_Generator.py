...
def create_plots(gdf, hazard_gdf, out_path, breaks):
    ...
    color_array = pd.cut(color_vals, bins=([0] + list(breaks.bins)), labels=[x[0] + 1 for x in enumerate(list(breaks.bins))])
...
def generate_report(gdf, hazus_results_dict, subcounty_results, county_results, inputs):
    ...
    try:
        ...
        breaks = nb(subcounty_results['EconLoss'], k=5)
        print(time() - t0)
        if len(breaks.bins) > 1:
            legend_item1 = breaks.bins[0]
            legend_item2 = breaks.bins[1]
            legend_item3 = breaks.bins[2]
            legend_item4 = breaks.bins[3]
            legend_item5 = breaks.bins[4]
        else: 
            legend_item1 = breaks.bins[0]
            legend_item2 = breaks.bins[0]
            legend_item3 = breaks.bins[0]
            legend_item4 = breaks.bins[0]
            legend_item5 = breaks.bins[0]
        ...
        template_pdf_path = './assets/templates/' + template + '.pdf'
        data_pdf_path = out_path + str(uuid.uuid1())+'.pdf'
        write_fillable_pdf(template_pdf_path, data_pdf_path, data_dict)
