...
from vtorch.common.utils import is_lazy, lazy_groups_of, ensure_list
...
class DataIterator(Registrable):
    ...
    max_instances_in_memory : ``int``, optional, (default = None)
        If specified, the iterators will load this many instances at a time into an
    ...
    def __init__(self,
        ...
        self._fields_with_pretrained_tokenizers = fields_with_pretrained_tokenizers
            ...
            if self._cache_instances and key in self._cache:
                ...
                for batch in batches:
                    ...
                    if self.vocab is not None:
                        batch.index_instances(self.vocab, self._fields_with_pretrained_tokenizers)

                    padding_lengths = batch.get_padding_lengths()
                    logger.debug("Batch padding lengths: %s", str(padding_lengths))
                    logger.debug("Batch size: %d", len(batch.instances))
                    tensor_dict = batch.as_tensor_dict(padding_lengths=padding_lengths, batch_first=self.batch_first)
    ...
    def _memory_sized_lists(self,
        ...
        # Get an iterators over the next epoch worth of instances.
        ...
        # into memory. So we convert the whole iterators to a list:
        else:
            yield list(iterator)
    ...
    def _ensure_batch_is_sufficiently_small(
        ...
        while excess:
            ...
            if self.vocab is not None:
                ...
                instance.index_fields(self.vocab, self._fields_with_pretrained_tokenizers)
