...
from vtorch.common.utils import namespace_match
from vtorch.common import Params, Registrable
from vtorch.common.checks import ConfigurationError
from vtorch.common.tqdm import Tqdm
from vtorch.common import loading
from vtorch.data import instance as adi  # pylint: disable=unused-import
...
def _read_pretrained_tokens(embeddings_file_path: str) -> List[str]:
    logger.info('Reading pretrained tokens from: %s', embeddings_file_path)
    tokens: List[str] = []
    model = loading.load_pickled_w2v_embeddings(embeddings_file_path)
...
class Vocabulary(Registrable):
    ...
    pretrained_files : ``Dict[str, str]``, optional
        ...
        in the Vocabulary.
    limit_pretrained_embeddings : ``Dict[str, int]``, optional
        If provided, specifies for each namespace a number of lines to keep from pretrained
        embedding files, even for words not appearing in the data.
    ...
    def __init__(self,
                 ...
                 limit_pretrained_embeddings: Dict[str, int] = None) -> None:
    ...
    def save_to_files(self, directory: str) -> None:
        ...
        for namespace, mapping in self._index_to_token.items():
            ...
            with codecs.open(os.path.join(directory, namespace + '.txt'), 'w', 'utf-8') as token_file:
                ...
                start_index = 1 if mapping[0] == self.padding_token else 0
    ...
    def set_from_file(self,
        ...
        """
        if is_padded:
            self._token_to_index[namespace] = {self.padding_token: 0}
            self._index_to_token[namespace] = {0: self.padding_token}
        else:
        ...
        if is_padded:
            assert self.oov_token in self._token_to_index[namespace], "OOV token not found!"
    ...
    def _extend(self,
        ...
        limit_pretrained_embeddings = limit_pretrained_embeddings or {}
        ...
        for namespace in counter:
            if namespace in pretrained_files:
                ...
                min_embeddings = limit_pretrained_embeddings.get(namespace, 0)
                pretrained_list = pretrained_list[:min_embeddings] if min_embeddings > 0 else pretrained_list
                if min_embeddings > 0:
                    tokens_old = tokens_to_add.get(namespace, [])
                    tokens_new = pretrained_list.copy()
    ...
    def extend_from_instances(self,
        ...
        self._extend(counter=namespace_token_counts,
                     ...
                     only_include_pretrained_words=only_include_pretrained_words,
                     tokens_to_add=tokens_to_add,
                     limit_pretrained_embeddings=min_pretrained_embeddings)
    ...
    def is_padded(self, namespace: str) -> bool:
        ...
        """
        return self._index_to_token[namespace][0] == self.padding_token
    ...
    def get_token_index(self, token: str, namespace: str = 'tokens') -> int:
        if token in self._token_to_index[namespace]:
            ...
            try:
                return self._token_to_index[namespace][self.oov_token]
            except KeyError:
