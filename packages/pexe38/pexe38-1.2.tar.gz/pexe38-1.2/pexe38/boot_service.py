...
import winerror
# We assume that pexe38 has magically set service_module_names
# to the module names that expose the services we host.
service_klasses = []
try:
    service_module_names
except NameError:
    print("This script is designed to be run from inside pexe38")
    sys.exit(1)
...
if cmdline_style == "pexe38":
    # Simulate the old pexe38 service command line handling (to some extent)
    ...
    # Unlike services implemented in .py files, when a pexe38 service exe is
