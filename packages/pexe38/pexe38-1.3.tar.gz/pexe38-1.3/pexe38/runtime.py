...
##             self.service_exe_files.append(dst)
    ...
    def build(self):
        ...
        if options.libname is not None:
            ...
            dll_bytes = pkgutil.get_data("pexe38", "resources.dll")
    ...
    def get_runstub_bytes(self, target):
        ...
        exe_bytes = pkgutil.get_data("pexe38", run_stub)
...
#                 resource.add_string(1000, "pexe38")
    ...
    def _create_script_data(self, target):
        ...
        if target.exe_type == "service":
            ...
            boot_code = compile(pkgutil.get_data("pexe38", "boot_service.py"),
                                "boot_service.py", "exec",
                                optimize=self.options.optimize)
            code_objects.append(boot_code)

        elif target.exe_type in ("console_exe", "windows_exe"):
            boot_code = compile(pkgutil.get_data("pexe38", "boot_common.py"),
                                "boot_common.py", "exec",
                                optimize=self.options.optimize)
            ...
            boot_code = compile(pkgutil.get_data("pexe38", "boot_ctypes_com_server.py"),
