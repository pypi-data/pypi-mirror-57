...
class MarathonSpawner(Spawner):
    # Load the app image
    image = Unicode("jupyterhub/singleuser", config=True)

    # The command to run 
    cmd = Unicode("jupyter notebook", config=True)
    ...
    def container_name(self):
        server_name = ''.join(random.choice(string.ascii_lowercase) for i in range(5))
        if self.name:
    ...
    def get_app_cmd(self):
        retval = self.cmd.replace("{username}", self.user.name)
    ...
    def get_env(self):
        ...
        env['JUPYTERHUB_ACTIVITY_URL'] = hub_api_url+"/users/"+self.user.name+"/activity"
    ...
    def start(self):
        ...
        while True:
            ...
            self.log.info("Checking spawned app %s healthy = %d", app_info.id, app_info.tasks_healthy)
            if app_info and app_info.tasks_healthy == 1:
                ip, port = self.get_ip_and_port(app_info)
                break
            yield gen.sleep(2)
        self.log.info("Spawned App Healthy! %s, %s", ip, port)
        return (ip, port)
    ...
    def _options_form_default(self):
      ...
      <input id="cpu" name="cpu" class="form-control" value="{cpu}"></input>
      </div>
      <br>
              <div class="form-group">

      <label for="mem">Memory</label>
      <input id="mem" name="mem" class="form-control" value="{mem}"></input>
      </div>
      <br>
              <div class="form-group">
