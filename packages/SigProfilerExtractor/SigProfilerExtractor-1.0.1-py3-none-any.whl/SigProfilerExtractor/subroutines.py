...
def inhouse_nmf(v, w=0, h=0, k=2, iterations=200000, tol= 0.000001):
    #np.random.seed(1)
    v = np.float32(v)
    #min_v = np.min(v[v>0])
    #min_v = 0.000000001
    ...
    #h = np.random.rand(h.shape[0], h.shape[1])
    conv = 1
    init_l2 = 100000000 # a big number
    for i in range(iterations):
        ...
        if (i+1)%250==0:
            ...
            current_l2 = calculate_similarities(v, est_v, sample_names=False)[0].iloc[:,-2]
            current_l2 = np.mean(current_l2)
            diff = abs(current_l2-init_l2)
            init_l2=current_l2
            #print(diff)
            if diff <= tol:
                conv+=1
...
def nnmf(genomes, nfactors, init="nndsvd"):
    ...
    w, h = initialize_nmf(genomes, nfactors , init=init)
    W, H = inhouse_nmf(genomes, w=w, h=h, k=nfactors, iterations=300000, tol=0.0001)
