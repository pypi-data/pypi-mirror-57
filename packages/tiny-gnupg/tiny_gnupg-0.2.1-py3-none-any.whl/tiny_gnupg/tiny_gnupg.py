...
class GnuPG:
    ...
    def read_output(self, command=None, inputs=b"", shell=False):
        try:
            return sp.check_output(
                command, input=inputs, shell=shell
            ).decode()
    ...
    def encrypt(self, message="", to=""):
        command = self.command("-esar", quote(to), with_passphrase=True)
    ...
    def raw_list_keys(self, uid=""):
        if uid:
            command = self.command("--list-keys", quote(uid))
        else:
    ...
    def format_list_keys(self, raw_list_keys_terminal_output):
        ...
        return dict(zip(fingerprints, names))
    ...
    async def network_import(self, uid=""):
        ...
        async with self.session as session:
            async with session.get(id_link) as response:
    ...
    async def network_export(self, uid=""):
        ...
        response = await self.raw_network_verify(payload)
    ...
    def text_export(self, uid=""):
        command = self.command("-a", "--export", quote(uid))
        return self.read_output(command)
