...
from izzy.misc import ArrayLike, assert_type
...
class GenericModel(ABC):
    ...
    def confusion_matrix(self, x, y):
        """
        Computes the confusion matrix for the model instance
...
def _accuracy(cm):
    ...
    # The index must be ordered the same way
    assert all(cm.index.values == classes)

    # Assuming we pass all these tests, convert to array
    cm = cm.values
...
def _get_tp_tn_fp_fn(y_true, y_pred, threshold=0.5):
    """
    Computes the confusion matrix and returns as a flattened dictionary
...
# Weight
def _weight(sample_weights=None, class_weights=None):
    """
    Creates weights for a model

    Parameters
    ----------
    sample_weights : ArrayLike
        Weights for each sample
    class_weights : ArrayLike
...
# Accuracy
def accuracy(y_true, y_pred, threshold=None):
    r"""
    ...
    In the case of multiclass, we can compute the accuracy by getting the trace of the confusion matrix and
    ...
    # Type check
    assert_type(y_true, ArrayLike, error_message='y_true must be ArrayLike')
    assert_type(y_pred, ArrayLike, error_message='y_pred must be ArrayLike')
    assert_type(threshold, float, none_okay=True, error_message='threshold must be float')

    # Return accuracy
    return _accuracy(confusion_matrix(y_true, y_pred, threshold=threshold))
...
def aic(log_likelihood, degrees_of_freedom):
    ...
    # Type check
    assert_type(log_likelihood, Number, error_message='log_likelihood must be number')
    assert_type(degrees_of_freedom, Number, error_message='degrees_of_freedom must be number')
...
def bic(log_likelihood, degrees_of_freedom, num_samples):
    ...
    assert_type(log_likelihood, Number, error_message='log_likelihood must be number')
    assert_type(degrees_of_freedom, Number, error_message='degrees_of_freedom must be number')
    assert_type(num_samples, int, error_message='num_samples must be integer')
...
def classify(y_prob, classes=None, threshold=None):
    ...
    # Type check
    assert_type(y_prob, ArrayLike, error_message='y_pred must be ArrayLike')
    assert_type(classes, ArrayLike, none_okay=True, error_message='classes must be ArrayLike')
    assert_type(threshold, float, none_okay=True, error_message='threshold must be float')

    # Convert y_prob to numpy array for convenience
    y_prob = np.array(y_prob)

    # If y_prob only has 1 dimensions, assume that this is a binomial problem and coerce it into [1 - y_pred, y_pred]
    if y_prob.ndim == 1:
        Warning('coercing into binomial form [1 - y_pred, y_pred]')
...
def confusion_matrix(y_true, y_pred, threshold=0.5, sample_weights=None):
    """
    ...
    y_true : ArrayLike
        True outcomes
    y_pred : ArrayLike
        Predicted outcomes. If probability, threshold will be passed to :func:`~classify` for classification
    threshold : float
    ...
    # Turn y_pred into classes if necessary
