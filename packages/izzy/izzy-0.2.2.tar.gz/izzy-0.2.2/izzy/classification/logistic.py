...
from .generic import _format_weight, format_x, GenericModel
...
class LogisticRegression(_LogisticRegression, GenericModel):
    ...
    # Degrees of freedom
    def degrees_of_freedom(self, weight=None, axis=1):
        """
        ...
        # Format input
        weight = _format_weight(weight, n=self.n_obs)

        # If axis = 0, DOF = # obs - # obs with 0 weight - # features (including intercept if present)
        if axis == 0:
            d = self.n_obs - np.sum(equal(weight, 0)) - self.n_features - self.fit_intercept
