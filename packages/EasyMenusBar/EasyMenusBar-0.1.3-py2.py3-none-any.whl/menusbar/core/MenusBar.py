...
class MemusBar(QWidget):
    ...
    def handlerBeforeEvent(self, type: int, filePath: str):
        pass
    ...
    def actionHandler(self):
        ...
        if type == "create_dir":
            ...
            if text:
                self.handlerBeforeEvent(self.HANLDER_CREATE_DIR, os.path.join(self.getDir(fileInfo), text))
                createDir(self.getDir(fileInfo), text)
                self.handlerAfterEvent(self.HANLDER_CREATE_DIR, os.path.join(self.getDir(fileInfo), text))

        elif type == "delete_dir":
            self.handlerBeforeEvent(self.HANLDER_DELETE_DIR, fileInfo.filePath())
            deleteDir(fileInfo.filePath())
            self.handlerAfterEvent(self.HANLDER_DELETE_DIR, fileInfo.filePath())
            ...
            if len(temp) < 2:
                ...
                if self.defualtSuffixList:
                    ...
                    text = text + "." + self.defualtSuffix
            if text:
                self.handlerBeforeEvent(self.HANLDER_CREATE_FILE, os.path.join(self.getDir(fileInfo), text))
                createFile(self.getDir(fileInfo), text)
                self.handlerAfterEvent(self.HANLDER_DELETE_FILE, os.path.join(self.getDir(fileInfo), text))

        elif type == "delete_file":
            self.handlerBeforeEvent(self.HANLDER_DELETE_FILE, file)
            deleteFile(file)
            self.handlerAfterEvent(self.HANLDER_DELETE_FILE, file)
