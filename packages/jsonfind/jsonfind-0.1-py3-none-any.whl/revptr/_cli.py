...
from .revptr import Revptr, format_list, find_format_list
...
def find_by(verbose, obj, query, format):
    ...
    res = Revptr.find_by(mode=format, obj=json.load(obj), path=query)
    click.echo(res)
...
def find_eq(verbose, obj, target, format):
    ...
    result = [Revptr.format_to(format, x)
              for x in Revptr.filter_eq(obj, target)]
...
def find_is(verbose, obj, target, format):
    ...
    result = [Revptr.format_to(format, x)
              for x in Revptr.filter_is(obj, target)]
    log.debug("result: %s", result)
    click.echo(result)
...
def find_subset(verbose, obj, target, format):
    ...
    result = [Revptr.format_to(format, x)
              for x in Revptr.filter_subset(obj, target)]
    log.debug("result: %s", result)
    click.echo(result)
...
def find_key(verbose, obj, target, format):
    log.debug("finding(subset) %s from %s", target, obj)
    result = [Revptr.format_to(format, x)
              for x in Revptr.filter_key(obj, target)]
    log.debug("result: %s", result)
    click.echo(result)
