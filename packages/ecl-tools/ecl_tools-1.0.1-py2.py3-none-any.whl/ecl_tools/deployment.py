...
from fabric import colors  # type: ignore
import boto
import cssmin  # type: ignore
import gzip
import jsmin  # type: ignore
import slimit  # type: ignore
...
def sync_media_to_s3(
    ...
    conn = boto.connect_s3(
        aws_access_key_id, aws_access_key_secret, calling_format=OrdinaryCallingFormat()
    )
    ...
    def upload_file(remote_key):
        ...
        with open(folder + remote_key, "b") as fp:
            fb = BytesIO()
            fb.write(fp.read())
            ...
            if computed_md5 != existing_md5:
                ...
                key.set_contents_from_string(gzipped_file.getvalue(), headers=headers)
                key.make_public()
    ...
    if cf_distribution_id:
        cf_conn = boto.connect_cloudfront(aws_access_key_id, aws_access_key_secret)
        cf_conn.create_invalidation_request(cf_distribution_id, paths_to_invalidate)
