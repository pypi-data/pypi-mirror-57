# Copyright (C) 2014-2019 by Dr. Dieter Maurer <dieter@handshake.de>
...
class ClientsUd(UdMixin):
  ...
  def receipts(self):
    ...
    for cd in self.collection.list():
      cl = clients.get_item(cd.client_id)
      prefix, suffix = ("<div class=\"noprint\">", "</div>") \
                       if cl.special else ("", "")
      cdp = getMultiAdapter((c, cd), IProxy)
      v = ReceiptsClientUd(cdp, r)
      v.headless = True
##      rl.append("<h4 class="noprint">%s</h1>" % 
...
##        + v()
##      )
      rl.append(prefix + v() + suffix)
    return self.page(
      body="<div class=\"pagesep\"></div>".join(rl)
      )

  def emails(self):
    cs = []
    self.title = self.translate(_(u"Emails"))
    for cd in self.collection.list((("delivery_date", None, "="),)):
      cs.append(clients.get_item(cd.client_id))
    return self.page(
      body=", ".join('"%s" &lt;%s&gt;' % (c.title, c.email)
                     for c in cs if not c.special)
      )
...
class ClientEditForm(CrudEditForm):
  ...
  def update(self):
    ...
    tr = self._crud().translate
    self.bottom = """<h4 class="summary">%s</h4>
    ...
    if cd.delivery_date is not None:
      ...
      accv = cd.account - total_price
      acc = "<b>%s EUR</b> (%s) <b> - %s EUR</b> (%s) <b> = %s EUR</b> (%s)" % (
        cd.account, tr(_(u"Old account")),
        total_price, tr(_(u"Delivery")),
        accv, tr(_(u"New account")),
        )
    self.bottom += acc
    self.accv = accv

  @property
  def title_extension(self):
    return "%s EUR" % self.accv
...
class ClientUd(UdMixin):
  ...
  @property
  def title_extension(self):
    return self.subforms[0].title_extension


class ReceiptsClientEditForm(ClientEditForm):
  buttons = Buttons()
  handlers = Handlers()
  
class ReceiptsClientUd(ClientUd):
  EDIT_FORM_FACTORY = ReceiptsClientEditForm

  @property
  def crud_fields(self):
    return super(ReceiptsClientUd,self).crud_fields.select(
      "article_title", "article_order_no",
      "units", "unit_price",
      "total_price",
      )
