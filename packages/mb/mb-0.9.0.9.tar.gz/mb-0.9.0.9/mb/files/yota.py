...
class Mixtape():
    ...
    def vlc_chapter_iter(self, fullscreen=False):
        def vlc_iter():
            for item in self:
                if fullscreen == False:
                    vlc_quiter = item.vlc()
...
class Sample():
    ...
    def vlc(self, fullscreen=False, quit=True):
...
class Cue():
    ...
    def srt_search(self, keyword, sample_length=7, min_interval=10, mixtape=True, search_yota=False):
        ...
        result = srt.main(keyword, self.youtube_hash, min_interval = min_interval, search_yota=search_yota, start_time=self.time_start)
        if mixtape:
            if result:
