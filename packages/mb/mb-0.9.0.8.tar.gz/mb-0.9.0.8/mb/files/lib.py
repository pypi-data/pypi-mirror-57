...
class ommString(str):
    ...
    #     new_str = str(self).replace(str(value), "")
    #     newOmmStr = ommString(new_str)
    #     result = newOmmStr.strip("..")
    #     result = result.replace("....","..")
    #     final = ommString(result)
    #     return final
...
class Convert():
    ...
    def sync():
        ...
        if resp:
            ...
            print(resp)
        function_logger.debug(f"synced with server skillporn.tv/post, server response {resp}")
    ...
    def _browser_open(*input_url):
        ...
        function_logger.debug(f"opened {inputstr} in browser tab")
    ...
    def vlc_open(self, fullscreen=False):
            ...
            function_logger.debug(f"opened URL {self.url} in VLC")
    ...
    def search_to_mixtape(search_string, clip_length=0, time_end_str="", time_start_str=""):
        ...
        function_logger.debug(f"search_to_mixtape() called with arg '{search_string}', result {' '.join(urls)}")
    ...
    def omm(omm_str, remember=True):
        ...
        if result:
            function_logger.debug(f"called omm() with arg {omm_str}, YouTube URL detected, result {result}")
            omm_str = result
        # check for .yota file
        ...
        if m:
            ...
            function_logger.debug(f"called omm() with arg {omm_str}, yota or bit detected, result {result}")
            return(result)
        else:
