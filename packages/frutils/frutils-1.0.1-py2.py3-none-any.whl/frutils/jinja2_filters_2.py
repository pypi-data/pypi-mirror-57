...
def string_for_boolean_filter(value, true_value, false_value):
    """
    Returns a different string depending on whether the value resolves to True or not.
    ...
    if not isinstance(true_value, string_types):
        raise FrklException("'true' value not a string: {}".format(true_value))

    if not isinstance(false_value, string_types):
        raise FrklException("'false' value not a string: {}".format(false_value))
...
def object_for_boolean_filter(value, true_value, false_value):
    """
