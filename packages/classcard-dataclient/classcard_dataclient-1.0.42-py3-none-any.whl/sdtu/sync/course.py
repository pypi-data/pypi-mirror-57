...
from utils.code import b64encode, get_md5_hash
...
class CourseTableSync(BaseSync):
    def __init__(self):
        ...
        today = date2str(now)
        manager_name = "{}-{}-{}".format(self.xn, self.xq, today)
        manager_number = today
    ...
    def get_rent_classroom(self):
        ...
        for index in range(total_page):
            ...
            for row in rows:
                ...
                try:
                    course_name = classroom_name + subject_name + str(teacher_number)
                except (Exception,):
                    continue
                if course_name in course_map:
                    course = course_map[course_name]
