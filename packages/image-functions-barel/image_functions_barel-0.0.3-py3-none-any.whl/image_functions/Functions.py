...
def video_generator(path: str, start: int = 0, length: int = 0) -> Generator[np.ndarray, None, None]:
    mylib_log.info(f"Creating video generator from {path}")
    try:
        capture = cv2.VideoCapture(path)
        length = length if length else int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
...
class VideoGenerator:
    def __init__(self, path, start=0, length=0):
        ...
        try:
            ...
            # init the length
            self.length = length if length else int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
        except Exception as e:
            mylib_log.error("In video generator __init__: " + str(e))
            exit()
