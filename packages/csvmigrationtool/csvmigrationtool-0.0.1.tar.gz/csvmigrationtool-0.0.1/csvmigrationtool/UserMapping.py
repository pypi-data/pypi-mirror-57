...
class UserMapping:
    ...
    def __init__(self, USER_YAML_FILE, jira, user):
    ...
    def is_user_potentially_the_same(self, firstname, lastname, potential_user_json_response):
        ...
        return (firstname.lower() in potential_user_displayname.lower() and
                (lastname.lower() in potential_user_email.lower() or lastname.lower() in potential_user_displayname.lower()))
    ...
    def find_user_name(self, firstname, lastname):
        potentialusers = self.filter_users_by_name(
            firstname, lastname,  self.jira.user_find_by_user_string(firstname))
        ...
        if (len(potentialusers) > 1):
            try:
                ...
                try:
                    # valid selection
                    selected_username = potentialusers[int(user_selection)]['name']
                except Exception:
                    # invalid selection
                    selected_username = firstname.lower()+'.'+lastname.lower()
                self.insert_user_to_user_dict(firstname,lastname,selected_username)
                return (selected_username)
