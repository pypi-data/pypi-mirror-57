...
def qvm(request) :
    ...
    time.sleep(1) # Wait for container to start running the server, otherwise Travis will non-deterministically fail
    request.addfinalizer(container.stop)
    return None

def get_test_program(measure=False):
    PI = float(pi.evalf())
    p = Program()
    p += X(0)
    p += Y(1)
...
def adjust_for_relative_phase(state0, state1):
    max = 0
    phase0 = 1
    phase1 = 1
...
def test_convert(qvm):
    ...
    initial_phaseless, final_phaseless = adjust_for_relative_phase(initial_wf.amplitudes, final_wf.amplitudes)
...
def test_from_tket():
    ...
    c.Measure(1,1)
    p = tk_to_pyquil(c)
    assert(len(p.instructions) == 8) # 5 gates, 2 measures, and an initial declaration of classical register
...
def test_measure():
    ...
    for i in p.instructions:
        if isinstance(i,Measurement):
            m_map[i.qubit] = i.classical_reg.offset
    tkc = pyquil_to_tk(p)
    p2 = tk_to_pyquil(tkc)
    m_map2 = {}
    for i in p2.instructions:
        if isinstance(i,Measurement):
            m_map2[i.qubit] = i.classical_reg.offset
    assert(m_map == m_map2)

def test_measures_are_last():
    c = Circuit(2,2)
