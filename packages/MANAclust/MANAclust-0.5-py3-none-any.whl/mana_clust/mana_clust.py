...
class cluster_omes():
    def __init__(self, cat_omes = None, num_omes = None, out_dir=None, do_plot=False, missing_str = 'NA', processes = 1, bin_size = 5000, force = True, no_h5 = False, store_16bit = False, process_mem = None):
        if cat_omes == None and num_omes == None:
            ...
            if store_16bit:
                self.num_omes_full, self.num_omes = do_all_numeric_omes(num_omes, processes = self.processes, bin_size = bin_size, force = force, no_h5 = no_h5, dtype = np.float16, process_mem = process_mem)
            else:
...
def process_test_omes(test_cat = [], 
    ...
    for cat in test_cat:
        ...
        test_cat_omes.append(categorical_ome(get_in_mat(cat, missing_str), path=cat))
