...
def write_consensus_chi_table_results(first_ome_name, second_ome_name, cont_mat, adj_res_mat, p_mat, temp_out_dir):
    ...
    out_table = [["group_1_ID", "group_2_ID", "number_in_both","adjusted_residuals","BH_corrected_p_vals"]]
    ## rows are the first group, columns are the second group
    for i in range(len(cont_mat)):## first group
        first_con_grp_name = first_ome_name+"_consensus_group_"+str(i)
        for j in range(len(cont_mat[i])):
            second_con_grp_name = second_ome_name+"_consensus_group_"+str(j)
            ## now append all of the results!
            out_table.append([first_con_grp_name, second_con_grp_name, cont_mat[i,j],adj_res_mat[i,j],p_mat[i,j]])
...
def compare_consensus_omes(consensus_type_dict, cluster_ome_names, test_ome_names, out_dir):
    ...
    all_cross_ome_results = [["group_1_ID", "group_2_ID", "number_in_both","adjusted_residuals","BH_corrected_p_vals"]]
    if len(cluster_ome_names) > 1:
        temp_out_dir = process_dir(os.path.join(consensus_comp_dir, "between_clustering_omes"))
        between_clustering_omes_results_table = [["group_1_ID", "group_2_ID", "number_in_both","adjusted_residuals","BH_corrected_p_vals"]]
