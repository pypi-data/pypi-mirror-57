...
class Ensemble(): 
    ...
    def __init__(self, classes,
        ...
        if len(self.ensemble_networks) == 2:
            ...
            print ("==>> WARNING: your networks are more than 2, This current version ONLY SUPPORTs 2 models ensemble")
    ...
    def do_ensemble(self, model_list,
        ...
        for i in self.ensemble_networks: 
            if i == 'densenet':
                network_output[i], _ = imrec.build_densenet_base(self.input_images,
                                                                dropout_rate = 0.20,
                                                                is_training = is_training,
                                                                top_layer_depth = 128) 
                network_vars[i] = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
                network_vars[i] = [x for x in network_vars[i] if x not in network_vars['resnet']]

            elif i == 'resnet': 
                network_output[i], _ = imrec.build_resnetv2(self.input_images,
                                                            is_training=is_training,
                                                            top_layer_depth = 128) 
                network_vars[i] = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
        
        self.out = (network_output[self.ensemble_networks[0]] + network_output[self.ensemble_networks[1]]) / 2.
        print ("==>> INFO: Building networks success")
        
        session = tf.Session()
        saver_all = tf.train.Saver()
        for idx, i in enumerate(self.ensemble_networks): 
            saver[i] = tf.train.Saver(var_list=network_vars[i])
            saver[i].restore(sess=session, save_path=save_path[idx])
        ...
        builder.add_meta_graph_and_variables(session, [tf.saved_model.tag_constants.SERVING],
                                             signature_def_map={'predict_result':prediction_signature},
