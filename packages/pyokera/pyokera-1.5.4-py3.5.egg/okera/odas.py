...
def _convert_col_native(
    ...
    if schema[col_idx].type.type_id == TTypeId.STRING or \
        ...
        offset = 0
        column = [None] * num_records
        dtype, length, type_name = _ttype_to_info[schema[col_idx].type.type_id]
        for idx in range(len(is_null)):
            if is_null[idx]:
                continue
        ...
        if schema[col_idx].type.precision <= 18:
            ...
            for idx in range(0, num_records):
                if is_null[idx]:
                    continue
                v = Decimal(int(longs[value_idx * 2 + 1])) * multiple + longs[value_idx * 2]
                column[idx] = v.scaleb(scale)
                value_idx += 1
        return column, 1, any_nulls, 0, 1
