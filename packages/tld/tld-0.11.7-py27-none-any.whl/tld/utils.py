...
__license__ = u'MPL-1.1 OR GPL-2.0-only OR LGPL-2.0-or-later'
...
def process_url(url, fail_silently=False, fix_protocol=False, search_public=True, search_private=True, parser_class=MozillaTLDSourceParser):
    ...
    if (not isinstance(url, SplitResult)):
        url = url.lower()
        if (fix_protocol and (not url.startswith(u'//')) and (not (url.startswith(u'http://') or url.startswith(u'https://')))):
