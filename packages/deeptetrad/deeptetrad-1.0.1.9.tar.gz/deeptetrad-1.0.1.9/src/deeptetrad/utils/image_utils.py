...
def generate_batch_images(image, n_images_per_batch, image_single_side_size):
    # Read image
    h, w = image.shape[:2]
            
    all_images = []
    
    img_a_side_size = image_single_side_size
    img_a_half_size = img_a_side_size >> 1
    
    an_empty_image = np.zeros([img_a_side_size, img_a_side_size, 3])
    
    for x_min in range(0, h - img_a_half_size, img_a_half_size):
        x_max = min(h, x_min + img_a_side_size)
        for y_min in range(0, w - img_a_half_size, img_a_half_size):
            y_max = min(w, y_min + img_a_side_size)
            a_cropped = image[x_min:x_max, y_min:y_max]
            all_images.append(a_cropped)
    
    n_new_images = base_round_up(len(all_images), n_images_per_batch)
    while len(all_images) < n_new_images:
        all_images.append(an_empty_image)
