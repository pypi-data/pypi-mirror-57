...
class server:
    ...
    def Accept(self):
        ...
        while True:
            ...
            _thread.start_new_thread(self.RE,(onn,))
            #_thread.start_new_thread(self.Updata,(onn,))
            print("客户端已连接,ip:", onn.getsockname()[0])
    def send(self,msg):
        self.objdata['server'] = msg
    def getdata(self):
        return self.objdata.copy()
    def RE(self,coon: socket.socket):
        while True:
          if self.objb[coon]:
            try:
                ...
                coon.send(t_data)
                # 3.发送真实数据
                cmd = demjson.encode(self.objdata)
                coon.send(cmd.encode('utf-8'))
                self.objb[coon] = False
                #coon.send(err_data)
            except:
                print("(10001)",end = "")
                break
    ...
    def Updata(self, coon: socket.socket):
        while True:
            try:
                ...
                temp = demjson.decode(cmd)
                
                if temp[0][len(temp[0])-4:]!='Hugn':
                    self.objdata[temp[0]] = temp[1]
                else:
...
class client:
    ...
    def start(self):
            ...
            #self.client2.connect((self.server_ip,self.server_port))
    ...
    def receive(self):
        while True:
            if self.msg != None:
                time.sleep(0.1)
                temp = demjson.encode([self.my_name,self.msg])
    ...
    def Get(self):
                time.sleep(0.1)
                temp2 = demjson.encode([self.my_name+'Hugn',True])
                self.client.send(temp2.encode('utf-8'))
                ...
                json_dic = demjson.decode(t_data.decode("utf-8"))
                ...
                all_data = all_data.decode("gbk")
                all_data = demjson.decode(all_data)
                self.obj = all_data
