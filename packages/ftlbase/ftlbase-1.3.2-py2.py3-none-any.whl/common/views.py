...
from common.utils import has_permission, unescape
...
def get_goto_url(goto, instance):
    try:
        url = reverse(goto)
...
def configuraButtonsNoForm(form, acao, goto, workitem=None):
    """
    Configura botões padrões de save ou delete e cancel no final do form
    """
    url = get_goto_url(goto, form.instance)
...
def commonRender(request, template_name, dictionary):
    ...
    try:
        url = get_goto_url(goto, form.instance if form else None)
        dic.update({'goto': url})
        linkCancel = "window.location.href='#%s'" % url
...
@login_required
def commonCadastro(request, acao=ACAO_VIEW, formModel=None, goto='index', **kwargs):
    """
    Cadastro genérico.
        Parâmetros são:
            pk: chave principal
    ...
    if request.method == 'POST' and ok:
        ...
        if all(True if d['isTree'] else d['formset'].is_valid() for d in detail) and form.is_valid():
            ...
            return commonProcess(form=form, formset=detail, request=request, acao=acao,
                                 goto=get_goto_url(goto, form.instance),
...
def common_workflow(request, id=None, goto='workflow_pending', dictionary=None):
    ...
    def _app_response(request, workitem, goto, dictionary):
        ...
        # try: TODO: Será?
        #     dic.update({'title': workitem.instance.content_object})
        # except:
        #     dic.update({'title': activity.process.title})
...
def include_CRUD(name, table=None, form=None, add=True, edit=True, *args, **kwargs):
    urls = []
    extrajavascript = kwargs.get('extrajavascript', '')
    ...
    dic = {'formModel': form, 'goto': goto, 'extrajavascript': extrajavascript, 'configuraButtons': configuraButtons,
    ...
    if add:
        urls.extend([
            url(r'^{0}/add/$'.format(name), commonCadastro,
    ...
    if edit:
        urls.extend([
            url(r'^{0}/(?P<pk>[-\w]+)/(?P<acao>\w+)/$'.format(name), commonCadastro,
                dic,
    ...
    if table:
        urls.extend([
            url(r'^{0}/$'.format(name), common_workflow_table_process,
                {'table': table, 'subject': subject, 'pending': pending, 'add': add, 'disableMe': disableMe}, name=name)
        ])
    if add:
        urls.extend([
...
def relatorio(request, *args, **kwargs):
    ...
    if configuraButtons:
        configuraButtonsNoForm(form, ACAO_REPORT, goto=home, workitem=workitem)
...
#     #     )))
#     #
#     # for field_name in get_all_field_names(obj):
...
#         logger = logging.getLogger('reversion_compare.compare')
#         logger.setLevel(logging.DEBUG)
#
#     return changed_fields

# def versionView(request, content_type=None, pk=None):
@login_required
def versionViewCompare(request, pk=None, *args, **kwargs):
    ...
    if not has_permission(request, model=Version, acao=ACAO_VIEW):
    ...
    fkv_all_new = v_new.revision.version_set.all().exclude(object_id=v_new.object_id, content_type=v_new.content_type)  # Versions de FK
    if v_old:
        fkv_all_old = v_old.revision.version_set.all().exclude(object_id=v_old.object_id, content_type=v_old.content_type)  # Versions de FK
    else:
    ...
    for fkv_new in fkv_all_new:
        ...
        if fkv_old:
            ...
            # Render diff
            # TODO: Precisa ou trata no template?
            ...
            # TODO: É novo, então mostra que foi inserido
            pass
