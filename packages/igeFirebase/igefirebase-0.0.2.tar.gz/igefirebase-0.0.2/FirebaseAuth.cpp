...
void FirebaseAuth::init()
{
	...
	while (initializer.InitializeLastResult().status() !=
		...
		if (FirebaseProcessEvents(1000)) return;  // exit if requested
	}
...
void FirebaseAuth::testcase()
{
	...
	if (m_auth->current_user() == nullptr) {
		...
		LOG(
			...
			m_auth->current_user()->uid().c_str(),
			m_auth->current_user()->display_name().c_str());
		m_auth->SignOut();
	}
	...
	// --- StateChange tests -----------------------------------------------------
	{
		...
		m_auth->AddAuthStateListener(&counter);
		m_auth->AddIdTokenListener(&token_counter);
		...
		Future<User*> sign_in_future = m_auth->SignInAnonymously();
		...
		// Refresh the token.
		if (m_auth->current_user() != nullptr) {
			Future<std::string> token_future = m_auth->current_user()->GetToken(true);
		...
		LOG("Current user %p", m_auth->current_user());  // DEBUG
		m_auth->SignOut();
		// Wait for the sign out to complete.
		WaitForSignOut(m_auth);
		...
		m_auth->RemoveAuthStateListener(&counter);
		m_auth->RemoveIdTokenListener(&token_counter);
	}

	// --- Auth tests ------------------------------------------------------------
	{
		UserLogin user_login(m_auth);  // Generate a random name/password
		user_login.Register();
		if (!user_login.user()) {
			LOG("ERROR: Could not register new user.");
		}
		else {
			// Test Auth::SignInAnonymously().
			{
				...
				ExpectTrue("SignInAnonymouslyLastResult matches returned Future",
					sign_in_future == m_auth->SignInAnonymouslyLastResult());
			...
			// Sign in with email and password that have already been registered.
			{
				Future<User*> sign_in_future = m_auth->SignInWithEmailAndPassword(
				...
				ExpectTrue(
					...
					sign_in_future == m_auth->SignInWithEmailAndPasswordLastResult());
			...
			{
				Future<User*> sign_in_anonymously_future = m_auth->SignInAnonymously();
				WaitForSignInFuture(sign_in_anonymously_future,
					"Auth::SignInAnonymously", kAuthErrorNone, m_auth);
			...
			// Use existing email. Should succeed.
			{
				...
				Future<User*> sign_in_cred_ok =
					m_auth->SignInWithCredential(email_cred_ok);
				...
				ExpectTrue("SignInWithCredentialLastResult matches returned Future",
					sign_in_cred_ok == m_auth->SignInWithCredentialLastResult());
			}
			...
			// Use existing email. Should succeed.
			{
				...
				Future<SignInResult> sign_in_future =
					m_auth->SignInAndRetrieveDataWithCredential(email_cred);
				WaitForSignInFuture(sign_in_future,
			...
			// Use bad Facebook credentials. Should fail.
			{
				...
				Future<User*> facebook_bad =
					m_auth->SignInWithCredential(facebook_cred_bad);
				WaitForSignInFuture(
					facebook_bad,
					"Auth::SignInWithCredential() bad Facebook credentials",
					kAuthErrorInvalidProviderId, m_auth);
			}
			...
			// Use bad GitHub credentials. Should fail.
			{
				...
				Future<User*> git_hub_bad =
					m_auth->SignInWithCredential(git_hub_cred_bad);
				WaitForSignInFuture(
			...
			// Use bad Google credentials. Should fail.
			{
				...
				Future<User*> google_bad = m_auth->SignInWithCredential(google_cred_bad);
				WaitForSignInFuture(
					google_bad, "Auth::SignInWithCredential() bad Google credentials",
					kAuthErrorInvalidCredential, m_auth);
			}
			...
			// Use bad Play Games (Android-only) credentials. Should fail.
			{
				...
				Future<User*> play_games_bad =
					m_auth->SignInWithCredential(play_games_cred_bad);
				WaitForSignInFuture(
					play_games_bad,
			...
			// Test Game Center status/login
			{
				...
				if (!is_authenticated) {
					LOG("Not signed into Game Center, skipping test.");
				}
				else {
					LOG("Signed in, testing Game Center authentication.");
					...
					if (credential_error == kAuthErrorNone) {
						...
						if (gc_credential_ptr == nullptr) {
							LOG("Failed to retrieve Game Center credential.");
						}
						else {
							Future<User*> game_center_user =
								m_auth->SignInWithCredential(*gc_credential_ptr);
			...
			// Use bad Twitter credentials. Should fail.
			{
				...
				Future<User*> twitter_bad =
					m_auth->SignInWithCredential(twitter_cred_bad);
				WaitForSignInFuture(
			...
			// Use bad OAuth credentials. Should fail.
			{
				...
				Future<User*> oauth_bad = m_auth->SignInWithCredential(oauth_cred_bad);
				WaitForSignInFuture(
			...
			// Use existing email. Should succeed.
			{
				Future<void> send_password_reset_ok =
					m_auth->SendPasswordResetEmail(user_login.email());
				...
				ExpectTrue(
					...
					send_password_reset_ok == m_auth->SendPasswordResetEmailLastResult());
			}

			// Use bad email. Should fail.
			{
				Future<void> send_password_reset_bad =
					m_auth->SendPasswordResetEmail(kTestEmailBad);
	...
	// Test anonymous user info strings.
	{
		...
		if (anon_sign_in_for_user.status() == ::firebase::kFutureStatusComplete) {
			...
			if (anonymous_user != nullptr) {
				LOG("Anonymous uid is %s", anonymous_user->uid().c_str());
				...
				{
					const User* pre_link_user = m_auth->current_user();
					...
					ExpectTrue("Linking maintains user",
						m_auth->current_user() == pre_link_user);
				}

				// Test Auth::SignInWithCredential(), signing in with bad credential.
				// Call should fail, and Auth's current user should be maintained.
				{
					const User* pre_signin_user = m_auth->current_user();
					ExpectTrue("Test precondition requires active user",
					...
					ExpectTrue("Failed sign in maintains user",
						m_auth->current_user() == pre_signin_user);
				}

				UserLogin user_login(m_auth);
				user_login.Register();
				...
				if (!user_login.user()) {
					...
					if (email_user != nullptr) {
						LOG("Email uid is %s", email_user->uid().c_str());
					...
					if (email_user != nullptr) {
						...
						LOG("User::provider_data() returned %d interface%s",
							provider_data.size(),
		...
		Future<User*> create_future_for_delete =
			m_auth->CreateUserWithEmailAndPassword(new_email_for_delete.c_str(),
				kTestPassword);
		WaitForSignInFuture(
	...
	}
	{
		...
		LOG("Anonymous uid(%s)", m_auth->current_user()->uid().c_str());
	}

	LOG("Completed Auth tests.");
}
...
bool FirebaseAuth::WaitForFuture(const FutureBase& future, const char* fn,
	...
	while (future.status() == ::firebase::kFutureStatusPending) {
		if (FirebaseProcessEvents(10000)) return true;
	}
...
bool FirebaseAuth::WaitForSignOut(firebase::auth::Auth* auth) {
	while (auth->current_user() != nullptr) {
		if (FirebaseProcessEvents(1000)) return true;
	}
	// Wait - hopefully - long enough for listeners to be signalled.
	FirebaseProcessEvents(1000);
	return false;
}
