...
static PyObject* firebaseAnalytics_LogEvent(firebaseAnalytics_obj* self, PyObject* args)
{
	...
	if (parameters && PyTuple_Check(parameters))
	{
		...
		for (uint32_t i = 0; i < numAttr; i++)
		{
			...
			if (PyTuple_Check(v))
			{
				...
				if (PyBytes_Check(valueObject))
				{
					char* value = PyBytes_AsString(valueObject);
					analyticParameters.push_back(analytics::Parameter(parameter_name, value));
				}
				...
				{
					const char* value = PyUnicode_AsUTF8(valueObject);
					analyticParameters.push_back(analytics::Parameter(parameter_name, value));
				}
			}
		}
	...
	{		
		if (PyBytes_Check(parameter_value))
		{
			...
			self->firebaseAnalytics->logEvent(eventName, name, value);
		}
...
PyMethodDef firebaseAnalytics_methods[] = {
	...
	{ "logEvent", (PyCFunction)firebaseAnalytics_LogEvent, METH_VARARGS, firebaseAnalyticsTestcase_doc },
