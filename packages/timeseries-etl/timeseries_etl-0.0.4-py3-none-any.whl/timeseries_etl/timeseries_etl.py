...
import argparse
import logging
logging.basicConfig(format='%(asctime)s [%(levelname)s] : [%(module)s:%(lineno)d]-%(name)s : %(message)s', level=logging.DEBUG)
...
class TimeSeriesEtl:
    ...
    def build_df_for_item(self, tsdatadef: TSDataDef, shifts: List[int], period: int,
        ...
        :return:
        """
        self.log.info("Building dataframe for predictor {0}: shifts {1}, period {2}, from_date {3}".format(tsdatadef.item_id, shifts, period, from_date))
...
if __name__ == "__main__":
    ...
    parser.add_argument("ttable", type=str, help="the table from where to extract the data for the target items")
    parser.add_argument("measure", type=str, help="the measure that should be used to build the timeseries e.g. speed")
    parser.add_argument("target", type=int, help="the target item id that we want to predict")
    ...
    parser.add_argument("--date", type=str, help="from which date should the process start extracting data from. The format is YYYY-MM-DD")
    parser.add_argument("--cpu", type=int, help="how many CPU's to use", default=4)
    parser.add_argument("--period", type=int, help="the period to use for the timeseries e.g. 5mins, 15 mins", default=5)
    ...
    query = "SELECT VARIANCE(speed) as var, item_id FROM {0}.{1} group by item_id having VARIANCE({2}) < 5".format(args.schema, args.ptable, args.measure)
