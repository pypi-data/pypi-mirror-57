...
# region Error Index and Boundary Index
    ...
    def get_boundary_index(fited_y):
        ...
        hist_count = 100
        diff_Y_sorted = sorted(diff_Y, reverse=True)

        i = 0
        while numpy.isnan(diff_Y_sorted[i]):
            i += 1
        i += 1
        hist, bins = numpy.histogram(diff_Y, bins=hist_count, range=(0, diff_Y_sorted[i]))
        ...
        i = int(diff_count / 2)
        while i < diff_count:
            if fited_y[diff_count - i] > 0:
                if i <= diff_count - 10:
                    diff = diff_Y[diff_count - i]
                else:
                    diff = (diff_Y[diff_count - i] + diff_Y[diff_count - i - 5] + diff_Y[diff_count - i - 10]) / 3.0
            else:
                if i <= diff_count - 2:
                    diff = diff_Y[diff_count - i]
                else:
                    diff = (diff_Y[diff_count - i] + diff_Y[diff_count - i - 1] + diff_Y[diff_count - i - 2]) / 3.0

            if diff > bins[s_max[0]]:
                i += 1
                # print("diff", diff_Y[diff_count - i + 1])
                break
            else:
                i += 1
