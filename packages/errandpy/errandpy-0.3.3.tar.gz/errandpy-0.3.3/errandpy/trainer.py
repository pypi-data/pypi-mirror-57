...
class Trainer(object):
    ...
    def __should_bias_forward(self, fit_y, boundary_index, error_index, delta):
        ...
        # print("int_fit_y: ", int_fb)
        # print("var_y: ", var_fb)
        # print("forward")
        if boundary_index < error_index:
            # boundary indexが短距離力に寄りすぎ or boundary index以降で斥力成分は大きい
            if int_fb < 0 or (int_fe - int_fb > 1 - self.accurate):
                return True
        else:
            # 平になるはずの場所で斥力成分は大きい
            value = numpy.sum(fit_y[boundary_index:boundary_index + self.biasStep])
            # print("v", value)
    ...
    def __should_bias_backward(self, fit_y, boundary_index, error_index, delta):
        int_fb = numpy.sum(fit_y[boundary_index:]) * delta
        var_fb = numpy.var(fit_y[boundary_index:] * 100)
        ...
        if boundary_index > error_index + self.biasStep:
            ...
            # print("v", value)
            if 0 < value < 1 - self.accurate:
                return True
            elif int_fe - int_fb > 1 - self.accurate:
                return True
            elif var_fb < 1 - self.accurate:
                return True
    ...
    def __should_bias_warp(self, fit_y, boundary_index, error_index):
        ...
        # elif (fit_y[Utility.clamp(0, self.cacheParam.arrayCount - 1, max_index - 1)]
        #       + fit_y[Utility.clamp(0, self.cacheParam.arrayCount - 1, max_index + 1)]
        #       + fit_y[max_index]) / 3 > 1 - self.accurate:
        #     return numpy.argmax(fit_y[error_index:]) + error_index - boundary_index
        # 大体は直線 99%の正解率なら分散は1%^2 = 0.0001
