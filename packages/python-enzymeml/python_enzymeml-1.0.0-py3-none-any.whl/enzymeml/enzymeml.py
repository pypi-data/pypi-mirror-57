...
class EnzymeMLFormat:
    ...
    def to_element(self, sid):
        ...
        for i in range(0, len(self.columns)):  # FIXME if later on missing columns do not have to be described
            c = self.columns[i]
            e = c.to_element(i)
            el.append(e)
...
class EnzymeMLCSV:
    ...
    def write(self, name):
        # FIXME this is just an error fix to prevent errors from writing read files
        if "./" in name:
            sp = str(name).split("./", 2)
            name = sp[0] + "data.csv"
...
__unit_ids = dict() # previously defined below, but needed here
...
def _create_model_sbml_document(exp_doc, doc, ident):
    ...
    # Handle units
    lou = m_exp.getListOfUnitDefinitions()
    global __unit_ids
    __unit_ids[ident] = len(lou)
...
def __model_reaction_kinetic_law(enzymeml, ident, obj):
    ...
    formula = sbml.readMathMLFromString(obj)
    kl.setMath(formula)
