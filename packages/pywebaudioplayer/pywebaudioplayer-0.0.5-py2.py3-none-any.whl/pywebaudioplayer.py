...
def _figure_margins(ax):
    ...
    if ax.get_lines():
        ...
        print(ax.get_lines())
        xdata_min = ax.get_lines().get_xdata()[0]
        xdata_max = ax.get_lines().get_xdata()[-1]
...
def trackswitch(tracks, text='', seekable_image=None, seek_margin=None, mute=True, solo=True, globalsolo=True, repeat=False, radiosolo=False, onlyradiosolo=False, spacebar=False, tabview=False):
    ...
    if seekable_image:
        html_code += '''
        <img src="{}" data-style="width: {}px;" class="seekable"{}/>'''.format(seekable_image, image_width, ' data-seek-margin-left="{}" data-seek-margin-right="{}"'.format(*seek_margin) if seek_margin else '')
    for track in tracks:
        html_code += '''
        <ts-track{}{}>
...
if __name__ == '__main__':
    ...
    freq = 440
    duration = 3
    time = np.arange(duration*samplerate)
    sine = np.sin(2*np.pi*freq*time/samplerate)
    a = IPython.display.Audio(sine, rate=samplerate)
    b = IPython.display.HTML('<h1>Header</h1>')
