...
def plot_k(f, data, ks,freqs, title=''):
    ...
    norm = plt.cm.colors.Normalize(vmax=50, vmin=0)
    ...
    ax2 = plt.twinx()
    ax2.set_ylabel('[Thz]')
    for j in range(tot_branches-1):
        ax2.scatter(k,freqs[:,j], c='b', marker='x', s=100)
        
    ax2.scatter(k,freqs[:,tot_branches-1], c='b', marker='x', s=100, label='freq from dispersion')
