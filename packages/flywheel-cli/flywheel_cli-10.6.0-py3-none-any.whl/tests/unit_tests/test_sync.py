...
from flywheel_cli.sync import fw_src
from flywheel_cli.sync import os_dst
from flywheel_cli.sync import s3_dst
...
def test_print_tree():
    ...
    tree_1 = io.StringIO()
    sync.print_tree(files, 'fw://test', fh=tree_1)
    assert tree_1.getvalue() == (
    ...
    tree_2 = io.StringIO()
    sync.print_tree(files, 'fw://test', verbose=True, fh=tree_2)
    assert tree_2.getvalue() == (
        'fw://test (14 bytes / 4 files)'.ljust(80) + '\n'
        '`- f (1 byte)\n'
        '`- a\n'
        '   |- e (3 bytes)\n'
    ...
    tree_3 = Mock(wraps=io.StringIO(), encoding='UTF-8')
    sync.print_tree(files, 'fw://test', verbose=True, fh=tree_3)
    assert tree_3.getvalue() == (
        'fw://test (14 bytes / 4 files)'.ljust(80) + '\n'
        '└─ f (1 byte)\n'
        '└─ a\n'
        '   ├─ e (3 bytes)\n'
