        ...
        if project_name != 'None':
            try:
                for chart in os.listdir(project_location+'/charts'):
                    subprocess.run(['helm', 'template', project_location+'/charts/'+chart, '--namespace', namespace])
            except:
    ...
    def deploy(self):
        ...
        if project_name != 'None':
            ...
            with open(project_path + '/temp.yaml', 'w+') as resource_file:
                temp = ''
                try:
                    for chart in os.listdir(project_path+'/charts'):
                        temp += subprocess.check_output(['helm', 'template', project_path+'/charts/'+chart, '--namespace', namespace]).decode('utf-8')+'\n'
                except:
                    raise RenderError(project_name)
