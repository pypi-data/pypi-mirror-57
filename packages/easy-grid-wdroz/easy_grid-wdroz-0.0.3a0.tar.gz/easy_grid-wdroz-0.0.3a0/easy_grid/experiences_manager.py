...
class ExperienceBase(object):
    ...
    """
    def __init__(self, python_executable, script_name=None, save_filename="latest.pkl", job_filename='latest.job', output_dir='output', job_name='latest'):
        """
        ...
        print(j2_template_path)
        j2_env = Environment(loader=FileSystemLoader(j2_template_path), trim_blocks=False)
    ...
    def run(self, sync=False):
        ...
        if not self.is_in_SGE(): # in case of bad usage, avoid grid-bombing...
            ...
            commands = "\n".join(['source /idiap/home/$(whoami)/.bashrc',
    ...
    def wait_until_finish(self):
        ...
        while(True):
            ...
            commands = "\n".join(['source /idiap/home/$(whoami)/.bashrc',
                                'mkdir -p {}'.format(self._output_dir),
                                'SETSHELL grid',
                                'qstat -u \'*\' | grep $(whoami) | grep {} | wc -l'.format(self._job_name)])
    ...
    def _generate_jobfile(self):
        ...
        experience_parameter_product = list(dict_product(self._experience_dict))
        nb_params = len(experience_parameter_product)
        self._save(experience_parameter_product)
        self.add_grid_parameter('-t', '1-{}:1'.format(nb_params))
        with open(self._job_filename, 'w') as f:
            f.write(self._template.render(grid_parameters=self._grid_parameters, 
...
if __name__ == '__main__':
    ...
    if ExperienceBase.is_in_SGE():
        ...
        my_experience = ExperienceBase(python_executable='/idiap/user/wdroz/pythonenv/iceberg_gpu/bin/python3')
        ...
        my_experience.add_export_parameter('LD_LIBRARY_PATH', '/idiap/temp/wdroz/cudnn5.1/cuda/lib64:$LD_LIBRARY_PATH')
