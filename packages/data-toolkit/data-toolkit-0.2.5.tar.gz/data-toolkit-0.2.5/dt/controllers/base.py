...
class Base(Controller):
    ...
    def ec2(self):
        from ..ext.aws_ec2 import get_ec2, update_ec2_ssh, start_ec2, stop_ec2
        if self.app.pargs.action=='ls':
            filter = self.app.pargs.filter or "live"
            print(get_ec2('live', self.app.pargs.profile, filter))

        elif self.app.pargs.action=='update':
            print(update_ec2_ssh(self.app.pargs.profile))

        elif self.app.pargs.action=='start':
            start_ec2(self.app.pargs.id, self.app.pargs.profile)

        elif self.app.pargs.action=='stop':
            stop_ec2(self.app.pargs.id, self.app.pargs.profile)
