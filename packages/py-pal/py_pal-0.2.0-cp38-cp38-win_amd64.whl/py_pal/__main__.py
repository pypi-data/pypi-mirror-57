...
def main():
    ...
    if args.save:
        ...
        filename, ext = os.path.splitext(os.path.basename(args.target))

        if args.format == 'csv':
            res.to_csv(Path(args.out) / "{}.{}".format(filename, 'csv'))
        if args.format == 'html':
            res.to_html(Path(args.out) / "{}.{}".format(filename, 'html'))
        if args.format == 'excel':
            res.to_excel(Path(args.out) / "{}.{}".format(filename, 'xlsx'))
        if args.format == 'json':
            res.to_json(Path(args.out) / "{}.{}".format(filename, 'json'))
        if args.visualize:
            res = res[[isinstance(x, Complexity) for x in res[Columns.COMPLEXITY]]]
