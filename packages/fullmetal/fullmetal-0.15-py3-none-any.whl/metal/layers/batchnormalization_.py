...
class BatchNormalization(Layer):
    ...
    def forward_pass(self, x, training=True):
        ...
        if self.running_mean is None:
            self.running_mean = np.mean(self.x.data, axis=0)
            self.running_var = np.var(self.x.data, axis=0)

        if training and self.trainable:
            mean = np.mean(self.x.data, axis=0)
            var = np.var(self.x.data, axis=0)
        ...
        self.X_centered = self.x.data - mean
        ...
        if requires_grad:
            ...
            if x.requires_grad:
                depends_on.append(Dependency(self.x, self.grad_x_batchNorm))
        else:
            depends_on = []
