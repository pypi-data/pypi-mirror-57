...
def _cover_by_rect(x, y, cache):
    import numpy as np
    r = cache.get((x, y))
    if r:
        return r
    if x < y:
        n, r = _cover_by_rect(y, x, cache)
        bst = n, np.array([i[[1, 0, 3, 2]] for i in r])
    else:
        bst = x * y, None
        for i in range(x // 2, 0, -1):
            n1, r1 = _cover_by_rect(i, y, cache)
            n2, r2 = _cover_by_rect(x - i, y, cache)
            if n1 + n2 < bst[0]:
                bst = n1 + n2, np.r_[r1, r2 + [i, 0, i, 0]]
        for i in range(y // 2, 0, -1):
            n1, r1 = _cover_by_rect(x, i, cache)
            n2, r2 = _cover_by_rect(x, y - i, cache)
            if n1 + n2 < bst[0]:
                bst = n1 + n2, np.r_[r1, r2 + [0, i, 0, i]]
    cache[x, y] = bst
    return bst

def cover_by_rect(lst1, lst2, cache={}):
    """
    全組合せを上限ありの四角形でカバーする
    入力
        lst1: 文字列リスト1
    ...
    for l, t, r, b in _cover_by_rect(len(lst1), len(lst2), cache)[1]:
        yield ('|'.join(lst1[i] for i in range(l, r)),
               '|'.join(lst2[i] for i in range(t, b)))
