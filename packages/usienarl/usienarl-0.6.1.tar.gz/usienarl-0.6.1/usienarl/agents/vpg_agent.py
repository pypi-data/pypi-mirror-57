...
class VPGAgent(Agent):
    """
    Vanilla Policy Gradient agent with Dirichlet sampling exploration policies for discrete action spaces.
    It is supplied with a VPG model. By default exploration is disabled. Change the trade-off parameters to enable it.
    ...
    def act_train(self,
        ...
        if self._dirichlet_trade_off >= 1.0:
            ...
            # Get the value of the model at the current observed state
            self._current_value_estimate = self._model.get_value(session, agent_observation_current)
