...
class GetMetadata:
    ...
    def __init__(self, _profile_name=None):
        """
        Parameters
        ----------
        access_key : str
        ...
        """
        if _profile_name:
            # using tp specify dummy or bad credentials in unit tests
            self.session = boto3.Session(profile_name=_profile_name)
            print("Using profile: ", _profile_name)

        else:
            try:
                # look for alpha profile
                self.session = boto3.Session(profile_name='alpha')
        ...
        try:
            # if these don't exist, it means the credentials couldn't be found
            access_key = self.session.get_credentials().access_key
            secret_access_key = self.session.get_credentials().secret_key
        except AttributeError:
            raise AttributeError('Credentials not found, check instructions found in Olive README.md')
