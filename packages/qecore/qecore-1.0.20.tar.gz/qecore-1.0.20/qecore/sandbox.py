...
class TestSandbox:
    ...
    def get_app(self, name, a11yAppName=None, desktopFileExists=True,
        ...
        if self.logging:
            log.info(" ".join((
                ...
                f"appProcessName={appProcessName})"
            )))

        return self.get_application(name, a11y_app_name=a11yAppName,
    ...
    def get_application(self, name,
        ...
        Thsi function has old api name:
        get_app(name, a11yAppName, desktopFileExists, desktopFileName, appProcessName)
        ...
        new_application = Application(name, a11y_app_name, desktop_file_exists,
                                      desktop_file_name, app_process_name,
    ...
    def get_flatpak(self, flatpak_id):
        """
        Return flatpak to be used in test.

        @param flatpak_id : str
            Unique name of flatpak, manatory format: org.flathub.app
        """

        if self.logging:
            log.info(f"get_flatpak(self, flatpak_id={flatpak_id}")

        flatpak = Flatpak(flatpak_id=flatpak_id)
        ...
        def embed_data(mime_type, data, caption):
            for formatter in context._runner.formatters:
                if "html" in formatter.name:
                    formatter.embedding(mime_type=mime_type, data=data, caption=caption)
                    return
    ...
    # Preserving olded api as we have tests with those function calls
