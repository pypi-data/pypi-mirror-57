...
def ccPolyline_Regular(radius: float, origin: Union[np.ndarray, List[float]],
                       z: Union[np.ndarray, List[float]], x: Union[np.ndarray, List[float]], side_num=120):
...
def ccMatchSolid_Thick(closed_pts: List[Union[np.ndarray, List[float]]], match_vt: Union[np.ndarray, List[float]],
                       match_dmax: float, base_poly: vtk.vtkPolyData, thick: float,
                       base_offset: float = 0) -> TopoDS_Shape:
    """
    匹配closed_pts包围的区域到面网格模型，得到匹配实体

    :param closed_pts: 首尾相连包围区域的点列
    :param match_vt: 匹配的投射方向
    :param base_poly: 匹配的模型
    :param match_dmax: vt方向最后方点的最大投射距离
    :param thick: 匹配实体的厚度
    :param base_offset: vt方向与模型保留的距离
    :return: 与模型相匹配的实体
    """
    ...
    top_pd = vmi.pdApproach_Pd(top_pd, match_vt, base_poly, match_dmax, thick + base_offset)
    bottom_pd = vmi.pdRayCast_Pd(top_pd, match_vt, base_poly, match_dmax, base_offset)
    top_pd = vmi.pdSmooth_Laplacian(top_pd, 50)

    # t = vtk.vtkTransform()
    # t.Translate(thick * match_vt)
    # bottom_pd = vmi.pdTransform(top_pd, t)

    top_pts = vmi.pdBorder_Pts(top_pd)
    top_wire = vmi.ccWire(vmi.ccSegments(top_pts, True))
    top_face = vmi.ccSh_Pd(top_pd)

    bottom_pts = vmi.pdBorder_Pts(bottom_pd)
    bottom_wire = vmi.ccWire(vmi.ccSegments(bottom_pts, True))
    bottom_face = vmi.ccSh_Pd(bottom_pd)

    profile = vmi.ccLoft([top_wire, bottom_wire], True, True)
    sh = vmi.ccSolid(vmi.ccSew([top_face, profile, bottom_face]))
    return sh


def ccMatchSolid_Face(closed_pts: List[Union[np.ndarray, List[float]]], match_vt: Union[np.ndarray, List[float]],
                      match_dmax: float, base_poly: vtk.vtkPolyData, base_offset: float = 0) -> TopoDS_Shape:
    """
    匹配closed_pts包围的区域到面网格模型，得到匹配实体
    ...
    top_wire = vmi.ccWire(vmi.ccSegments(top_pts, True))
    top_face = vmi.ccFaceOnPlane(top_wire, z_pt[zmin], match_vt)
    top_wire = ccExplore_Once(top_face)[0]

    profile = vmi.ccLoft([top_wire, bottom_wire], True, True)
    shape = vmi.ccSolid(vmi.ccSew([top_face, profile, bottom_face]))
    return shape


def ccMatchSh_Face(face: TopoDS_Shape, match_vt: Union[np.ndarray, List[float]],
                   base: TopoDS_Shape) -> Optional[TopoDS_Shape]:
    """
    匹配face到形状，得到匹配实体
    ...
    :return: 
    """
    l = ccDiagonal(base) + np.linalg.norm(ccCenter(base) - ccCenter(face))
    sh = ccPrism(face, match_vt, l)
    sh = ccBoolean_Difference([sh, base])

    solids = ccExplore_Recursive(sh, TopAbs_SOLID)

    if len(solids):
        c = ccCenter(face)
        d_s = {np.linalg.norm(c - ccCenter(s)): s for s in solids}
        return d_s[min(d_s)]
