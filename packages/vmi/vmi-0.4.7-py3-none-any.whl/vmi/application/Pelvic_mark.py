...
# 访问数据列表
def load_by_list():
    global case_uid, case_info, case_image_data, voi_image, mark_hash, center_props, center_pts
    tree = QTreeWidget()
    tree.setColumnCount(5)
    tree.setHeaderLabels(['姓名', '年龄', '模态', '层数', '序列'])

    for x in data_db.find({"$and": [{"markData": {'$exists': False}}, {"SeriesInstanceUID": {'$exists': True}}]}).sort(
...
def output_upload():
    mark_dict = {
        ...
        '髂翼最高点': [center_pts[4], center_pts[5]],
        '坐骨结节': [center_pts[6], center_pts[7]],
        '髂后上棘': [center_pts[8], center_pts[9]],
        '泪滴': [center_pts[10], center_pts[11]]}
...
def _Overlay(image_data, axis):
    ...
    if axis == 0:
        overlay_data = np.flipud(overlay_data)
    elif axis == 1:
        overlay_data = overlay_data
    elif axis == 2:
        overlay_data = np.fliplr(overlay_data)
...
# 查看已上传数据
def check_upload():
    global case_uid, case_info, case_image_data, voi_image, mark_hash, center_pts
    tree = QTreeWidget()
    tree.setColumnCount(5)
    tree.setHeaderLabels(['姓名', '年龄', '模态', '层数', '序列'])
...
def NoButtonWheel(**kwargs):
    if kwargs['picked'] is features_box_list[2]:
        ...
        voi_volum.setOpacityScalar({threshold - 1: 0, threshold: 1})
...
if __name__ == '__main__':
    ...
    # 记录标记数据
    markData_db: pymongo.collection.Collection = client.testDB.normalPelvisCTmarkFile
    markFile_db = gridfs.GridFS(client.testDB, collection='normalPelvisCTmarkFile')
    ...
    for i in range(12):
        center_props.append(vmi.PolyActor(voi_view, color=[0, 0, 0], pickable=True))
    ...
    features_box_list = [vmi.TextBox(voi_view, pickable=True, text='上传标记数据', size=[0.12, 0.04], pos=[0.12, 0.24],
                                     anchor=[0, 0]),
                         vmi.TextBox(voi_view, pickable=True, text='下载原始数据', size=[0.12, 0.04], pos=[0, 0.24],
                                     anchor=[0, 0]),
                         vmi.TextBox(voi_view, pickable=True, text='阈值：200 HU', size=[0.12, 0.04], pos=[0, 0.18],
                                     anchor=[0, 0]),
                         vmi.TextBox(voi_view, pickable=True, text='显示体数据', size=[0.12, 0.04], pos=[0.12, 0.18],
                                     anchor=[0, 0]),
                         vmi.TextBox(voi_view, pickable=True, text='查看已上传数据', size=[0.12, 0.04], pos=[0, 0.3],
                                     anchor=[0, 0])
                         ]
    ...
    for i in range(12):
        image_props.append(vmi.PolyActor(image_view, color=[0, 0, 0], pickable=True))
