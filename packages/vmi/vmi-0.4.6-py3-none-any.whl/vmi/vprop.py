...
class PolyActor(QObject, vmi.Menu, vmi.Mouse):
    ...
    def bindData(self, other=None) -> None:
        ...
        self._Mapper.SetInputData(self._BindData._Data)
...
class ImageSlice(QObject, vmi.Menu, vmi.Mouse):
    ...
    def __init__(self, view: vmi.View = None, visible: bool = True, pickable: bool = True,
        ...
        def aboutToShow():
            self.actions['SlicePlaneValueNormal'].setText(
                '法向 (Normal)' + ' = ' + repr(self._BindSlicePlane._SlicePlane.GetNormal()))
    ...
    def __getstate__(self):
        ...
        state_set = {kw: getattr(self, kw) for kw in ['_Data', '_Pickable', '_Visible', '_ColorWindow', '_LookupTable']}
        state = {kw: state[kw] for kw in state if kw not in state_set}
    ...
    def __repr__(self):
        ...
        state['_SlicePlane'] = {'GetOrigin': self.slicePlane().GetOrigin(),
                                'GetNormal': self.slicePlane().GetNormal()}
        return pprint.pformat(state, width=400, compact=True)
    ...
    def bindSlicePlane(self, other=None) -> None:
        """关联断层平面，可以关联其它对象，other = self 关联自身对象，other = None 关联当前对象"""
        if hasattr(other, '_SlicePlane'):
            self._BindSlicePlane = other
        self._Mapper.SetSlicePlane(self.slicePlane())
        self.updateInTime()

    def bindColorWindow(self, others: List) -> None:
        """关联窗宽窗位，设置列表内的所有对象彼此关联，others = [] 取消关联"""
        self._BindColorWindow = [i for i in others if i is not self]
        for i in self._BindColorWindow:
            i._BindColorWindow = (self._BindColorWindow + [self]).copy()
    ...
    def setColorWindow(self, color_window: List[int]) -> None:
        ...
        self.setLookupTable(t)

        for i in self._BindColorWindow:
            i._ColorWindow[0] = self._ColorWindow[0]
            i._ColorWindow[1] = self._ColorWindow[1]
            i.setLookupTable(t)
