...
def unpickle_vtkImageData(data):
    if len(data) == 0:
        ...
        with tempfile.TemporaryDirectory() as p:
            ...
            instance = vtk.vtkImageData()
            instance.DeepCopy(r.GetOutput())
            instance.SetOrigin([-h.GetQOffsetX(), -h.GetQOffsetY(), h.GetQOffsetZ()])
            instance.SetSpacing([h.GetPixDim(1), h.GetPixDim(2), h.GetPixDim(3)])
    return instance
...
def setPickleNIFTI_gz(gz):
    ...
    全局量，注意序列化/反序列化时保持一致"""
    global NIFTI_suffix
    NIFTI_suffix = '.nii.gz' if gz else '.nii'
