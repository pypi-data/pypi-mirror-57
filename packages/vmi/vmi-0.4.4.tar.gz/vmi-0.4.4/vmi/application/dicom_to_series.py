        ...
        for i in range(len(filenames_list)):
            ...
            with pydicom.dcmread(filenames[0]) as ds:
                ...
                if ds['Modality'].value != 'CT':
                    print('{} 模态不符 Modality={}'.format(i, ds['Modality'].value))
                    continue
                if 'StudyDescription' in ds:
                    if 'Spine' in ds['StudyDescription'].value:
                        print('{} 检查不符 StudyDescription={}'.format(i, ds['StudyDescription'].value))
                        continue
                ...
                if one is not None:
                    if len(filenames) < one['slices']:
                        print('{} 本地跳过 slices={} < {}'.format(i, len(filenames), one['slices']))
                        continue
                ...
                upload(image, kw_value)
                print('{} 上传成功 {} {}'.format(i, len(filenames), ds['SeriesInstanceUID'].value))
