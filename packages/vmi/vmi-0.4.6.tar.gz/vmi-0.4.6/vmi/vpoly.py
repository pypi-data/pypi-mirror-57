...
def pdRayCast_Pd(pd: vtk.vtkPolyData, vt: Union[np.ndarray, List[float]], target: vtk.vtkPolyData,
                 max_dist: float, offset: float = 0) -> vtk.vtkPolyData:
    """
...
def pdApproach_Pt(pt: Union[np.ndarray, List[float]], vt: Union[np.ndarray, List[float]], target: vtk.vtkPolyData,
                  max_dist: float, safe_dist: float = 0, base_kd=None) -> np.ndarray:
    """
    点pt沿方向vt逼近到网格base的投射点

    :param pt: 输入点
    :param vt: 投射方向
    :param target: 目标网格
    :param max_dist: 最大逼近距离限制
    :param safe_dist: 逼近时保持的安全距离
    :param base_kd: 目标网格的vtkKdTreePointLocator
    :return: 逼近点
    """
    if base_kd is None:
    ...
    safe_dist = max(np.linalg.norm(vt), safe_dist)

    if safe_dist <= 0:
        raise Exception('dmin = {}'.format(safe_dist))
...
def pdApproach_Pd(pd: vtk.vtkPolyData, vt: Union[np.ndarray, List[float]], target: vtk.vtkPolyData, max_dist: float,
                  safe_dist: float = 0) -> vtk.vtkPolyData:
    """
    网格pd中包含的所有点沿方向vt逼近到网格base，保持pd单元关系不变
    ...
    for i in range(pd.GetNumberOfPoints()):
        pt = pd.GetPoint(i)
        pt = pdApproach_Pt(pt, vt, target, max_dist, safe_dist)
        out.GetPoints().SetPoint(i, pt)

    return out


def pdOBB_Pts(pts) -> Dict[str, Any]:
    """
