import boto3
import ast
from aws.region import get_elbv2
from aws.region import get_elb
...
def list_elbs():
    ...
    """
    elbList = boto3.client('elb').describe_load_balancers().get('LoadBalancerDescriptions')
    return elbList


def get_elb(name: str = None):
    """
...
def list_elbv2s():
    ...
    """
    elbList = boto3.client('elbv2').describe_load_balancers().get('LoadBalancers')
    return elbList


def get_elbv2(arn: str = None, name: str = None):
    """
...
def list_target_groups():
    ...
    """
    targetGroupList = boto3.client('elbv2').describe_target_groups().get('TargetGroups')
...
def get_target_group(name: str = None, arn: str = None):
    ...
    :param arn: The ARN of the Target Group
...
def add_target(vpc_name: str = None, port: int = 443, target_name: str = None, target_arn: str = None, group_name: str = None, group_arn: str = None):
  if vpc_name is None:
    print('must supply vpc name')
    return False
  ...
  if group_name is None and group_arn is None:
    ...
    return False
  print(f"setting vpc to {vpc_name}")
  set_current_vpcid(vpc = get_vpc(name = vpc_name))
  ...
  boto3.client('elbv2').register_targets(TargetGroupArn = group['TargetGroupArn'], Targets = [t_param])

  return True

def get_tags(arn: str) -> list:
    """
    ...
    tagList = boto3.client('elbv2').describe_tags(ResourceArns=[arn]).get('TagDescriptions')[0].get('Tags')
    res = {}
    for x in tagList:
        key = x.get('Key')
        val = x.get('Value')
