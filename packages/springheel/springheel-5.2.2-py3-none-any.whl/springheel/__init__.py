...
def copyButtons(site,old_buttons_path,socialbuttons_path):
    ...
    logMsg(logmesg,".")
    social_links = socialbuttons.getButtons(site,"RSS")[0]
...
def build():
    ...
    site.jsonpoints = {"generated_on":starttime,**config.__dict__,"archive_page":"".join([site.config.base_url,"archive.html"]),"categories":[]}
    ...
    for i in comics_base:
        ...
        dictified_strip["url"] = "".join([site.config.base_url,html_filename])
        ...
        statline = stat_line
        if site.config.social_icons == "True":
            icons = " ".join(["",socialbuttons.getButtons(site,translated_strings["rss_s"])[1]])
        else:
            icons = ""
