...
def from_file(f):
    ...
    Returns:
    An iterator of tuples, each representing a single MARC record.
    """
    with open(f, "rb") as file:
        ...
        while 1:
            try:
                ...
                yield marc_tuple(rec, reclen, offset)
                offset += reclen
...
def marc_tuple(d, reclen=None, offset=None):
    ...
    arg2 length of record (optional)

    arg3 offset in file (optional)

    Returns:

    A tuple representing the MARC record.

    The returned value will be a a tuple with three elements: the
    leader data, the fields, and the byte offset of the record in the
    file (if applicable). The leader contains fifteen values from the
    standar MARC leader, minus the first field, record length. The
    values tuple contains one tuple for each field.
    ...
    ldr = __leader(d[0:LDR_LEN].decode(), reclen)
    vf = __variable_fields(
        __directory(d[LDR_LEN:ldr[8]-5].decode()), d[ldr[8]-5:])
    return (ldr, vf, offset)
...
def __leader(d, reclen):
    """ Unpack the leader into its separate values. """
    return (reclen, d[0], d[1], d[2], d[3], d[4], int(d[5]), int(d[6]),
