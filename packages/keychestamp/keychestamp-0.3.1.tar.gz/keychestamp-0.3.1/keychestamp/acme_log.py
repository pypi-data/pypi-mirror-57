    ...
    def repeater(req_hdrs, code, path, req_body, resp_body, resp_hdrs):
        ...
        try:
            ...
            if '/directory' in path:
                logging.usage("directory",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent)
                pass
                ...
                if ('contact' not in response) or ('initialIp' not in response) or ('status' not in response):
                    ...
                    logging.usage("new-acct", path=request_path, epoch=int(time.time()), client=client, server=server, code=code,
                                  agent=agent, contact=response['contact'], email=email, account=account, source=response['initialIp'],
                                  status=response['status'])
                    pass
            elif 'acme/new-nonce' in path:
                logging.usage("new-nonce",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent)
                pass
                ...
                if (body is None) or ('kid' not in body):
                    ...
                    logging.usage("new-order",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code,
                                  agent=agent, domains=identifiers, authz=authz, finalize=finalize, account=kid, order=loc)
                pass
                ...
                if (body is None) or ('kid' not in body):
                    ...
                    kid = urlparse(body['kid']).path
                    logging.usage("revoke-cert",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code,
                                  agent=agent, account=kid)
            elif 'acme/key-change' in path:
                if (body is None) or ('kid' not in body):
                    logging.error("Unrecognized body of key-change", msg=req_body)
                else:
                    kid = urlparse(body['kid']).path
                    logging.usage("key-change",  path=request_path, epoch=int(time.time()), client=client, server=server,
                                  code=code, agent=agent, account=kid)
                pass
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                    ...
                    logging.usage("auth",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                    ...
                    logging.usage("final",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                    ...
                    loc = urlparse(body['url']).path
                    status = response['status']
                    logging.usage("chall",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                    ...
                    logging.usage("order",  path=request_path, epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
