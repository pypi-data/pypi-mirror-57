...
class QueueHandler(logging.Handler):
    ...
    def __init__(self, api_logging=True, file_logging=None, fname=None):
        ...
        if api_logging:
            self.usage_url = "https://keychest.net/api/v1.0/amp/event"
        else:
            self.usage_url = None
    ...
    def send_usage(self, stop_event):
        ...
        while not stop_event.is_set():  # type: multiprocessing.Event
            try:
                record = self.queue_usage.get(timeout=0.5)
                if record is None:
                    continue
        ...
        sys.stderr.write("queue handler usage terminated\n")
    ...
    def receive(self, stop_event):
        ...
        while not stop_event.is_set():  # type: multiprocessing.Event
            try:
                ...
                traceback.print_exc(file=sys.stderr)
                sys.stderr.write("Exception in queue_handler %s" % str(ex))
        sys.stderr.write("queue handler logs terminated\n")
    ...
    @staticmethod
    def create_json_msg(message, usage_log, **kws):
        # noinspection PyUnresolvedReferences
        _temp = {
            'msg': message,
            'params': {},
            'id': logging.use_counter,
            'proxy': logging.proxy_id,
        ...
        if kws and (len(kws) > 0):
            ...
            if usage_log and (code_not or path_not or server_not):
                logging.error("Missing mandatory item in log message", code=code_not, path=path_not, server=server_not)
                # sys.stderr.write("Missing mandatory item in log message %s\n" % json.dumps(kws))
