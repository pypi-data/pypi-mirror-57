    ...
    def repeater(req_hdrs, code, path, req_body, resp_body, resp_hdrs):
        ...
        try:
            ...
            if '/directory' in path:
                logging.usage("directory", epoch=int(time.time()), client=client, server=server, code=code)
                pass
                ...
                logging.usage("new-nonce", epoch=int(time.time()), client=client, server=server, code=code)
                pass
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                ...
                elif (response is None) or ('identifier' not in response) or ('status' not in response):
                    ...
                    logging.usage("chall", epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
                                  account=kid, chall=loc, status=status, identifier=identifiers)
                pass
                ...
                if (body is None) or ('kid' not in body) or ('url' not in body):
                    ...
                    logging.usage("order", epoch=int(time.time()), client=client, server=server, code=code, agent=agent,
                                  account=kid, chall=loc, status=status, identifier=identifiers, finalize=final)
            else:
