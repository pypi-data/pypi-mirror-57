...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    cakey = AmpConfig.join_with_script_dir('ca.key')
    cacert = AmpConfig.join_with_script_dir('ca.crt')
    certkey = AmpConfig.join_with_script_dir('cert.key')
    certdir = AmpConfig.join_with_script_dir('certs/')
    timeout = 5
    lock = threading.Lock()
    ...
    def connect_intercept(self):
        ...
        with self.lock:
            if not os.path.isfile(certpath):
                ...
                p2 = Popen(["openssl", "x509", "-req", "-days", "365", "-CA", self.cacert, "-CAkey", self.cakey,
...
if __name__ == '__main__':
    logging.debug("keychestamp started")
    test()
