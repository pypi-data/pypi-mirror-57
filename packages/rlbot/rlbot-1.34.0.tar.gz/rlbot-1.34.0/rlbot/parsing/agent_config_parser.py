...
def parse_bot_loadout(player_configuration, bot_config, loadout_header):
    ...
    primary_color_lookup_string = bot_config.get(loadout_header, 'primary_color_lookup')
    if primary_color_lookup_string:
        player_configuration.primary_color_lookup = parse_color_string(primary_color_lookup_string)

    secondary_color_lookup_string = bot_config.get(loadout_header, 'secondary_color_lookup')
    if secondary_color_lookup_string:
        player_configuration.secondary_color_lookup = parse_color_string(secondary_color_lookup_string)


def parse_color_string(color_string: str) -> Color:
    color_array = json.loads(color_string)
    return Color(
        red=color_array[0],
        green=color_array[1],
        blue=color_array[2],
