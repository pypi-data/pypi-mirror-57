...
class Scheduler(object):
    ...
    def next_request(self):
        ...
        if request and self.stats:
            ...
            self.LastGetRequest = 0
        if request is None:
            self.LastGetRequest += 1
            print('空闲次数 - ', self.LastGetRequest)
            if self.LastGetRequest > 3:
                print('关闭爬虫 - ', self.spider.name)
                self.spider.crawler.engine.close_spider(self.spider, 'request queue is empty with self.LastGetRequest try')
        return request
