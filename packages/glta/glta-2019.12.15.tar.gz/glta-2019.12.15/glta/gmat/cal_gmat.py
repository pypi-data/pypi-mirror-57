...
def agmat(bed_file, inv=True, small_val=0.001, out_fmt='mat'):
    """
    加性基因组关系矩阵及其逆
    :param bed_file: plink二进制文件
    :param inv: 布尔值，默认为True，是否求逆
    :param small_val: 对角线元素加很小的值，使矩阵可逆
    :param out_fmt: 输出的格式，mat-矩阵形式（默认）、row_col_val-行、列、值形式、id_id_val-个体号、个体号、值形式
    :return: 返回值矩阵形式，输出相应形式到文件
    """
