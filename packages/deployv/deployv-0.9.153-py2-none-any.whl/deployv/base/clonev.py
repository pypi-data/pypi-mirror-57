...
class CloneV(object):
    ...
    def clone_repositories(self):
        ...
        try:
            ...
            self.config.key_ssh_path = deploy_key(
...
def deploy_key(ssh_key, folder_path):
    ...
    """
    if not utils.is_base64_encode(ssh_key):
        _logger.warning("Can not use the specified ssh key. It is not encoded in base64")
        return False
    _logger.info('Deploying keys')
    ssh_key = utils.decode(b64decode(ssh_key))
    filename = path.join(folder_path, 'id_rsa')
