...
def get_bool_from_text(value):
    ...
    if value in ["0", "no", "n", "false"]:
        return False
    raise EException("Unrecognized boolean value '%s'" % value)
        ...
        for line in text.splitlines():
            ...
            if line[0] == '[':
                ...
                if m:
                    ...
                    raise EException("ConfigParser: Bad syntax '%s'" % line)
            if field:
                if (self._allowed_fields and field not in self._allowed_fields and
                        raise_unexpected_field):
                    raise EException("ConfigParser: Unrecognized field '%s'" % field)
                else:
                    current_lines = []
                    self._sections[field] = current_lines
            else:
                if current_lines is None:
                    raise Exception("ConfigParser: Unexpected line '%s'" % line)
                if parse_lines:
                    line = line.split(' #', 1)[0]
                    line = line.split('    #', 1)[0]
                    line = line.strip()
    ...
    def __getattr__(self, name):
        if name in self._sections:
            ...
            if self._allowed_fields and name in self._allowed_fields:
                ...
                raise EException("ConfigParser: Unrecognized field '%s'" % name)
