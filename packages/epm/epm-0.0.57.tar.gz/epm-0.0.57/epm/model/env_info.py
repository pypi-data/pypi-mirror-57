...
from epm.utils.log import logger
    ...
    def loads(text):
        ...
        for env_def in text.splitlines():
            try:
                if env_def:
                    if "=" not in env_def:
                        raise EException("Invalid env line '%s'" % env_def)
                    tmp = env_def.split("=", 1)
                    name = tmp[0]
                    value = unquote(tmp[1])
                    package = None
                ...
                raise EException("Error parsing the env values: %s" % str(exc))
    ...
    def update(self, env_obj):
        ...
        if env_obj:
            ...
            # DepsEnvInfo. the OLD values are always kept, never overwrite,
                ...
                raise EException("unknown env type: %s" % env_obj)
    ...
    def loads(text):
        ...
        for line in text.splitlines():
            if not lib_name and not line.startswith("[ENV_"):
                raise EException("Error, invalid file format reading env info variables")
