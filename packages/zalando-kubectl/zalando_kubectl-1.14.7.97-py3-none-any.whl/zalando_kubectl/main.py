...
from .templating import read_senza_variables, prepare_variables, copy_template
...
@click.option("-i", "--incident", help="Incident number", required=True, type=str)
@click.option("-u", "--user", help="User that should be given access, defaults to the current user", required=False)
@click.argument("reason", nargs=-1, required=True)
@click.pass_context
...
def check_latest_zkubectl_version():
    ...
    if latest_release_version != zalando_kubectl.APP_VERSION:
        clickclick.warning(
            "You not running the latest version of zkubectl (current version: {}, latest version: {})."
