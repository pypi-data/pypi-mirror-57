...
class DingtalkPlugin(notify.NotificationPlugin):
    author = 'zhaodazao'
    author_url = 'https://code.aliyun.com/PlayGoods-Materiel/wanwu-senstry-dingtalk.git'
    version = sentry.VERSION
    description = "Integrates Testdingtalk robot."
    resource_links = [
        ('Bug Tracker', 'https://code.aliyun.com/PlayGoods-Materiel/wanwu-senstry-dingtalk.git'),
        ('Source', 'https://code.aliyun.com/PlayGoods-Materiel/wanwu-senstry-dingtalk.git'),
    ]

    slug = 'testdingtalk'
    title = 'testdingtalk'
    conf_title = title
    conf_key = 'testdingtalk'

    project_conf_form = DingtalkForm
    timeout = getattr(settings, 'SENTRY_DINGTALK_TIMEOUT', 3) 
    logger = logging.getLogger('sentry.plugins.testdingtalk')
    ...
    def get_config(self, project, **kwargs):
        return [{
            'name': 'urls',
            'label': 'testdingtalk robot url',
            'type': 'textarea',
            'help': 'Enter testdingtalk robot url.',
