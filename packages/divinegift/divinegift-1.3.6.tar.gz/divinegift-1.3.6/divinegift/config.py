from divinegift.logger import log_info, log_err, log_warning, log_debug
...
class Settings:
    ...
    def decrypt_passwords(self):
        log_debug('Try to decrypt all passwords')
        for key in self.settings.keys():
            try:
                for subkey in self.settings.get(key).keys():
                    try:
                        self.decrypt_password(key, subkey)
                        log_info(f'Password for {key}[{subkey}] decrypted')
    ...
    def parse_settings(self, file_: str = './settings.ini', encoding: str = 'utf-8',
        ...
        if log_changes and json_data:
            dict_c = dmain.dict_compare(self.settings_encrypted, json_data)
            ...
            if len(removed) > 0:
                for r in list(removed):
                    log_warning(f'Removed {r}: {self.settings_encrypted.get(r)}')
            if len(modified) > 0:
                for r in list(modified):
                    log_warning(f'Modified {r}: {modified.get(r)[0]} -> {modified.get(r)[1]}')
        elif not json_data:
            try:
    ...
    def encrypt_password(self, conn_name: str, pass_field: str ='db_pass', ck_fname: str = 'key.ck'):
        ...
        self.settings[conn_name][pass_field] = encrypt_str(self.settings[conn_name][pass_field], cipher)

    def decrypt_password(self, conn_name: str, pass_field: str ='db_pass', ck_fname: str = 'key.ck'):
        if not self.cipher_key:
            self.cipher_key = read_key(ck_fname)
        cipher = get_cipher(self.cipher_key)
        self.settings[conn_name][pass_field] = decrypt_str(self.settings[conn_name][pass_field], cipher)

    def encode_password(self, conn_name: str, pass_field: str ='db_pass'):
        settings = self.get_settings().copy()
        settings[conn_name][pass_field] = encode_password(settings[conn_name][pass_field])
        self.set_settings(settings)

    def decode_password(self, conn_name: str, pass_field: str ='db_pass'):
        settings = self.get_settings().copy()
        settings[conn_name][pass_field] = decode_password(settings[conn_name][pass_field])
        self.set_settings(settings)
