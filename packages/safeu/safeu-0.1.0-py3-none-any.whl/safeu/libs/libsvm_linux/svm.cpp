...
public:
	...
	void Solve_b(int l, const QMatrix& Q, const double *p_, const schar *y_,
		   double *alpha_, double Cp, double Cn, double *upbound, double eps,
		   SolutionInfo* si, int shrinking);
protected:
	int active_size;
	schar *y;
		...
		for(i=0;i<nr_class;i++)
			for(int j=i+1;j<nr_class;j++)
			{
				...
				for(k=0;k<ci;k++)
				{
					sub_prob.x[k] = x[si+k];
					sub_prob.y[k] = +1;
					sub_prob.upbound[k] = upbound[si+k];
				...
				for(k=0;k<cj;k++)
				{
					...
					sub_prob.y[ci+k] = -1;
					sub_prob.upbound[ci+k] = upbound[sj+k];
