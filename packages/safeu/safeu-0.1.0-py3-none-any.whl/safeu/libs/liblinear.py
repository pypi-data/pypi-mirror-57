...
try:
	...
	if sys.platform == 'win32':
		liblinear = CDLL(path.join(dirname, r'liblinear_windows\liblinear.dll'))
	else:
...
class problem(Structure):
	...
	_types = [c_int, c_int, POINTER(c_double), POINTER(
		POINTER(feature_node)), c_double, POINTER(c_double)]
	_fields_ = genFields(_names, _types)
...
class parameter(Structure):
	_names = ["solver_type", "eps", "C", "nr_weight",
		"weight_label", "weight", "p", "init_sol"]
	_types = [c_int, c_double, c_double, c_int, POINTER(
		c_int), POINTER(c_double), c_double, POINTER(c_double)]
	_fields_ = genFields(_names, _types)
	...
	def parse_options(self, options):
		...
		if self.flag_find_C:
			...
			if not self.flag_solver_specified:
				...
				raise ValueError(
					"Warm-start parameter search only available for -s 0 and -s 2")
...
class model(Structure):
	...
	def get_decfun(self, label_idx=0):
		w = [liblinear.get_decfun_coef(self, feat_idx, label_idx)
		                               for feat_idx in range(1, self.nr_feature + 1)]
...
fillprototype(liblinear.find_parameter_C, None, [POINTER(problem), POINTER(
	parameter), c_int, c_double, c_double, POINTER(c_double), POINTER(c_double)])
fillprototype(liblinear.cross_validation, None, [POINTER(
	problem), POINTER(parameter), c_int, POINTER(c_double)])
...
fillprototype(liblinear.set_print_string_function,
