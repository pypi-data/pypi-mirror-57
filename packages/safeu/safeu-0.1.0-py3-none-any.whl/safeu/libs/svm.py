...
try:
	...
	if sys.platform == 'win32':
		libsvm = CDLL(path.join(dirname, r'libsvm_windows\libsvm.dll'))
	else:
...
def gen_svm_nodearray(xi, feature_max=None, isKernel=False):
	...
	if scipy and isinstance(xi, tuple) and len(xi) == 2\
		...
		if not isKernel:
			...
			# index starts from 0 for precomputed kernel
...
def csr_to_problem(x, prob, isKernel):
	...
	if jit_enabled:
		csr_to_problem_jit(x.shape[0], x.data, x.indices,
		                   x.indptr, prob_val, prob_ind, prob.rowptr, indx_start)
	else:
		csr_to_problem_nojit(x.shape[0], x.data, x.indices,
		                     x.indptr, prob_val, prob_ind, prob.rowptr, indx_start)


class svm_problem(Structure):
	_names = ["l", "y", "x", "upbound"]
	_types = [c_int, POINTER(c_double), POINTER(
