...
The metric method is called by performance_metric(ground-truth, prediction, 
param_dict) All metric method would pop the parameters in the param_dict first. 
"""
...
def accuracy_score(y_true, y_pred, param_dict=None):
    ...
    if param_dict is None:
        param_dict = {'sample_weight': None}
    
    sample_weight = param_dict.get('sample_weight', None)
...
def zero_one_loss(y_true, y_pred, param_dict=None):
    ...
    """
    if param_dict is None:
        param_dict = {'normalize': True, 'sample_weight': None}

    normalize = param_dict.get('normalize', True)
    sample_weight = param_dict.pop('sample_weight', None)

    score = accuracy_score(y_true, y_pred, param_dict)
...
def F1_score(y_true, y_pred, param_dict=None):
    """
    ...
    """
    if param_dict is None:
        param_dict = {'pos_label': 1, 'sample_weight': None}
    pos_label = param_dict.get('pos_label', None)

    return f1_score(y_true=y_true, y_pred=y_pred, pos_label=pos_label)
    # p, r, t = _precision_recall_curve(y_true, y_pred, param_dict)
    # return 2 * (p * r) / (p + r)


def get_fps_tps_thresholds(y_true, y_score, param_dict=None):
    '''
    ...
    if (pos_label is None 
            and not (np.array_equal(classes, [0, 1])
    ...
    desc_score_indices = np.argsort(y_score.reshape(
        1, y_score.shape[0]), kind="mergesort")[::-1]
    thresholds = np.array(y_score)[desc_score_indices]
    y_true = np.array(y_true)[desc_score_indices]
    y_score = np.array(y_score)[desc_score_indices].reshape(1, y_score.shape[0])

    y_true = y_true[0].reshape(1, y_true[0].shape[0])
    tps = []
    fps = []
    thresholds = thresholds.reshape(1, thresholds.shape[1])

    y_true = [1 if i == [True] else 0 for i in y_true[0]]
    for threshold in thresholds[0]:
        y_prob = [1 if i >= threshold else 0 for i in y_score[0]]
...
def roc_curve(y_true, y_score, param_dict=None):
    ...
    y_true : array, shape = [n_samples]
        ...
        pos_label should be explicitly given in param_dict.
    ...
    fps, tps, thresholds = get_fps_tps_thresholds(y_true, y_score, param_dict)
    ...
    if tps[1] <= 0:
        raise ValueError("No positive samples in y_true,true "
...
def roc_auc_score(y_true, y_score, param_dict=None):
    ...
    y_true : array, shape = [n_samples] or [n_samples, n_classes]
        True binary _labels or binary label indicators. True binary labels. 
    ...
    """
    if param_dict is None:
        param_dict = {'pos_label': None, 'sample_weight': None}

    fpr, tpr, _ = roc_curve(y_true, y_score, param_dict)
    return _auc(fpr, tpr)    


def hamming_loss(y_true, y_pred, param_dict=None):
...
def one_error(y_true, y_pred, param_dict=None):
    '''
...
def coverage_error(y_true, y_score, param_dict=None):
...
def label_ranking_loss(y_true, y_score, param_dict=None):
...
def label_ranking_average_precision_score(y_true, y_score, param_dict=None):
...
def micro_auc_score(y_true, y_score, param_dict=None):
    """
...
def Average_precision_score(y_true, y_score, param_dict=None):
    """
...
def minus_mean_square_error(y_true, y_pred, param_dict=None):
