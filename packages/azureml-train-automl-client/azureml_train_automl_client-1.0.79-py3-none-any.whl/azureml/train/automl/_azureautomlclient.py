...
class AzureAutoMLClient:
    ...
    def _create_parent_run_for_remote(self, run_config_object, X=None, y=None, sample_weight=None,
        ...
        self._update_current_run_properties()
    ...
    def _update_current_run_properties(self):
        """Update properties on the current run object."""
        # TODO: Remove with task 416022
        display_task_type_property = {'display_task_type': self._get_display_task()}
        ...
        self.logger.info(
            "[RunId:{}]SDK dependencies versions:{}.".format(
                self.current_run.id, user_sdk_dependencies_property['dependencies_versions']))

        # Make sure to batch all relevant properties in this single call in-order to avoid multiple network trips to RH
        properties_to_update = {**display_task_type_property, **user_sdk_dependencies_property}
