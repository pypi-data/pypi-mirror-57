...
class AmazonDaxClient(object):
    ...
    def _retryable_request(self, client, scale, op_name, **kwargs):
        ...
        retryer = RetryHandler(self._client_config, scale, self._cluster)
        while True:
            response = None
            try:
...
class RetryHandler(object):
    ...
    def __init__(self, config, scale, cluster):
    ...
    def _wait_for_cluster(self, exc):
        ...
        if exc.code == DaxErrorCode.NoRoute:
            self._cluster.wait_for_routes(min_healthy=1, leader_min=1, timeout=self._retry_timeout)
