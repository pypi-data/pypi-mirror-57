...
def Count(term, *others, value=None, values=None, condition=None):
    ...
    if value is None and values is None:
        value = 1
    assert value is not None or values is not None, str(value) + " " + str(values)
...
def NValues(term, *others, condition=None, excepting=None):
    terms = flatten(term, others)
    checkType(terms, [Variable])
    if excepting is not None:
        excepting = flatten(excepting)
    checkType(excepting, ([int], type(None)))
    return _wrapping_by_complete_or_partial_constraint(ConstraintNValues(terms, Condition.build_condition(condition), excepting))
...
def Cardinality(term, *others, occurrences, closed=None):
    terms = flatten(term, others)
    checkType(terms, ([Variable]))
    ...
    for i, occ in enumerate(occurs):
        ...
        if isinstance(occ, list):
            ...
            occurs[i] = flat
    # if len(occurs) == 1 and len(values) > 1:
    #     occurs = [occurs[0]] * len(values)
