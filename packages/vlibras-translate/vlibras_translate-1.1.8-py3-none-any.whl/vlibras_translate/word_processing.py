...
class WordProcessing(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        with open(path.join(directory, 'lugares.csv'), 'r', encoding='utf-8') as f:
            for l in f:
                self.whitelist_words.append(l.split())

        with open(path.join(directory, 'famosos.csv'), 'r', encoding='utf-8') as f:
            for l in f:
                self.whitelist_words.append(l.split())
    ...
    def mask_tag_sentence(self, tag_sentence, name_symbol='"', number_symbol="'", error_symbol='#', input_lang=''):
        ...
        for tag in tag_sentence:
            ...
            elif input_lang == 'gi' and re.match(r'\w+_(cidade|estado|país|região|famoso)', tag[0]):
                continue
