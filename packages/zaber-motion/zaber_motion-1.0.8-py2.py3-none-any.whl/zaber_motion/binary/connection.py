...
class Connection:
    ...
    def __setup_events(self) -> None:
        ...
        self._unknown_response = events.take_until(self.disconnected)\
            .filter(self.__filter_event('binary/interface/unknown_response'))\
            .map(lambda ev: UnknownResponseEvent.from_protobuf(ev[1]))\

        self._alert = events.take_until(self.disconnected)\
            .filter(self.__filter_event('binary/interface/reply_only'))\
            .map(lambda ev: ReplyOnlyEvent.from_protobuf(ev[1]))\

        events.filter(self.__filter_event('interface/disconnected'))\
            .take(1)\
            .map(lambda ev: convert_exception(ev[1].error_type, ev[1].error_message))\
