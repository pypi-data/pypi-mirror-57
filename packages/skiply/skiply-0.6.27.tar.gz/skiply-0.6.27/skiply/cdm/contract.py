...
def get_contract_with_service_code(service_code):
    session = db_session()
    try:
        results = session.query(Contract).filter(Contract.services.has(AssociationContractService.service_id._in(Service.get_service_from_code(service_code).id))).first()
