...
class DisplayFactory:
    ...
    def run(self):
        ...
        subflow_flg = self.params['subflow_flg']
        if subflow_flg:
            subflow_interval = self.params['subflow_interval']
            subflow = f'subflow_{subflow_flg}_interval_{subflow_interval}'
        else:
            subflow = f'flow'
        ...
        for i, (sub_ds_name, sd_inst) in enumerate(self.dataset_dict.items()):  # {sub_name: SingleDataset, ... }
            ...
            fig_name = os.path.join(self.params['opt_dir'],
                                    os.path.dirname(sd_inst.dataset_dict['iat_dict']['feat_file']), subflow,
        ...
        if subflow_flg:
            ...
            subflow = f'flow'
        output_file = os.path.join(self.params['opt_dir'], self.params['ipt_dir'], subflow, self.params['expt_name'],
                                   f'{detector_name}-gs_{gs_flg}-results.txt')
    ...
    def _plot_roc_auc(self, sd_inst, output_file='output_data/figures/roc_of_different_algorithms.pdf',
        ...
        for idx, (key, result_dict) in enumerate(dataset_dict.items()):
            ...
            if feature_set_key == 'samp_set':
                ...
                q_sampling_rate = f'{q_sampling_rate:.1f}'
