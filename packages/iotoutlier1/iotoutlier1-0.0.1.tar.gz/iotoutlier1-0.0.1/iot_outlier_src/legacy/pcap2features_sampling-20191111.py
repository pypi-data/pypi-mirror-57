...
def parse_dataset(pcap_file, labels_csv, num_pkt_thresh=2, feat_set='', fft_bin=10, verbose=True,
    ...
    if feat_set == 'IAT':  # get IATs
        ...
        if sampling == None:  # default sampling rate
            ...
            quant = 0.3  # for sampling rate
            sampling = np.quantile(sampling_arr,
                                   q=quant)  # make 0.9 flow_duration /fft bin less than the sampling_rate.
...
def main():
    ...
    for (pcap_dir, label_dir) in dir_tuple:  # for different days' data
        ...
        for srcIP in srcIP_lst:  # for different IPs
            ...
            print(f'\nobtain Baseline set 2')
            samp_base_set = 'SampBaseline'  # label sampled features
            samp_base_fids, samp_base_features, samp_base_labels, sampling = parse_dataset(pcap_file,
