...
from _config import *
from data_process import transform_iat_to_fixed_out, merge_files_to_one, transform_samp_baseline_to_fixed_out
...
def conduct_comparison_experiment(detector_name='',
    ...
    fig_name += f'-feat_set:{prop_set}-gridsearch:{gridsearch_flg}-{detector_name}'
    if prop_feat == 'FFT':
        fig_name += f'fft_part:{fft_part}'

    # 3 save all result in pdf
    if len(fig_name) > 200:  # avoid fig_name too long
        fig_name = fig_name[:200]
...
def main(experiment='ind', detector_name='GMM', prop_feat='IAT', quant=0.9,
    ...
    if experiment == 'ind':
        ...
        pparam_dict = {  # IAT or FFT. (key, value): key-feature set, value-file
            ...
            prop_feat: f'{input_dir}' + '/srcIP_{srcIP}/Friday-WorkingHours/srcIP_{srcIP}.pcap-IAT.dat'
        }

        # prop_set params: might include multi-files
        bparam_dict = {  # (key, value): key-feature set, value-file
            # Baseline 1 features file
            'StatBaseline': f'{input_dir}' + '/srcIP_{srcIP}/Friday-WorkingHours/srcIP_{srcIP}.pcap-{feat_set}-' \
                                             'dim_10.dat',
        }
        ...
        for idx, srcIP in enumerate(srcIP_lst):
            ...
            bparam_dict[samp_feat_set] = f'{input_dir}' + \
                                         '/srcIP_{srcIP}/Friday-WorkingHours/srcIP_{srcIP}.pcap-' + '{feat_set}.dat'
...
if __name__ == '__main__':
    ...
    for gs_i, gridsearch_flg in enumerate([True, False]):  #
