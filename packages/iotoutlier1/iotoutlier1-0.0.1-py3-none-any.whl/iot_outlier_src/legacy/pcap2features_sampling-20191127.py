...
def random_select_flows(flows, labels, experiment='ind', random_state=42):
    if experiment in ['ind', 'mix', 'indv_data', 'comb_data']:
        return flows, labels
    ...
    print(f'len(flows): {len(flows)}, len(lables): {len(labels)}, get_all_flows_flg: {get_all_flows_flg}, '
          f'cnt_normal: {part_normal_thres}, cnt_anomaly: {part_anomaly_thres}')

    return flows, labels


def parse_dataset(pcap_file, labels_csv, experiment='ind', num_pkt_thresh=2, feat_set='', fft_bin=10, verbose=True,
                  sampling_type=None, sampling=None, q_sampling_rate=0.9):
...
def parse_dataset_backup(pcap_file, labels_csv, num_pkt_thresh=2, feat_set='', fft_bin=10, verbose=True,
                         sampling_type=None, sampling=None, q_sampling_rate=0.9):
    '''Parses pcap file into flow features (such as, IAT, pkts_rate and max_pkt) and labels flow into string labels
...
def parse_dataset_smart_tv(pcap_file_lst, labels_csv=None, num_pkt_thresh=2, feat_set='', fft_bin=10, verbose=True,
                           sampling_type=None, sampling=None, q_sampling_rate=0.9):
    '''Parses pcap file into flow features (such as, IAT, pkts_rate and max_pkt) and labels flow into string labels
    ...
    '''
    if verbose:
        funcparams_dict = {'pcap_file': pcap_file_lst, 'labels_csv': labels_csv,
                           'sampling_type': sampling_type, 'sampling': sampling,
                           'num_pkt_thresh': num_pkt_thresh, 'feat_set': feat_set, 'verbose': verbose}
        pprint(OrderedDict(funcparams_dict), name=parse_dataset.__name__)
    ...
    for i, pcap_file in enumerate(pcap_file_lst):
        flows_i = _load_pcap_to_flows(pcap_file, num_pkt_thresh)  # get all flows which at least has more than 2 packets
...
#
# def pcap2features_CICIDS2017(experiment='ind', input_dir='input_data/CICIDS2017', q_fixed_iat=0.9, q_sampling_rate=0.9):
...
#
#     if experiment == 'ind':
#         pcap_dir = 'Friday-WorkingHours'
#
#     elif experiment == 'more':
#         pcap_dir = 'Merged-WorkingHours'  # Merged Friday and Monday pcaps
#
...
#             pcap_file_lst = []
#             label_file_lst = []
#             for (pcap_dir_v, label_dir_v) in dir_tuple:  # for different days' data
#                 pcap_file_lst.append(os.path.join(input_dir, f'srcIP_{srcIP}', pcap_dir_v, f'srcIP_{srcIP}.pcap'))
#                 label_file_lst.append(os.path.join(input_dir, f'srcIP_{srcIP}', label_dir_v, f'srcIP_{srcIP}.csv'))
#
#             merged_pcaps = os.path.join(input_dir, f'srcIP_{srcIP}', pcap_dir, f'srcIP_{srcIP}.pcap')
#             if not os.path.exists(os.path.dirname(merged_pcaps)):
#                 os.makedirs(os.path.dirname(merged_pcaps))
...
#             combined_csv = pd.concat([pd.read_csv(f) for f in label_file_lst])
#             # export to csv
#             merged_labels = os.path.join(input_dir, f'srcIP_{srcIP}', pcap_dir, f'srcIP_{srcIP}.csv')
#             print(f'{merged_labels}')
#             combined_csv.to_csv(merged_labels, index=False, encoding='utf-8-sig')
#     elif experiment == 'mix':
#         # merge 5 IPs in one
#         pcap_dir = 'mix-WorkingHours'  # Merged Friday and Monday pcaps
#
#         dir_tuple = [('Friday-WorkingHours', 'Friday-WorkingHours')  # (pcap_dir, label_dir)
#                      ]
#         pcap_file_lst = []
...
#         merged_pcaps = os.path.join(input_dir, f'srcIP_{mixed_srcIP}', pcap_dir, f'srcIP_{mixed_srcIP}.pcap')
#         if not os.path.exists(os.path.dirname(merged_pcaps)):
#             os.makedirs(os.path.dirname(merged_pcaps))
#         cmd = f"mergecap -w {merged_pcaps} " + ' '.join(pcap_file_lst)
#         print(f'{cmd}')
...
#         combined_csv = pd.concat([pd.read_csv(f) for f in label_file_lst])
#         # export to csv
#         merged_labels = os.path.join(input_dir, f'srcIP_{mixed_srcIP}', pcap_dir, f'srcIP_{mixed_srcIP}.csv')
#         print(f'{merged_labels}')
#         combined_csv.to_csv(merged_labels, index=False, encoding='utf-8-sig')
#
#         srcIP_lst = [mixed_srcIP]
#
...
#     sampling_lst = []
#
#     # print('\n\n' + '-' * 50 + f'pcap_dir: {pcap_dir}, label_dir: {label_dir}' + '-' * 50)
#     for srcIP in srcIP_lst:  # for different IPs
#         print('\n' + '+' * 35 + f'srcIP: {srcIP}' + '+' * 35)
#
#         pcap_file = os.path.join(input_dir, f'srcIP_{srcIP}', pcap_dir, f'srcIP_{srcIP}.pcap')
#         label_file = os.path.join(input_dir, f'srcIP_{srcIP}', pcap_dir, f'srcIP_{srcIP}.csv')
#
#         # 1) obtain fids, features, labels from pcap and label_file
#         print(f'\nobtain IAT set')
#         prop_set = 'IAT'  # only IAT
...
#     # break
#
#     return q_fixed_iat, pcap_dir, q_sampling_rate, sampling_lst
#
#
# def pcap2features_smart_tv(experiment='ind', input_dir='input_data/smart-tv-roku-data', q_fixed_iat=0.9,
#                            q_sampling_rate=0.9):
#     """ Obtain raw features data: IAT, Baseline 1 and Baseline 2
#         1) IAT (with different dimensions of different flows),
#         2) Baseline 1 (statistic: such as max, mean and std): fixed length (10 dimensions vector)
...
#     normal_pcap_file = os.path.join(input_dir, 'multi-srcIPs/normal_anomaly/merged_normal.pcap')
#     anomaly_pcap_file = os.path.join(input_dir, 'multi-srcIPs/normal_anomaly/merged_anomaly.pcap')
#     pcap_file = os.path.join(input_dir, f'multi-srcIPs/normal_anomaly/{srcIP}')
#     # 1) obtain fids, features, labels from pcap and label_file
#     print(f'\nobtain IAT set')
#     prop_set = 'IAT'  # only IAT
#     prop_fids, prop_features, prop_labels, _ = parse_dataset_smart_tv(
#         pcap_file_lst=[normal_pcap_file, anomaly_pcap_file], labels_csv=None,
#         num_pkt_thresh=num_pkt_thresh, feat_set=prop_set)
#     iat_file = f'{pcap_file}-{prop_set}.dat'  # store IATs which have different dimensions to file
#     dump_data(data=(prop_fids, prop_features, prop_labels), output_file=iat_file)
#
...
#         pcap_file_lst=[normal_pcap_file, anomaly_pcap_file],
#         labels_csv=None,
#         sampling_type=sampling_type,
#         sampling=None,
#         q_sampling_rate=q_sampling_rate,
...
@execute_time
def main_pcap2features(experiment='ind', q_fixed_iat=0.9, q_sampling_rate=0.9, dataset_name='smart-tv-roku-data'):
    input_dir = f'input_data/q_sampling_rate_{q_sampling_rate}/{dataset_name}'
    if dataset_name == 'CICIDS2017':
        q_fixed_iat, pcap_dir, q_sampling_rate, sampling_lst = \
            pcap2features_CICIDS2017(experiment=experiment, input_dir=input_dir, q_fixed_iat=q_fixed_iat,
                                     q_sampling_rate=q_sampling_rate)
    elif dataset_name == 'smart-tv-roku-data':
        q_fixed_iat, pcap_dir, q_sampling_rate, sampling_lst = \
            pcap2features_smart_tv(experiment=experiment, input_dir=input_dir, q_fixed_iat=q_fixed_iat,
                                   q_sampling_rate=q_sampling_rate)
...
if __name__ == '__main__':
    ...
    main_pcap2features(experiment='more',
                       q_fixed_iat=0.9,  # it impacts IAT/FFT/Baseline 2 (pcap2features)
                       q_sampling_rate=0.9, dataset_name='CICIDS2017')  # dataset_name='smart-tv-roku-data'
