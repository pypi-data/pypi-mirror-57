...
from data_process import stat_data
...
if __name__ == '__main__':
    ...
    for i, srcIP in enumerate(srcIP_lst):
        ...
        if data_aug:
            ...
            input_file = f'input_data/CICIDS2017/srcIP_{srcIP}/Friday-WorkingHours/srcIP_{srcIP}.pcap-{base_set}-dim_10.dat'

        output_file = input_file + '_flows_duration.dat'
        _, features, _ = load_pickled_data(input_file)
        input_file = output_file
        IAT_len_arr = [v[0] for v in features]
        stat_data(np.asarray(IAT_len_arr).reshape(-1, 1))
        ...
        if rescale_flg:
            IAT_len_arr = [value for value in IAT_len_arr if value < 1]
            bins = 30
        ...
        title = f'srcIP:{srcIP},\nmin:{(min(IAT_len_arr))}, max:{np.round(max(IAT_len_arr))}'
        plt.title(f"{title}")
        plt.ylabel('Counts')
        # plt.xlabel('Number of packets in each flow')
