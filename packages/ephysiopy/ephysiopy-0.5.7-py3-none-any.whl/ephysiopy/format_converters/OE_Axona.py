...
class OE2Axona(object):
	...
	def exportPos(self, ppm=300, jumpmax=100):
		...
		pos_header['duration'] = str(int(self.last_pos_ts - self.first_pos_ts).astype(np.int))
	...
	def makeLFPData(self, hdf5_continuous_data: np.array, eeg_type='eeg', gain=1000):
		...
		hdf5_continuous_data = hdf5_continuous_data[0:int(self.last_pos_ts-self.first_pos_ts)*dst_rate]
