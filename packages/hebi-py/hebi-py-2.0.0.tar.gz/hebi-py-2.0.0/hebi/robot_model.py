...
class RobotModel(UnmanagedObject):
  ...
  def metadata(self):
    """
    Retrieves info about the elements which compose this robot
    """
...
class _ActuatorMetaData(object):
...
class _RigidBodyMetaData(object):
  __slots__ = []
...
class _EndEffectorMetaData(object):
...
def import_from_hrdf_string(hrdf_string, warning_callback=None):
  ...
  if len(warnings) > 0:
    if warning_callback is None:
      msg = f'Importing HRDF from buffer generated the following warning(s):\n'
      import sys
      sys.stderr.write(msg + '\n'.join(warnings) + '\n')
    else:
      warning_callback(warnings)
