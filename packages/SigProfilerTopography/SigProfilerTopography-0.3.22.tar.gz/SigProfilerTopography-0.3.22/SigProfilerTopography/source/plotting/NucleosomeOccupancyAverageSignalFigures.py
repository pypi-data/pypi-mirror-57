...
#########################################################
def plotSignatureBasedFigures(mutationType,signature2PropertiesListDict,sample2Signature2NumberofMutationsDict,outputDir,jobname,isFigureAugmentation,numberofSimulations,libraryFilename,libraryFilenameMemo,occupancy_type,plusOrMinus):
    if (occupancy_type==NUCLEOSOMEOCCUPANCY):
...
def calculate_fold_change(output_dir,numberofSimulations,signature,cancer_type,encode_hm):
    ...
    if (avg_real_signal is not None) and (avg_simulated_signal is not None):
        ...
        print('avg_real_signal:%f\tavg_simulated_signal:%f\tfold change:%f' % (avg_real_signal, avg_simulated_signal, fold_change))
        return fold_change
    else:
        print('%s %s nan' % (signature, encode_hm))
        return None
########################################################
...
def fill_signature2CancerType2Biosample2HM2FoldChangeDict(output_dir,numberofSimulations,signature,cancer_type,epigenomics_files_memos,epigenomics_biosamples,occupancy_type):
    ...
    list=[]
    # encode_hms_we_are_interested_in=['H3K36me3','H3K4me1','H3K4me3','H3K27ac','H3K9me3','H3K27me3']
...
def plot_pcawg_heatmap(list):
    ...
    for signature in signature2Biosample2EncodeHM2FoldChangeDict:
        cancer_type_encode_biosamples, hms, average_fold_change_array = fill_average_fold_change_array(signature,cancer_type,signature2Biosample2EncodeHM2FoldChangeDict[signature])

        cancer_type_encode_biosamples_list=[]
        for cancer_type_encode_biosample in cancer_type_encode_biosamples:
            cancer_type=cancer_type_encode_biosample[0]
            encode_biosample=cancer_type_encode_biosample[1]
            cancer_type_encode_biosamples_list.append('%s, %s' %(cancer_type,encode_biosample) )

        ##########################################################################
        # fig, ax = plt.subplots() #does not plot and blocks multiprocessing
        if len(cancer_type_encode_biosamples)>20:
        ...
        #Blue White Red
        im, cbar = heatmap(average_fold_change_array, cancer_type_encode_biosamples_list, hms, ax=ax, cmap='seismic', cbarlabel="Fold Change [Real mutations/Simulated Mutations]",vmin=0, vmax=2)
        ...
        fig.tight_layout()
        filename = '%s_rows_cancertypes_biosamples_columns_interested_hms_heatmap.png' % (signature)
