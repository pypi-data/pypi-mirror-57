...
from SigProfilerTopography.source.commons.ChrBasedSignalArrays import readBEDandWriteChromBasedSignalArrays
import time
import platform
...
# November 1, 2019
# Just fill the list
def fillInputList(using_pyBigWig,using_chrBasedArray,outputDir,jobname,chrLong,simNum,chromSizesDict,library_file_with_path,library_file_type,library_file_df_grouped,sample2NumberofSubsDict,sample2NumberofIndelsDict,sample2NumberofDinucsDict,
...
def test_combined_prepare_chrbased_data_fill_signal_count_arrays(occupancy_type,
    ...
    elif (((library_file_type == BED) or (library_file_type == NARROWPEAK)) and (library_file_df_grouped is not None)):
        chrom_based_library_df = library_file_df_grouped.get_group(chrLong)
...
def combined_prepare_chrbased_data_fill_signal_count_arrays(
    ...
    #For big files we can consider chrBasedArrays
    if (using_chrBasedArray and os.path.exists(chrBasedSignalFile)):
        chrBasedSignalArray = np.load(chrBasedSignalFile, mmap_mode='r')
        # For testing purposes
        # chrBasedSignalArray = np.load(chrBasedSignalFile,mmap_mode=None)
    ...
    #Fill online chrbasedArray
    #For small files filling online chrBasedArrays are very costly.
    # elif (((library_file_type == BED) or (library_file_type == NARROWPEAK)) and (library_file_df is not None) and (os.path.exists(library_file_with_path))):
    #     chrom_based_library_df = library_file_df_grouped.get_group(chrLong)
    #     # chrBasedSignalArray and library_file_df  signal column is of type np.float32
    #     chrBasedSignalArray = np.zeros(maximum_chrom_size, dtype=np.float32)
...
def combined_prepare_chrbased_data_fill_signal_count_arrays_using_inputList(inputList):
    ...
    library_file_df_grouped=inputList[9]
    sample2NumberofSubsDict=inputList[10]
    sample2NumberofIndelsDict=inputList[11]
    sample2NumberofDinucsDict=inputList[12]
    sample2SubsSignature2NumberofMutationsDict=inputList[13]
    sample2IndelsSignature2NumberofMutationsDict=inputList[14]
    sample2DinucsSignature2NumberofMutationsDict=inputList[15]
    subsSignature2PropertiesListDict=inputList[16]
    indelsSignature2PropertiesListDict=inputList[17]
    dinucsSignature2PropertiesListDict=inputList[18]
    plusorMinus=inputList[19]
    sample_based=inputList[20]
...
def occupancy_analysis(genome,
    ...
    #     library_file_df_grouped=library_file_df.groupby('chrom')
        ...
        if (file_extension.lower()=='bed'):
            ...
            library_file_type = NARROWPEAK
        readBEDandWriteChromBasedSignalArrays(genome, library_file_with_path, occupancy_type)
    ...
    ###################################################################################
        ...
        for simNum, chrLong in sim_num_chr_tuples:
            # Accumulation
            pool.apply_async(test_combined_prepare_chrbased_data_fill_signal_count_arrays, (occupancy_type,using_pyBigWig,using_chrBasedArray,
                          outputDir, jobname, chrLong, simNum, chromSizesDict, library_file_with_path,
                          library_file_type, library_file_df_grouped,
                          sample2NumberofSubsDict, sample2NumberofIndelsDict, sample2NumberofDinucsDict,
                          sample2SubsSignature2NumberofMutationsDict, sample2IndelsSignature2NumberofMutationsDict,
