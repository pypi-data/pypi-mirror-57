...
class Opssat1(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"DP0OPS":
                self.info = self._root.OpssatPayload(self._io, self, self._root)
    ...
    class CubeSatProtocol(KaitaiStruct):
        ...
        def _read(self):
            self.header = self._root.CspHeader(self._io, self, self._root)
            if  ((self.header.destination == 10) and (self.header.priority == 3) and (self.header.source == 5) and (self.header.destination_port == 31)) :
                self.beacon_data = self._root.CspBeaconData(self._io, self, self._root)
    ...
    class OpssatPayload(KaitaiStruct):
        ...
        def _read(self):
            self.opssat_payload = self._root.CubeSatProtocol(self._io, self, self._root)
            self.crc_32c = self._io.read_u4be()
            self.rs_block = [None] * (32)
            for i in range(32):
                self.rs_block[i] = self._io.read_u1()
