...
import re
import requests
from azureml._vendor.azure_storage.blob import (BlobPermissions, BlockBlobService, ContentSettings)
    ...
    if (not deploy_config_file):
        if compute_type == "mir":
            deploy_config = MirWebservice.deploy_configuration(
                ...
                tls_mode=tls_mode, certificate_fingerprints=certificate_fingerprints, sku=sku)
    else:
        ...
        if compute_type == "mir":
            deploy_config = params_to_mir_config(autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas,
                                                 ...
                                                 certificate_fingerprints, sku, deploy_config_from_file)
...
def params_to_mir_config(autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas, autoscale_refresh_seconds,
                         ...
                         description, tls_mode, certificate_fingerprints, sku, config_from_file):
