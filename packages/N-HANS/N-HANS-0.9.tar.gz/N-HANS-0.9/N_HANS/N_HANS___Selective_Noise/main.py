...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
#   Description:      N-HANS contains two parts, speech denoiser and speech separator. This is the second part.        #
...
import tensorflow as tf, time, numpy as np, scipy.io.wavfile
...
#               ||
#               \/
from N_HANS.N_HANS___Selective_Noise import blocks, reader
dense, conv2d, batch_norm, flatten, lstm = blocks.dense, blocks.conv2d, blocks.batch_norm, blocks.flatten, blocks.lstm
...
#[os.system('mkdir {}'.format(dir_name)) for dir_name in ['wav_dump', 'dump', 'checkpoints', 'summaries']]
...
def model(inputs, istrain):
  ...
  with tf.variable_scope('embedding'):
    ...
    nout = tf.nn.avg_pool2d(nout, [1, nout.shape[1].value, nout.shape[2].value, 1], [1, 1, 1, 1], 'VALID') # [mb, 1, 1, 512]
...
def evaluate(outputs, ereader, step):
  ...
  for i in range(0, len(startlocations)):
    ...
    # Save the waveforms
    cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg = outputs['cleanpath'][s], outputs['noisepospath'][s], outputs['noisenegpath'][s], outputs['snr_pos'][s], outputs['snr_neg'][s]
    cleanpath = cleanpath.split('/')[-1][:-4]
    noisepospath = noisepospath.split('/')[-1][:-4]
    noisenegpath = noisenegpath.split('/')[-1][:-4]
    mixed_filename = '{}_{}_{}_{}_{}_{}_{}_{}.wav'.format(modelname, step, cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg, 'mixed')
    denoised_filename = '{}_{}_{}_{}_{}_{}_{}_{}.wav'.format(modelname, step, cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg, 'denoised')
    pos_filename = '{}_{}_{}_{}_{}_{}_{}_{}.wav'.format(modelname, step, cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg, 'posNoise')
    neg_filename = '{}_{}_{}_{}_{}_{}_{}_{}.wav'.format(modelname, step, cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg, 'negNoise')
    target_filename = '{}_{}_{}_{}_{}_{}_{}_{}.wav'.format(modelname, step, cleanpath, noisepospath, noisenegpath, snr_pos, snr_neg, 'target')
    ...
    scipy.io.wavfile.write(os.path.join(FLAGS.wav_dump_folder, target_filename), 16000, targetsamples)
    scipy.io.wavfile.write(os.path.join(FLAGS.wav_dump_folder, pos_filename), 16000, possamples)
    scipy.io.wavfile.write(os.path.join(FLAGS.wav_dump_folder, neg_filename), 16000, negsamples)
    ...
    def save_and_eval():
        ...
        for ereader in ereaders:
          ...
          if FLAGS.dump_results:
            for k in results.keys():
              np.save(os.path.join(FLAGS.dump_results, '{}_{}_{}_{}'.format(modelname, ereader.name, tstep, k)), aggregators[k])
