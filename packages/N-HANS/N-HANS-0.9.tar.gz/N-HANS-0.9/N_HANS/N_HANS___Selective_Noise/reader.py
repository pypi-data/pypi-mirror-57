...
tf.app.flags.DEFINE_string('noise_wav_dir', './noise_wav_dir/', '')
...
def combine_signals(istrain, cleanpath, noisepospath, noisenegpath):
    try:
        ...
        if istrain:
          ...
          return target, noisepossignal, noisenegsignal, mixed, np.array(snr_pos, dtype=np.int32), np.array(snr_neg, dtype=np.int32)
...
class read_seeds:
    ...
    def __init__(self, name, queuesize, min_after_dequeue, nthreads):
        ...
        noise_wav_dir = FLAGS.noise_wav_dir
        if name == 'train':
            self.seedspaths = [os.path.join(speech_wav_dir, 'train.pkl'), os.path.join(noise_wav_dir, 'train.pkl')]
        elif name == 'valid':
            self.seedspaths = [os.path.join(speech_wav_dir, 'valid.pkl'), os.path.join(noise_wav_dir, 'valid.pkl')]
        elif name == 'test':
            self.seedspaths = [os.path.join(speech_wav_dir, 'test.pkl'), os.path.join(noise_wav_dir, 'test.pkl')]
    ...
    def get_examples(self, istrain, seedsqs):
        ...
        target_wav, noise_pos_wav, noise_neg_wav, mix_wav, snr_pos, snr_neg = tf.py_func(combine_signals, [istrain, clean_seed, noise_pos_seed, noise_neg_seed], [tf.float32, tf.float32, tf.float32, tf.float32, tf.int32, tf.int32])

        # flatten
        target_wav, noise_pos_wav, noise_neg_wav, mix_wav = [tf.reshape(x, [-1]) for x in (target_wav, noise_pos_wav, noise_neg_wav, mix_wav)]

        # stft
        target_fft = tf.contrib.signal.stft(target_wav, self.frame_length, self.frame_step, fft_length=self.frame_length)
        noise_pos_fft = tf.contrib.signal.stft(noise_pos_wav, self.frame_length, self.frame_step, fft_length=self.frame_length)
        noise_neg_fft = tf.contrib.signal.stft(noise_neg_wav, self.frame_length, self.frame_step, fft_length=self.frame_length)
        mix_fft = tf.contrib.signal.stft(mix_wav, self.frame_length, self.frame_step, fft_length=self.frame_length)
        # Magnitude
        target_mag, noise_pos_mag, noise_neg_mag, mix_mag = [tf.abs(x) for x in [target_fft, noise_pos_fft, noise_neg_fft, mix_fft]]
        # Phase
        # temp_wav = mix_wav
        mix_phase = tf.angle(mix_fft)
        target_phase = tf.angle(target_fft)
        pos_phase = tf.angle(noise_pos_fft)
        neg_phase = tf.angle(noise_neg_fft)

        # log
        target_log, noise_pos_log, noise_neg_log, mix_log = [tf.log(x + 1e-5) for x in [target_mag, noise_pos_mag, noise_neg_mag, mix_mag]]

        if istrain:
            # padding
            target_log, noise_pos_log, noise_neg_log, mix_log, mix_phase, target_phase, pos_phase, neg_phase = [self.pad_1D_for_windowing(x, self.window_frames) for x in [target_log, noise_pos_log, noise_neg_log, mix_log, mix_phase, target_phase, pos_phase, neg_phase]]
            ...
            for _ in range(self.random_slices):
                ...
                targetcropped, mixedcropped, mixedphcropped, targetphcropped, poscropped, posphcropped, negcropped, negphcropped, noiseposcontext, noisenegcontext = \
                    self.synchronized_1D_crop(target_log, noise_pos_log, noise_neg_log, mix_log, mix_phase, target_phase, pos_phase, neg_phase, self.window_frames, self.context_frames)

                # Add to list
                for (lst, tnsr) in zip([targetslices, mixedslices, mixedphslices, targetphslices, posslices, posphslices, negslices, negphslices, noise_pos_slices, noise_neg_slices], [targetcropped, mixedcropped, mixedphcropped, targetphcropped, poscropped, posphcropped, negcropped, negphcropped, noiseposcontext, noisenegcontext]):
                    lst.append(tnsr)

            # Stack and return
            return tf.stack(targetslices, axis=0), tf.stack(mixedslices, axis=0), tf.stack(mixedphslices, axis=0), tf.stack(targetphslices, axis=0), tf.stack(posslices, axis=0), tf.stack(posphslices, axis=0), tf.stack(negslices, axis=0), tf.stack(negphslices, axis=0), \
                ...
                tf.constant([''] * self.random_slices, dtype=tf.string), tf.constant([''] * self.random_slices, dtype=tf.string), tf.constant([0] * self.random_slices, dtype=tf.int32), tf.constant([0] * self.random_slices, dtype=tf.int32)
        else:
    ...
    def synchronized_1D_crop(self, target, noise_pos, noise_neg, mixed, mixedph, targetph, posph, negph, winlength, contextlength):
        ...
        #targetrest = tf.concat([clean[:winstart], clean[winstart+winlength:]], axis=0)
        ...
        posreststart = tf.random_uniform([], dtype=tf.int32, minval=0, maxval=posrestmaxval+1)
        ...
        negreststart = tf.random_uniform([], dtype=tf.int32, minval=0, maxval=negrestmaxval + 1)
    ...
    def get_inputs(self):
        if not hasattr(self, 'ph'):
            self.ph = [tf.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='targetph'),
                       ...
                       tf.placeholder(dtype=tf.float32, shape=[None, self.window_frames, 201], name='posph'),
                       tf.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='posphaseph'),
                       tf.placeholder(dtype=tf.float32, shape=[None, self.window_frames, 201], name='negph'),
                       tf.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='negphaseph'),
                       tf.placeholder(dtype=tf.float32, shape=[None, self.context_frames, 201], name='noiseposcontextph'),
                       tf.placeholder(dtype=tf.float32, shape=[None, self.context_frames, 201], name='noisenegcontextph'),
...
if __name__ == '__main__':
    ...
    with g.as_default():
        ...
        for i in range(1):
            print(i)
            t1 = time.time()
            target_, mixed_, mixedph_, targetph_, pos_, posph_, neg_, negph_, noiseposcontext_, noisenegcontext_, location_, cleanpath_, noisepospath_, noisenegpath_, snr_pos_, snr_neg_ = sess.run(reader.batch, feed_dict=feed_dict)
            t2 = time.time()
