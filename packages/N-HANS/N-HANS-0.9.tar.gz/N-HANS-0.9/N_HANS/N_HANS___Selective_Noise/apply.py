...
def apply_demo(speechpath, pospath, negpath, save_to):
    ...
    with g.as_default():
        with tf.device('/cpu:0'):
            ...
            mix_spectrum, pos_spectrum, neg_spectrum = [tf.math.log(tf.abs(wav_stft) + 1e-5) for wav_stft in [mix_stft, pos_stft, neg_stft]]
    ...
    with eg.as_default():
        ...
        with tf.device('/gpu:0'):
            in_ph = [tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='targetph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 35, 201], name='mixedph'),
                  ...
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 35, 201], name='posph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='posphaseph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 35, 201], name='negph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 1, 201], name='negphaseph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 200, 201], name='noiseposcontextph'),
                  tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, 200, 201], name='noisenegcontextph'),
...
def apply_snc(mixedpath, pospath, negpath, save_to):
    ...
    with g.as_default():
        with tf.device('/cpu:0'):
            ...
            mix_spectrum, pos_spectrum, neg_spectrum = [tf.math.log(tf.abs(wav_stft) + 1e-5) for wav_stft in
