...
def test_removeSubjects_multiple_columns():
    ...
    with pytest.raises(ValueError):
        importing.removeSubjects(df, cols, subjectExprs=exprs)

    # combine columns within var
    df, cols, data = gendata()
    exprs = ['all(v1 > 2) && any(v2 < 7)']
    exp = all(data[:, :3] > 2) & any(data[:, 3:] < 7)
    got = importing.removeSubjects(df, cols, subjectExprs=exprs)
