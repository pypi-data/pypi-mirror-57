...
class DataSet:
	...
	def do_warp(self,cutline_feature=None,destroy_after=True,out_epsg=None,**kwargs):
		...
		gdal.Warp(temp_filepath,self.ds,srcSRS=f'EPSG:{self.epsg}',dstSRS=f'EPSG:{out_epsg or self.epsg}',
		          cutlineDSName=cutline_feature,**dict(dict(cropToCutline=not cutline_feature is None),**kwargs))
		ds=DataSet(temp_filepath)
		# cropped_ds=gdal.Warp('',self.ds,format='VRT',cropToCutline=cropToCutline,cutlineDSName=cutline_feature,**kwargs)
	...
	def translate(self,options=None,format=None,
	              ...
	              callback=None,callback_data=None,destroy_after=True):
		temp_filepath=f"/vsimem/{uuid.uuid4()}"
		gdal.Translate(temp_filepath,self.ds,options=gdal.TranslateOptions(
		...
		ds=DataSet(temp_filepath,name='translated')
		if destroy_after:
			self.ds=None
		return ds
