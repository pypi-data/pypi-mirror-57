	...
	def draw(self, ax, TGT=None):
		...
		for fs in self._FSlist:
			if fs._type == "function":
				...
				ax.plot(x,y, "--", label=fs._term)
			else:
				sns.regplot(fs._points.T[0], fs._points.T[1], marker="d", fit_reg=False, ax=ax)
				f = interp1d(fs._points.T[0], fs._points.T[1], bounds_error=False, fill_value=(0,0))
				ax.plot(x, f(x), "--", label=fs._term)
				if TGT is not None:
					ax.plot(TGT, f(TGT), "*", ms=10, label="x")
		ax.set_title(self._concept)
		ax.legend(loc="best")
		return ax
...
def find_index_operator(string, verbose=True):
	print ("Looking for operator in", string)
	pos = 0
	par = 1
	while(par>0):
		#print(pos)
...
def curparse(STRINGA, verbose=False):
	...
	if regex.match(STRINGA):
		...
		if verbose:	print (" * Regular expression is matching")
		...
		if verbose:	print (" * Regular expression is not matching:", STRINGA)
		...
		print( " * After parentheses removal:", removed_parentheses)
