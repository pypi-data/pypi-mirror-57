...
from assemblyline_v3_service.common.classification import InvalidClassification
from assemblyline_v3_service.common.context import Context
from assemblyline_v3_service.common.net import is_valid_ip, is_valid_domain, is_valid_email, is_valid_port
from assemblyline_v3_service.common.str_utils import StringTable, NamedConstants, safe_str
from assemblyline_v3_service.common.dict_utils import unflatten
from assemblyline_v3_service.common.tagging import tag_list_to_dict
from assemblyline_v3_service.common.compat_tag_map import v3_lookup_map
...
class Result(dict):
    ...
    def _score_heuristic(self, score):
        heur = {
            'heur_id': 'v3_compat',
            'score': score,
        }
    ...
    def append_tag(self, tag):
        ...
        self.add_tag(tag.tag_type, tag.value,
    ...
    def report_heuristic(self, heur):
        log.warning("Reporting Heuristics is not supported for v3 services running in AL v4.")
    ...
    def finalize(self):
        ...
        # Move all Result root tags to 'Other Tags' section
        if self.tags:
            section = ResultSection(title_text="Other Tags")
        ...
        # Move score to a heuristic
        for section in range(len(self.sections)):
            if not self.sections[section].body:
                self.sections[section].body = None

            if self.sections[section].score != 0:
                self.sections[section]['heuristic'] = self._score_heuristic(self.sections[section].score)

            if self.sections[section].tags:
                tags = []
                for tag in self.sections[section].tags:
                    tag['type'] = v3_lookup_map[tag['type']]
                    tags.append(tag)
                self.sections[section].tags = unflatten(tag_list_to_dict(tags))
            else:
                self.sections[section].tags = {}

            self.sections[section].pop('subsections', None)
            self.sections[section].pop('score', None)
            self.sections[section].pop('parent', None)
            self.sections[section].pop('finalized')
        ...
        if hasattr(sect, 'subsections'):
            if len(sect.subsections) != 0:
                for item in sect.subsections:
                    temp_item = item
                    temp_item.pop('subsections', None)
                    self.temp_list.append(temp_item)
                    self.flatten_subsections(item)
        else:
            for item in sect:
                temp_item = item
                popped = temp_item.pop('subsections', None)
                self.temp_list.append(temp_item)
