...
class DriveService:
	...
	def upload_file(self, name, filePath, folder_id, mime = None):
		if mime == None:
	...
	def list_folders_by_name(self, name, extraArguments=None):
		return self.__list_items_by_name(name, "mimeType = 'application/vnd.google-apps.folder'", extraArguments)

	def list_files_by_name(self, name, extraArguments=None):
		return self.__list_items_by_name(name, "mimeType != 'application/vnd.google-apps.folder'", extraArguments)

	def __list_items_by_name(self, name, extraQuery=None, extraArguments = None):
		query = ""
		name = name.split(" ")
		for i in name:
			query = query + "fullText contains '" + i + "' and "
		
		if extraQuery != None:
			query += " and " + extraQuery
		if extraArguments != None:
			query += " " + extraArguments
		
		while True:
			if "  " in query or "and and" in query:
				query = query.replace("  "," ").replace("and and","and")
			else:
				break
