...
def pad_array(array, kh, kw, sh, sw):
    '''Pad array before and after W & H according kernel size and stride.
...
def random_crop(array, crop_height=256, crop_width=256):
    '''
    Crop image(label) randomly
    '''
    if (crop_height < array.shape[0] and crop_width < array.shape[1]):
        x = np.random.randint(0, array.shape[0] - crop_height)  # row
        y = np.random.randint(0, array.shape[1] - crop_width)  # column
        return array[x:x + crop_height, y:y + crop_width]

    elif (crop_height == array.shape[0] and crop_width == array.shape[1]):
        return array
...
def random_crop_pair(image, label, crop_height=256, crop_width=256):
    '''
...
def random_crop_balance(image, label, crop_h=256, crop_w=256, num_classes=0):
    '''
