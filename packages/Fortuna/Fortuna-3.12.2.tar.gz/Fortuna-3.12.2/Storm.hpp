...
namespace Storm {  // Version 3.2.2
    ...
    namespace GearBox { // Internal
        ...
        auto approximation_clamp(Func && approximation, Storm::Integer target, Storm::Integer upper_bound) -> Storm::Integer {
            if (target >= 0 and target < upper_bound) return target;
            return approximation(upper_bound);
        }
        template <typename Func>
        auto analytic_continuation(Func && func, Storm::Integer input, Storm::Integer offset) -> Storm::Integer {
