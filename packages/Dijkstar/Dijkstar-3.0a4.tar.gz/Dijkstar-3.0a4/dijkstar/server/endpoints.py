...
async def find_path(request: Request) -> JSONResponse:
    ...
    parameters:
        ...
        - in: query
          ...
          required: false
          format:
              Semicolon-separated list of edges; each edge has the form
              "{u}:{v}:{edge_data}"
          description:
              Edges to add to the annex graph. The nodes for these edges
              that exist in the main graph will first be copied to the
              annex graph. Direct travel between these nodes will be
              disabled in the annex graph by removing their connecting
              edges. Finally, the specified edges will be added to the
              annex graph.
        - in: query
          name: cost_func
          required: false
    ...
    if annex_edges:
        ...
        for item in annex_edges:
            ...
            edges_to_add.append((u, v, edge))
        annex = graph.subgraph(existing_nodes)
        for u, v, edge in edges_to_add:
