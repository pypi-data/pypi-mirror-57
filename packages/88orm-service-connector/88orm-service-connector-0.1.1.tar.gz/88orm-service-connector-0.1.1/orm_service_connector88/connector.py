...
class _VirtualInstance(object):
    def __init__(self, model: str, payload: dict, app_label=None):
        self.__model = model
    ...
    def __setattr__(self, key, value):
        try:
            ...
            if key not in ["__model", "_attrs", "_payload", "_app_label"]:
                self._attrs.update({
                    key: value
                })
        except Exception:
...
class ORMServices(object):
    ...
    def __init__(self, model, fields=['__all__'], exclude_fields=None, app_label=None):
        if isinstance(model, str):
            ...
            self._model_name = model._meta.model_name
        else:
    ...
    def __update_payload(self, name, data) -> None:
        try:
            existed = self._payload.get(name).copy()  # type: List
    ...
    def __resolve_q(self, params: Q, result, extra_params) -> List:
        ...
        if isinstance(params, Q):
            ...
            if extra_params:
                ...
                return self.__resolve_q(params=params, result=result,
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if isinstance(value, dict) and with_relation:
                    value = self.__bind(model=key, data=value,
