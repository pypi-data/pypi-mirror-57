...
class _RLBaseEstimator(ABC):
    ...
    def __init__(self,
        ...
        :param rl_framework: Orchestration framework to be used in the experiment. The default is Ray version 0.7.2
        :type rl_framework: azureml.contrib.train.rl.RLFramework
        :param job_queue_timeout: The maximum time the job can spend in queued state in seconds. Jobs stay in the
            compute state while they are waiting for the compute target to be ready to start the job.
        :type job_queue_timeout: int
    ...
    def _get_simulator_configuration(simulator_configuration):
        if simulator_configuration:
            ...
            if not simulator_configuration.environment:
                simulator_configuration.environment = EnvironmentDefinition()
