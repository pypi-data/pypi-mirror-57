    ...
    class Cache:
        ...
        def __parse(self):
            ...
            while True:
                        ...
                        if date_regex:
                            if week.search(bytes.fromhex(date_regex.group()).decode()[5:9]):
                                if date_regex.group()[-2:]=="20":
                                    ...
                                    access=access.strip()
                                    access = datetime.datetime.strptime(access, '%d %b %Y %H:%M:%S')
                        ...
                        if expires_regex:
                            if week.search(bytes.fromhex(expires_regex.group()).decode()[8:12]):
                                if expires_regex.group()[-2:] == "20":
                                    ...
                                    expiry = expiry.strip()
                                    expiry = datetime.datetime.strptime(expiry, '%d %b %Y %H:%M:%S')
                        ...
                        if last_modified_regex:
                            if week.search(bytes.fromhex(last_modified_regex.group()).decode()[14:18]):
                                if last_modified_regex.group()[-2:]=="20":
                                    ...
                                    last_modify = datetime.datetime.strptime(last_modify, '%d %b %Y %H:%M:%S')
                ...
                mkdict["last_modified_time"] = last_modify.strftime("%Y-%m-%d %H:%M:%S")
    ...
    class Cookie:
        ...
        def __parse(self):
            ...
            for cookie in cookies_open:
                ...
                mkdict["name"] = cookie[name]
                mkdict["value"] = cookie[value]
                c_time = convert_time.convert_time_firefox1(cookie[creationTime])
                mkdict["timezone"]=c_time.strftime("%Z")
                mkdict["creation_time"] = c_time.strftime("%Y-%m-%d %H:%M:%S")
                mkdict["last_accessed_time"] = convert_time.convert_time_firefox1(cookie[lastAccessed]).strftime("%Y-%m-%d %H:%M:%S")
                mkdict["expiry_time"] = convert_time.convert_time_firefox3(cookie[expiry]).strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    for i in range(0, len(downloads_open)):
                        if anno_attribute[0] == downloads_open[i][0]:
                            if anno_attribute[1] == "downloads/destinationFileURI":
                                ...
                                c_time=convert_time.convert_time_firefox1(downloads_open[i][2])
                                mkdict["download_start_time"] = c_time.strftime("%Y-%m-%d %H:%M:%S")
                            elif anno_attribute[1] == "downloads/metaData":
                                tempdict = eval(downloads_open[i][1])
                                c_time = convert_time.convert_time_firefox2((tempdict["endTime"]))
                                mkdict["download_end_time"] = c_time.strftime("%Y-%m-%d %H:%M:%S")
        ...
        def __parse(self):
            ...
            for containerid in cache_container_id.keys():
                ...
                for cache in cache_container.records:
                    ...
                    mkdict["timezone"] = convert_time.convert_time(cache.get_value_data_as_integer(13)).strftime("%Z")
                    ...
                    mkdict["access_time"] = convert_time.convert_time(cache.get_value_data_as_integer(13)).strftime("%Y-%m-%d %H:%M:%S")
                    mkdict["creation_time"] = convert_time.convert_time(cache.get_value_data_as_integer(10)).strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    if str(cache.get_value_data_as_integer(11))[0] != 1:
                        ...
                        mkdict["expiry_time"] = convert_time.convert_time(cache.get_value_data_as_integer(11)).strftime("%Y-%m-%d %H:%M:%S")

                    mkdict["last_modified_time"] = convert_time.convert_time(cache.get_value_data_as_integer(13)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        def __parse(self):
            ...
            for containerid in cookies_container_id.keys():
                ...
                for cookie in cookies_container.records:
                    ...
                    mkdict["type"] = "cookies"
                    mkdict["browser"] = "IE10+ Edge"
                    mkdict["timezone"] = convert_time.convert_time(cookie.get_value_data_as_integer(10)).strftime("%Z")
                    mkdict["name"] = cookie.get_value_data_as_string(18)
                    mkdict["value"] = ""
                    mkdict["creation_time"] = convert_time.convert_time(cookie.get_value_data_as_integer(10)).strftime("%Y-%m-%d %H:%M:%S")
                    mkdict["last_accessed_time"] = convert_time.convert_time(cookie.get_value_data_as_integer(13)).strftime("%Y-%m-%d %H:%M:%S")
                    mkdict["expiry_time"] = convert_time.convert_time(cookie.get_value_data_as_integer(11)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        def __parse(self):
            ...
            for containerid in downloads_container_id.keys():
                ...
                for download in downloads_container.records:
                    ...
                    c_time=convert_time.convert_time(download.get_value_data_as_integer(13))
                    mkdict["timezone"] = c_time.strftime("%Z")
                    mkdict["download_start_time"] = c_time.strftime("%Y-%m-%d %H:%M:%S")
        ...
        def __parse(self):
            ...
            for containerid in history_container_id.keys():
                ...
                for visit in history_container.records:
                    ...
                    mkdict["from_visit"] = ""
                    mkdict["keyword_search"] = ""
                    c_time=convert_time.convert_time(visit.get_value_data_as_integer(13))
                    mkdict["visit_time"] = c_time.strftime("%Y-%m-%d %H:%M:%S")
                    mkdict["timezone"] = c_time.strftime("%Z")
