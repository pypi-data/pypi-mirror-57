...
ver_file_classes = [PythonVersionFile, NpmVersionFile]
...
def set_defaults(script_args: ScriptArgs, ver_file_for_meta: VersionFile):
    ...
    if not script_args.build_version:
        local_version_string = f'dev{int(round(time.time() * 1000))}'
        script_args.build_version = '{}-{}'.format(ver_file_for_meta.get_version(), local_version_string)
