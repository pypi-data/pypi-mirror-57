...
def parse_local_date_time(datetime_string):
    if datetime_string == '':
        return None
    return datetime.strptime(datetime_string, '%Y-%m-%dT%H:%M:%S')

def parse_UTC_date_time(datetime_string):
    if datetime_string == '':
        return None
...
def parse_gate_info(lookup_key, flight):
    ...
    print(f'terminal: {terminal}, gate: {gate}, baggage: {baggage}')
