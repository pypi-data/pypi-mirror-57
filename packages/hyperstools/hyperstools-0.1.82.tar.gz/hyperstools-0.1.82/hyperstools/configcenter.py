...
class ConfigHandle(object):
    ...
    def __init__(self, url, application, profile, username, password):
        self.config = requestConfigServer(url, application, profile, username, password)
...
def requestConfigServer(url, application, profile, username, password):
    """
    请求配置中心 获取项目配置
    :param url: git仓库地址
    ...
    response = requests.get(
        urljoin(url, f"{application}/{profile}"),
...
def getConfigCenter(
    url=None, application=None, profile=None, username=None, password=None
):
    """
   获取配置中心类
   :param url: git仓库地址
   ...
   """
    configHandler = ConfigHandle(url, application, profile, username, password)
