...
from sesh.config.base_config import (
    SESH_COOKIE_FILE, SESH_DB_PATH, SESH_SESSION_TIMEOUT
)
from sesh.enum import UserRole
from sesh.error import ExitCode, UserNotFound
from sesh import show
...
def _get_active_session(args):
    ...
    custom_timeout = os.environ.get('SESH_SESSION_TIMEOUT')

    if custom_timeout:
        timeout = int(custom_timeout)
    else:
        timeout = SESH_SESSION_TIMEOUT

    if SESH_COOKIE_FILE.exists():
        with open(str(SESH_COOKIE_FILE), 'r') as f:
            cookie = json.load(f)
