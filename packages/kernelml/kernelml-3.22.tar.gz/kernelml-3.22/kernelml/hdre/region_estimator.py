...
#             self.kde_target[count] = np.flipud(self.kde_target[count])
    ...
    def prune_clusters(self,X,pad=1.0,limit=0):
        ...
        self.kmldata.update_by_cycle = np.delete(self.kmldata.update_by_cycle[self.num_dim:], reduce, axis=0)
        
        self.kmldata.update_by_realization = np.delete(self.kmldata.update_by_realization[self.num_dim:], reduce, axis=0)
    
        self.kmldata.update_by_realization = np.delete(self.kmldata.update_history.T[self.num_dim:], reduce, axis=0).T
        
        self.kmldata.number_of_parameters - len(reduce)
