...
class PluginSkeleton(object):
    ...
    @staticmethod
    def registry_plugin(file_exec, func_exec, version_code):
        try:
            payload = {
    ...
    def run(self, file_exec, func_exec, version_code, ignore_register=False):
        # register plugin
        register_status = self.registry_plugin(file_exec, func_exec, version_code)
        ...
        if previous_job:
            ...
            while True:
                ...
                if count_jobs_finish == len(jobs):
                    results = {item[0]: item[1].result[item[0]] for item in jobs.items()}
                    if input_data:
