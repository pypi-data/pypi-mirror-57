...
def run_command(
    module, args, check_rc=False, close_fds=False, executable=None, data=None, cwd=None
):
    """
    ...
    def chown_dir(self, chown_user, target_path):
        if chown_user is not None:
            rc, out, err = self.module.run_command(
                "%s -R %s %s" % (self.chown_path, chown_user, target_path)
            )
            if rc > 0:
                self.module.fail_json(
                    msg="failed to chown %s: %s\n\t%s" % (self.pkg.name(), out, err)
                )
    ...
    def acquire(self):
        ...
        if exists(self.params, "src"):
            ...
            cache_dir = path.expanduser(
                get_env("battleschool_cache_dir", "~/Library/Caches/battleschool")
            )
            ...
            if not exists(self.params, "dest"):
                ...
                if num_files != 1:
                    self.module.fail_json(
                        msg="failed to locate download for %s: %s file(s) found in %s"
...
class ZipArchive(Archive):
    ...
    def open(self):
        ...
        rc, out, err = self.module.run_command(
            "%s %s -d %s" % (self.unzip_path, self.pkg_path(), self.target_dir)
        )
        if rc > 0:
            self.module.fail_json(
                msg="failed to unzip %s: %s\n\t%s" % (self.pkg_path(), out, err)
            )
...
class DmgArchive(Archive):
    ...
    def open(self):
        ...
        if rc > 0:
            self.module.fail_json(
                msg="failed to attach %s: rc: %s, %s, err: %s"
                % (self.pkg.name(), rc, out, err)
            )
    ...
    def close(self):
        rc, out, err = self.module.run_command(
            '%s unmount "%s"' % (self.hdiutil_path, self.dmg_volume)
        )
        if rc > 0:
            self.module.fail_json(
                msg="failed to unmount %s: %s\n\t%s" % (self.pkg.name(), out, err)
            )
...
class PkgPackage(Package):
    ...
    def find_version(self, module):
        rc, out, err = run_command(
            self.module,
            "%s --pkg-info=%s 2>&1 |grep version| awk '{print $2}'"
            % (self.pkgutil_path, self._pkg_name),
        )
        if rc > 0:
            module.fail_json(
                msg="failed to find version via pkgutil %s: %s %s"
                % (self._pkg_name, out, err)
            )
        return out.strip()
    ...
    def install(self, pkg_path):
        ...
        rc, out, err = self.module.run_command(
            '%s -pkg "%s" -target /' % (self.installer_path, pkg_path)
        )
        if rc > 0:
            return out, err
...
class AppPackage(Package):
    def __init__(self, module):
        ...
        if exists(self.params, "creates"):
            ...
            self.module.fail_json(
                msg="for pkg_type=app one of app_creates or archive_path must not be empty"
            )
...
class ScriptPackage(Package):
    def __init__(self, module):
        ...
        self._exe_path = module.get_bin_path(
            self.params["script_executable"], True, ["/usr/bin", "/usr/local/bin"]
        )
...
class Installer(object):
    ...
    def install(self, acquire_only):
        ...
        if pkg.is_installed() and not acquire_only:
            self.module.exit_json(
                changed=False,
                version=pkg.version(),
                msg="package %s already present" % pkg.name(),
            )
        else:
            ...
            if acquire_only:
                ...
                self.module.exit_json(
                    changed=acquired, msg="%s pkg %s" % (state, archive.source())
                )
                return
            ...
            if failed:
                archive.close()
                self.module.fail_json(
                    msg="failed to install package %s: %s %s"
                    % (archive.pkg_path(), failed[0], failed[1])
                )
            ...
            self.module.exit_json(
                changed=True,
                version=pkg.version(),
                msg="installed package %s" % pkg.name(),
            )
    ...
    def validate(self):
        if self.params["pkg_type"] == "app" and self.params["archive_type"] == "none":
            self.module.fail_json(
                msg="pkg_type can not = 'app' with archive_type = 'none'"
            )
