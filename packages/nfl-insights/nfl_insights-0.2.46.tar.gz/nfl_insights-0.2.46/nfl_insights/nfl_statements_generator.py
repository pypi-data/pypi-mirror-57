...
class NFLStatements:
    ...
    @contendo_classfunction_logger
    def _generate_all_team_statements(self, calculationsFilter=None, statsFilter=None):
    ...
    @contendo_classfunction_logger
    def _generate_all_player_statements(self, calculationsFilter=None, statsFilter=None):
        if not calculationsFilter:
            calculationsFilter = self.language['entities']['calculations'].keys()
        logger.debug('Generating all stats for calculation: %s', calculationsFilter)
        if not statsFilter:
            statsFilter = self._playerStats.statsDict.keys()
        logger.debug('Generating all stats: %s', statsFilter)

        _writeHeaders = True
        _statementsList = list()
        self.statementsDF = pd.DataFrame(columns=self._columns)
        for key1, _statDict in self._playerStats.allStatsDict.items():
            _season, _statname, _object = key1
            if not _statname in statsFilter:
                continue
            logger.debug('Covering main key: %s', key1)
    ...
    def _refactor_definition_json(self):
        ...
        for _statname, _statDef in _statDefs.items():
            ...
            for _statObject in _statObjects:
                _statsSource, _entityType, _obj, _objectType = _statObject
                if _obj in ['none', 'all', '']:
                    continue

                _objectSentences = _statStatements.get(_objectType, dict())
                _objectDef = self._teamStats.generator.ptmapDict[_objectType]
                _newObjectDef = dict()
                ...
                if _tt in _statTeams:
                    ...
                    _newObjectDef['isRelevant'] = False
                ...
                _newStatDef[_teamType] = _newObjectDef
    ...
    def _update_statement_text(self, statementRow: dict) -> None:
        # get stat text
        _teamObject = self._teamStats.generator.ptmapDict[statementRow['statObject']]['TeamType']
...
def test():
    ...
    # return
    #nfls._generate_all_team_statements(calculationsFilter=['stat', 'ratio', 'pct', 'per_game', 'per_drive'], statsFilter=[])
    nfls._generate_all_player_statements(calculationsFilter=['stat', 'ratio', 'pct', 'per_game', 'per_drive'], statsFilter=[])
