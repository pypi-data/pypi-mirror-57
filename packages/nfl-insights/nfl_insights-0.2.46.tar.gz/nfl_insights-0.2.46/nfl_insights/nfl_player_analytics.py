...
class NFLPlayerAnalytics:
    ...
    def _build_all_metrics(self, seasons):
        ...
        for statName, statDef in self.statsDict.items():
            ...
            for season in seasons:
                ...
                for _obj in objects: #, 'defensePlayer']:
                    # calculate stat/object direction
                    if _obj in ['none', 'all']:
                        continue

                    _object = '{}Player'.format(_obj.strip())
                    isAscending =  self.generator.is_ascending(obj=_object, statDef=statDef)
                            ...
                            for dim1 in _dimensionGroupsDict[dimGroup1]:
                                ...
                                for dim2 in _dimensionGroupsDict[dimGroup2]:
                                    ...
                                    if 'StatRatio' in statDef:
                                        calculation = statDef['Type'] #'ratio'
    ...
    @contendo_classfunction_logger
    def get_stats_table(self, statName:str, playerObject: str, dimension: str, calculation: str, season=NFL_DEFAULT_SEASON) -> pd.DataFrame:
        _mainkey = (season, statName, playerObject)
        _dims = ['all', dimension]
        _dims.sort()
...
def test():
    ...
    #ostats = NFLPlayerAnalytics(seasons=[NFL_DEFAULT_SEASON], build=True)
    ostats = NFLPlayerAnalytics(seasons=[NFL_DEFAULT_SEASON], build=False)
    logger.info (ostats.allPlayersStatsDF.shape)
    print (ostats.get_stats_table(statName='completions', playerObject='receivingPlayer', dimension='all', calculation='stat'))
    return
