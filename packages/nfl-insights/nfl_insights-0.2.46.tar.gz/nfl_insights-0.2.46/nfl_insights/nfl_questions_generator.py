...
class NFLQuestions:
    ...
    def __init__(self, season=NFL_DEFAULT_SEASON, ostats=None):
        ...
        self._playerStats = NFLPlayerAnalytics([season])
    ...
    def _generate_questions_from_stats(self, statDF: pd.DataFrame, statName:str, object:str) -> pd.DataFrame:
        ...
        _teamObject = self._teamStats.generator.ptmapDict[object]['TeamType']
        _entityType = self._teamStats.generator.ptmapDict[object]['EntityType']
        _statdef = self.language['entities']['statnames'][statName][_teamObject]
        _isAscending = self._teamStats.generator.is_ascending(obj=object, statDef=self._teamStats.statsDict[statName])

        if self._teamStats.generator.ptmapDict[object]['EntityType']=='team':
            statDF['playerName'] = ''
    ...
    def _update_question_fields(self, questionRow: dict) -> None:
        ...
        _objectDef = self._teamStats.generator.ptmapDict[_statObject]
        ...
        if all(x == 'all' for x in _dimList):
            ...
            _dimDoPre = self._teamStats.generator.dimentionsDict[_dim]['DoPre']
        # calculate rank text
    ...
    def generate_questionnaire(self, nQuestionnaires=1) -> pd.DataFrame:
        ...
        for _statName, _statDef in self._teamStats.statsDict.items():
            ...
            _teamObjects = [(self._teamStats, 'team', team, '{}Team'.format(team)) for team in _statDef['TeamObjects'].split(',')]
            _playerObjects = [(self._playerStats, 'player', player, '{}Player'.format(player)) for player in _statDef['PlayerObjects'].split(',')]
            _statObjects = _teamObjects + _playerObjects
            #print (_statName, _objects)
            _smallStatsList=list()
            for _statObject in _statObjects:
                _statsSource, _objTYpe, _obj, _object = _statObject
                if _obj in ['none', 'all', '']:
                    continue
                for _dim, _dimDef in self._teamStats.generator.dimentionsDict.items():
                    ...
                    _statsTable = _statsSource.get_stats_table(_statName, _object, _dim, _calc)
                    ...
                    if self._statementsGenerator._smallnum(_statsTable):
                        ...
                        if _calc=='per_game':
                            ...
                            _statsTable = self._teamStats.get_stats_table(_statName, _object, _dim, 'stat')
                            if self._statementsGenerator._smallnum(_statsTable):
                                logger.debug('Discarding due to small stat: %s', params)
                                continue
...
def test():
    ...
    nflq._teamStats.generator.nfldata.bqu.upload_file_to_gcp(NFL_GCP_BUCKET, NFL_QUESTIONNAIRE_FILE, NFL_QUESTIONNAIRE_FILE, timestamp=True)
    return
