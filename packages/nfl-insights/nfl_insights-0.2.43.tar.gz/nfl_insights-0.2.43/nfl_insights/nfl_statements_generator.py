...
class NFLStatements:
    ...
    def _generate_all_statements(self, calculationsFilter=None, statsFilter=None):
        ...
        self._columns = ['Interest', 'TextOK', 'CompOK', 'Text', 'subject', 'rankText', 'dimensionText', 'dimprep',
                         'statnameText', 'ObjectDescription', 'calculationText', 'presAction', 'pastAction', 'X', 'teamName', 'dimensions',
        ...
        for _statname, _statDef in _statDefs.items():
            ...
            for _teamType in ['offenseTeam', 'defenseTeam', 'kickingTeam', 'returnTeam']:
                ...
                if _tt in _statTeams:
                    ...
                    _newTeamDef['statDescriptions'] = _teamSentences.get('statDescriptions', [_statDef['Description']])
    ...
    def _update_statement_text(self, statementRow: dict) -> None:
        ...
        if _calc.find('per_')==0:
            _calc='stat'
        # get allow/alowwing texts
        # if _statdef.get('isAllowed', False):
        #     _allowsText=''
        #     _allowsPost=''
        #     _allowingText = ''
        # else:
        #print(self.language['entities']['statnames'][statementRow['statName']])
        _pastAction=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['pastAction'])
        if statementRow['statObject'].find('Team')>-1:
            _presAction=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['presActionPlural'])
        else:
            _presAction=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['presActionSingle'])
