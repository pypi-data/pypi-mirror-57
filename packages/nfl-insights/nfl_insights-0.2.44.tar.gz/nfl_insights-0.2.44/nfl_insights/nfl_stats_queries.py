...
class NFLStatsQueries():
    ...
    def is_ascending(self, obj:str, statDef: dict) -> bool:
        if obj.replace('Team', '') in statDef['Direction'].split(','):
            _isAscending = False
        else:
            _isAscending = True
        return _isAscending
    ...
    def pbp_get_stat(self, statName, object, dimensions=['all'], season=NFL_DEFAULT_SEASON, aggfunc=None, playType=None, filter='isTrue', trace=False):
        ...
        isAscending=self.is_ascending(teamObject, statDef)
    ...
    def pbp_get_composed_stat(self, compstat, object, dimensions=['all'], season=NFL_DEFAULT_SEASON, filter='isTrue'):
        ...
        objectDef = self.ptmapDict.get(object,{})
        teamObject = objectDef['TeamType']
        isAscending=self.is_ascending(teamObject, compstatDef)
        #
...
def test():
    ...
    # return
    df = generator.pbp_get_stat(statName='yardsPassed', object='offenseTeam', dimensions=['all', 'all'], season=NFL_DEFAULT_SEASON)
