...
class NFLStatements:
    ...
    def _generate_all_statements(self, calculationsFilter=None, statsFilter=None):
        ...
        self._columns = ['Interest', 'TextOK', 'CompOK', 'Text', 'subject', 'rankText', 'dimensionText', 'dimprep',
                         'statnameText', 'ObjectDescription', 'calculationText', 'allowsText', 'allowsPost', 'allowingText', 'X', 'teamName', 'dimensions',
    ...
    def _update_statement_text(self, statementRow: dict) -> None:
        ...
        if _statdef.get('isAllowed', False):
            ...
            _allowsText=self._get_random_text_from_textlist(self.language['entities']['objectDefinitions']['allowsText'])
            _allowsPost=self._get_random_text_from_textlist(self.language['entities']['objectDefinitions'][statementRow['statObject']]['allowsPost'])
            _allowingText = self._get_random_text_from_textlist(self.language['entities']['objectDefinitions']['allowingText'])
        ...
        _statementFormat = {
            ...
            'allowsText': _allowsText,
            'allowsPost': _allowsPost,
            'allowingText': _allowingText,
...
def test():
    ...
    #nfls._generate_all_statements(calculationsFilter=['per_game', 'ratio'], statsFilter=['defensePoints', 'fumbles', 'interceptionReturnYds', 'rushTD', 'kickingPoints', 'yardsPerCatch'])
    # Sort the sentences
