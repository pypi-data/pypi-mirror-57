...
class NFLQuestions:
    ...
    def _generate_questions_from_stats(self, statDF: pd.DataFrame, statName:str, object:str) -> pd.DataFrame:
        ...
        if self._teamStats.statsDict[statName]['Sentiment'] == object.replace('Team', ''):
            _isAscending = True
        else:
            _isAscending = not _statdef['isPositive']

        _fields = ['teamId', 'teamName', 'statValue', 'count', 'rank', 'nGames']
    ...
    def _update_question_fields(self, questionRow: dict) -> None:
        ...
        if _calculation.find('per_')>-1:
            _actionText=self._get_random_text_from_textlist(self.language['entities']['statnames'][questionRow['statName']][questionRow['statObject']]['presActionSingle'])
        else:
            _actionText=self._get_random_text_from_textlist(self.language['entities']['statnames'][questionRow['statName']][questionRow['statObject']]['pastAction'])
        ...
        _questionFormat = {
            ...
            'objectText': self.language['entities']['statnames'][questionRow['statName']][questionRow['statObject']]['ObjectText'],
        ...
        for _statName, _statDef in self._teamStats.statsDict.items():
            ...
            for _obj in _teamObjects:
                for _dim, _dimDef in self._teamStats.generator.dimentionsDict.items():
                    ...
                    _statsTable = self._teamStats.get_stats_table(_statName, _object, _dim, _calc)
...
def test():
    ...
    _questionnareDF = nflq.generate_questionnaire(nQuestionnaires=10)
