...
class NFLStatements:
    ...
    def _generate_all_statements(self, calculationsFilter=None, statsFilter=None):
        ...
        self._columns = ['Interest', 'TextOK', 'CompOK', 'Text', 'subject', 'rankText', 'dimensionText', 'dimprep',
                         'statnameText', 'ObjectDescription', 'calculationText', 'actionText', 'X', 'teamName', 'dimensions',
    ...
    def _refactor_definition_json(self):
        ...
        for _statname, _statDef in _statDefs.items():
            ...
            for _teamType in ['offenseTeam', 'defenseTeam', 'kickingTeam', 'returnTeam']:
                ...
                _objectDef = self._teamStats.generator.ptmapDict[_teamType]
                _newTeamDef = dict()
                ...
                if _tt in _statTeams:
                    ...
                    _newTeamDef['statDescriptions'] = _teamSentences.get('statDescriptions', [_statname+'-xxxx'])
                    _newTeamDef['format'] = _statDef['Format']
                    # which text to show for ObjectText
                    if _statDef['ShowObjText']:
                        _newTeamDef['ObjectText'] = _objectDef['ObjectText']
                    else:
                        _newTeamDef['ObjectText'] = _objectDef['StatObject']

                    # copy the isAllowed if exists
                    if 'isAllowed' in _teamSentences:
                        _newTeamDef['isAllowed'] = True
                else:
                    _newTeamDef['isRelevant'] = False


                if 'isPositive' in _newTeamDef:
                    if _newTeamDef['isPositive']:
                        _newTeamDef['presActionSingle'] = _statDef['PresPosSingle'].split(',')
                        _newTeamDef['presActionPlural'] = _statDef['PresPosPlural'].split(',')
                        _newTeamDef['pastAction'] = _statDef['PastPos'].split(',')
                        _newTeamDef['adverbText'] = [""]
                    else:
                        _newTeamDef['presActionSingle'] = _statDef['PresNegSingle'].split(',')
                        _newTeamDef['presActionPlural'] = _statDef['PresNegPlural'].split(',')
                        _newTeamDef['pastAction'] = _statDef['PastNeg'].split(',')
                        _newTeamDef['adverbText'] = _statDef['NegAdverb'].split(',')
    ...
    def _update_statement_text(self, statementRow: dict) -> None:
        ...
        # calculate rank text
        _rankText = self._get_random_text_from_textlist(self.language['entities']['ranks'][str(int(statementRow['rank']))])
        # get object description
        _objectDescription = self._get_random_text_from_textlist(self.language['entities']['objectDefinitions'][statementRow['statObject']]['descriptions'])
        ...
        if _calculation.find('per_')>-1:
            _actionText=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['presActionPlural'])
        else:
            _actionText=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['pastAction'])
        _adverbText=self._get_random_text_from_textlist(self.language['entities']['statnames'][statementRow['statName']][statementRow['statObject']]['adverbText'])
    ...
    def _smallnum(self,statTable):
         ...
         if statTable['rank'].nunique() <= 6:
            result = True
         if statTable['calculation'].iloc[0] == "per_game" and statTable["statValue"].max() < 5 and "TD" not in statTable["statName"].iloc[0]:
             result = True
    ...
    def _get_comparison_text(self, statTable, statementRow):
        ...
        if any(statTable["rank"] == comprank):
            ...
            if facdif / abs(statementRow["stdFactor"]) > 0.3 and comprow["statValue"]!=0:
                ...
                candidateTemplate = self._get_random_text_from_textlist(self.language["entities"]['comparisons']["next"])
