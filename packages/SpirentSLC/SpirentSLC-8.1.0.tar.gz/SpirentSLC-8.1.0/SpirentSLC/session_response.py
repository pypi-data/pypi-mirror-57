...
class SessionActionResponse(object):
    ...
    def __init__(self, message, nested_steps=()):
    ...
    def _make_extension(self, alias):
        ...
        return {(None, alias.name): function}
    ...
    def steps_report(self):
        def step_report(msg, ns=True):
            ...
            return step_rep.format(step_name, nd.stepId, nd.action.action, msg.duration,
                                   nd.action.command, msg.details.body)

        def traverse_tree(step):
            res = u""
            for st in step.nested_steps:
                res += step_report(st.message)
                res += traverse_tree(st)
            return res

        report = step_report(self.message, False) + traverse_tree(self)
        return report
    ...
    def _query(self, query):
        return self._xml.xpath(query, extensions=self._xpath_extensions)
