...
def _gui(fenetre, fileIn=None):
	...
	with guiHelper.SmoothWindowCreation(fenetre, screens):
		...
		with splash.Splash(fenetre, os.path.join(songfinder.__dataPath__, 'icon.png'), 0, screens):
			# Compile cython file and cmodules
			if not songfinder.__portable__:
				try:
					import subprocess
			...
			try:
				if os.name == 'posix':
					img = ImageTk.PhotoImage(file = os.path.join(songfinder.__dataPath__, 'icon.png') )
					fenetre.tk.call('wm', 'iconphoto', fenetre._w, img) # pylint: disable=protected-access
				else:
					fenetre.iconbitmap( os.path.join(songfinder.__dataPath__, 'icon.ico'))
			except Exception: # pylint: disable=broad-except
				warnings.warn(traceback.format_exc())
			if fileIn :
				fileIn = fileIn[0]
