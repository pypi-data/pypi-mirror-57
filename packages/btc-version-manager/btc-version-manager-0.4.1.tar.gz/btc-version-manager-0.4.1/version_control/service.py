...
from version_control.debugger import debug_method, Debugger
...
from version_control.utils import get_class_from_path
...
def get_version_manager_class() -> Type['VersionControlManager']:
    ...
    manager_class_settings_var_name = 'VERSION_MANAGER_CLASS'

    class_path_from_settings = getattr(settings, manager_class_settings_var_name, None)
    manager_class = get_class_from_path(class_path_from_settings)
    if class_path_from_settings and not manager_class:
        logger.error('Не найден укаазанный в "settings" класс менеджера версий. Используется стандартный - '
                     '"VersionControlManager"')
    return manager_class or VersionControlManager
...
class VersionControlManager:
    ...
    __TRANSACTION_IS_PATCHED: bool = False
    ...
    def provide_storage_verification(self) -> None:
        ...
        if self.atomic_requests_enabled:
            if not self.__class__.__TRANSACTION_IS_PATCHED:
                connection = transaction.get_connection()
                if connection.in_atomic_block:
                    transaction.on_commit(self.verify_storage)
                    self.__class__.__TRANSACTION_IS_PATCHED = True
        else:
            self.verify_storage()

    def get_storage(self) -> Type[VersionControlTemporaryStorage]():
    ...
    def get_service_parameters(self, **kwargs):
        ...
        service_parameters['version_change_url'] = kwargs.pop('version_change_url', '')
        service_parameters['version_change_app'] = kwargs.pop('version_change_app', '')
