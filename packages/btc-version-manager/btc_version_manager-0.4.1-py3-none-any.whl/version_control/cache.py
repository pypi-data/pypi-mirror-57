...
def get_version_manager_storage_class() -> Type['VersionControlTemporaryStorage']:
    ...
    storage_class_settings_var_name = 'VERSION_MANAGER_STORAGE_CLASS'

    class_path_from_settings = getattr(settings, storage_class_settings_var_name, None)
    storage_class = get_class_from_path(class_path_from_settings)
    if class_path_from_settings and not storage_class:
        logger.error('Не найден указанный в "settings" класс хранилища версий. Используется стандартный - '
                     '"VersionControlTemporaryStorage"')
    return storage_class or VersionControlTemporaryStorage
...
class VersionControlTemporaryStorage:
    ...
    That storage is a wrapper over a regular dictionary that is cached in redis (or another) and can be
    restored from it at any given time by the unique key 'request_key'.
    The storage contains all necessary data for writing versions and can be processed in other processes (celery).
    The 'is_verified' flag sets on active transaction commit,
    the 'is_cleaned' flag designed to verify completion of additional storage processing (if specified) before writing
    versions. Both flags must be set to 'True' for versions recording.
    """
