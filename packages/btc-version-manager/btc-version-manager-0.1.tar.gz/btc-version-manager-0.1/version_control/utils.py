...
def build_vcs_relation_tree(parent_object: Model) -> tuple:
    ...
    if parent_object:
        ...
        def prepare_relations(collector_object: NestedObjects, get_missed: bool = False) -> NestedObjects:
            ...
            for key, relations in deepcopy(collector_copy.edges).items():
                for value in relations:
                    is_vcs_object = hasattr(value, vcs_enabled_flag)
        ...
        def format_callback(obj: Model) -> str:
            ...
            if hasattr(obj, vcs_enabled_flag):
                try:
...
# region BTC version manager parse features

def safety_get_attribute(base_object: Any, attr_expression: str, default: Any = None) -> Any:
    """
    Method that gets attribute value based on expression
    :param base_object: source object
    :param attr_expression: attribute access expression
    :param default: default value (in the absence of 'value')
    :return: Any
    """
...
def safety_parse_dict(base_dict: dict, key_expression: str, default: Any = None) -> Any:
    """
    Method that gets value from dictionary based on expression
    :param base_dict: source dictionary
    :param key_expression: key expression to access value
    :param default: default value (in the absence of 'value')
    :return: Any
    """
