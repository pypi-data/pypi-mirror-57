'''
This module contains classes specific to the MediaRoom platform.
'''
...
class MediaRoomRemoteControl(RemoteControl):
    '''
    Class to send IR codes to MediaRoom set-top boxes.
    ...
    def press(self, key, message=None, delay=0.5, show_message=True, timestamp_name=None):
        '''
        Press a MediaRoom remote control button
    ...
    def press_and_wait(self, key, wait_fn, message=None):
        '''
        Press a MediaRoom remote control button and wait
...
class MediaRoomHelper:
    '''
    Class containing helper functions for MediaRoom testing
    '''
    ...
    def open_bell_app_launcher(self):
        ...
        def validate_bell_app_launcher():
            frame = self.dut.display.get_frame()
            template_support_icon = cv2.imread('tests/bell/mediafirst/templates/template_support_icon.png')
            template_youtube_icon = cv2.imread('tests/bell/mediafirst/templates/template_youtube_icon.png')
            template_netflix_icon = cv2.imread('tests/bell/mediafirst/templates/template_netflix_icon.png')
            if (imgmatch.match_template(self.dut, template_support_icon, frame, crop_rect=Region(150, 926, 1900, 100)) or
                imgmatch.match_template(self.dut, template_youtube_icon, frame, crop_rect=Region(150, 926, 1900, 100)) or
                imgmatch.match_template(self.dut, template_netflix_icon, frame, crop_rect=Region(150, 926, 1900, 100))):
                return True
            return False
        ...
        try:
            ...
            while True:
                if validate_bell_app_launcher():
                    ...
                    self.dut.durations.append({'bell_app_launch_time': time_taken})
                    launch_success = True
                    break
                if time.time() - execution_start_time >= timeout_time:
    ...
    def record_duration(self, duration_name):
        ...
        '''
        self.dut.durations.append({duration_name: time.time()})
    ...
    def calculate_duration(self, duration_name, start_timestamp_nm, end_timestamp_nm):
        ...
        self.dut.durations.append({duration_name: duration_diff})
    ...
    def verify_vod_bell_icon(self):
        ...
        '''
        template_bell_icon = cv2.imread('tests/bell/mediafirst/templates/template_vod_bell_icon.png')
