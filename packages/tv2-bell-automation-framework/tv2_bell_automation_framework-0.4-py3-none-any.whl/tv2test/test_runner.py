...
parser.add_argument('--iters', default=iters_default, type=int,
                    help='To run tests n number of times (defaults to 1, if not specified)')
parser.add_argument('--teams_webhook_url', default=teams_webhook_url_default,
                    help='Webhook URL to post fail test case result in MS Teams')
...
parser.add_argument('--is_video_recording', default=is_video_recording_default,
                    help='Flag to check whether to save and record video')
...
class TestRunner:
    ...
    def __init__(self, itachip=args.itachip, itachport=args.itachport, vdev=args.vdev,
                 continous=args.c, iters=args.iters, teams_webhook_url=args.teams_webhook_url,
                 elk_url=args.elk_url, google_api_key=args.google_api_key,
                 is_video_recording=args.is_video_recording, test_cases=args.test_cases, test_number=1):
        self.itachip = itachip
        self.itachport = itachport
        self.vdev = vdev
    ...
    def execute_test_case(self, test_case_class_var=None):
        if test_case_class_var is not None:
            self.test_cases = [test_case_class_var]
        # Iterate over the number of times to execute the test cases, Or continous execution
        while True if self.continous else self.iters > 0:
            # Iterate over the test cases to run
            for test_case_path in self.test_cases:
                try:
                    if test_case_class_var is not None:
                        test_case_class = test_case_class_var
                        video_output_file_name = 'out_youtube.mp4'
                        test_case_name = 'youtube'
                    else:
                        test_case_full_path = '%s' % test_case_path
                    ...
                    options = {
                        'itach_hostname': self.itachip,
                        'teams_webhook_url': self.teams_webhook_url,
                        'elk_url': self.elk_url,
                        'google_api_key': self.google_api_key,
                        'is_video_recording': self.is_video_recording,
                    ...
                    with DeviceUnderTestFactory().get_device(test_case_device_name, test_options) as dut:
                        ...
                        dut.display.add_text_annotation('Starting test case: %s' % test_case_name,
                                                        top_left_cord_pos=(0, 0), duration=12.0)

                        suite = unittest.TestSuite()
                        suite.addTest(
                            TestCaseModule.parametrize(test_case_class, dut=dut, mf=mf, test_name=test_case_name,
                                                       test_result=test_result, test_number=self.test_number))
                    ...
                    self.test_number += 1
                    if test_case_class_var is None:
...
if __name__ == "__main__":
    test_runner_object = TestRunner()
    test_runner_object.execute_test_case()
