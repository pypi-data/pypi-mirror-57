...
class Script(PlainScript):
    ...
    def main(self):
        ...
        if self.options.resolve_mrs and not self.options.slots:
            ...
            def same_as_last(slot):
                slot_dict = slot.toDict()
                last = d.db.get(
                    '%s.%d' % (slot.name, slot_build_ids[slot.name]), {}).get(
                        'config', {})
                bid = str(last.get('build_id', 0))
                # drop elements not to be compared
                clean_slot_dict(last)
                clean_slot_dict(slot_dict)
                json.dump(
                    last,
                    open(slot.name + '.' + bid, 'wb'),
                    indent=2,
                    sort_keys=True)
                json.dump(
                    slot_dict,
                    open(slot.name + '.new', 'wb'),
                    indent=2,
                    sort_keys=True)
                return json.dumps(
                    last, sort_keys=True) == json.dumps(
                        slot_dict, sort_keys=True)

            # remember all names, to report those that have been discarded
            dropped_slots = set(slot.name for slot in slots)

            slots = [slot for slot in slots if not same_as_last(slot)]
        ...
        if mr_slots_config:
            ...
            # if the mr slot is new, and it has the special field 'ci_test',
            # we can a back link to the matching reference slot
            elif 'ci_test' in mr_slot.metadata:
                mr_slot.metadata['ci_test']['reference'] = (ref_slot.name,
                                                            ref_slot.build_id)
