...
def t5():
   ...
   n = 10       #上移10
   for i in range(0,j):
      mouse_move(random.randint(0,1000), random.randint(0,1000))
      mouse_absolute(x,y,x,y-n)
      #windll.user32.SetCursorPos(po.x, po.y)
      mouse_click(random.randint(0,1000), random.randint(0,1000))
      #print(str(j-i))
def t6():
   ...
   subprocess.Popen(r"explorer /select," + r"C:\Users\Administrator\Desktop\test.odp")
   time.sleep(5)
   hwnds_2 = autogui.findWindows('CabinetWClass')

   hwnds_find = list(set(hwnds_2).difference(set(hwnds)))

   #win32gui.SendMessage(hwnd, win32con.WM_DROPFILES , WPARAM(FILETIME("C:\\Users\\Administrator\\Desktop\\test.odp")), 0)
