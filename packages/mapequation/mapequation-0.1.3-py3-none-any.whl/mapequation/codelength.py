...
def codelength(links, tree, entropy_function=np.log):
...
if __name__ == "__main__":
    from grassberger import grassberger, coefficients
    from read_file import read_net, read_tree

    _, train_links = read_net("test/training_seed0_order2_1.net", include_states=False, weight_type=int)
    _, validation_links = read_net("test/validation_seed0_order2_1.net", include_states=False, weight_type=int)
    ...
    Lm = codelength(train_links, tree, grassberger(Gn))
    print(f"Grassberger: training {Lm} bits")

    Lm = codelength(validation_links, tree, grassberger(Gn))
