...
from MountainRed.utils import deprecated
...
@deprecated('Use table2model instead')
def table2model_deprecated(table_location: str) -> models.Model:
    """
    这是为了兼容老版本的API的参数而设计的接口
    该接口将被在全新版本中弃用
    table_location 为 主机名.库名.表名
    """
    dbinfo = table_location.split('.')
    if not len(dbinfo) == 3:
        raise MetaReflexError("参数错误!请检查表位置描述字符串是否符合规范：主机名.库名.表名")
    table_name = dbinfo[2]
    db_alias = "%s.%s" % (dbinfo[0], dbinfo[1])
    return table2model(table_name, db_alias)
