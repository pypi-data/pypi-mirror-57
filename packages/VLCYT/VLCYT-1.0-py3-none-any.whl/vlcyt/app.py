...
class VLCYT:
    ...
    def __init__(self, playlist_url, song_info_enabled=True):   
        ...
        self.total_songs = len(self.playlist["items"]) # Amount of songs in the Pafy object
    ...
    def play_playlist_songs(self):
        ...
        """
        while True:
            if not self.loop_song and not self.shuffle_playlist and not self.back_song and not self.skip_song: # No extra features enabled. Default.
                self._get_next_song()
...
def is_valid_file(parser, arg):
    ...
    if not os.path.exists(arg):
        parser.error(f"The filepath {arg} does not exist! Be sure to include quotes around the path, view help for more info.")
...
def parse_args():
    ...
    parser.add_argument('youtube_playlist_URL', metavar='YouTube Playlist URL', help='URL to a YouTube Playlist')
    parser.add_argument("-v", metavar="VLC Install Directory", type=lambda x: is_valid_file(parser, x), help="If you're getting a FileNotFound error, use this option to pass in your VLC install directory. Example: \"C:\\Program Files\\VideoLAN\VLC\" Be sure to include the quotes.")
...
def add_vlc_dir_to_path(vlc_dir):
    ...
    """
    app_dir = os.getcwd()
    os.add_dll_directory(r"C:\Program Files\VideoLAN\VLC") if not vlc_dir else os.add_dll_directory(vlc_dir)
    os.chdir(app_dir)
    global vlc
    import vlc
