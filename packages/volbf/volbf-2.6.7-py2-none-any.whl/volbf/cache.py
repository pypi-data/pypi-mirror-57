...
""" This module implements the volbf caching subsystem.

The volbf caching subsystem has the following design goals:
 ...
 2) Cached objects are stored by a hierarchical key namespace. Keys
    ...
    - http://www.volbf.org/schema#configuration/renderer specifies
...
2) History logging and audit logs. Currently volbf works by
...
4) Reporting framework. By having a persistant caching framework we
   now have the concept of a volbf analysis session. In other
   words, each new execution of volbf adds new information to
...
default_cache_location = os.path.join((os.environ.get("XDG_CACHE_HOME") or os.path.expanduser("~/.cache")), "volbf")

config.add_option("CACHE-DIRECTORY", default = default_cache_location,
                  cache_invalidator = False,
                  help = "Directory where cache files are stored")

class CacheContainsGenerator(exceptions.volbfException):
