...
import volbf.plugins
import volbf.plugins.overlays.basic as basic
import volbf.plugins.overlays.native_types as native_types
import volbf.exceptions as exceptions
import volbf.obj as obj
import volbf.debug as debug
import volbf.dwarf as dwarf
import volbf.scan as scan
import volbf.plugins.linux.common as linux_common
import volbf.plugins.linux.flags as linux_flags
import volbf.addrspace as addrspace
import volbf.utils as utils
import volbf.protos as protos
...
linux_overlay = {
    ...
    'volbf_MAGIC': [None, {
        'DTB'           : [ 0x0, ['volbfDTB', dict(configname = "DTB")]],
        'ArmValidAS'   :  [ 0x0, ['volbfLinuxARMValidAS']],
        'IA32ValidAS'  :  [ 0x0, ['volbfLinuxIntelValidAS']],
        'AMD64ValidAS'  :  [ 0x0, ['volbfLinuxIntelValidAS']],
        }],
    'vm_area_struct' : [ None, { 
    ...
    class AbstractLinuxProfile(obj.Profile):
        ...
        def _merge_anonymous_members(self, vtypesvar):
            ...
            try:
                ...
                import pdb
                pdb.set_trace()
                raise exceptions.volbfException("Inconsistent linux profile - unable to look up " + str(e))
...
for path in set(volbf.plugins.__path__):
...
class net_device(obj.CType):
    ...
    def mac_addr(self):        
        ...
        if macaddr == "00:00:00:00:00:00":
            if type(self.dev_addr) == volbf.obj.Pointer:
...
class task_struct(obj.CType):
    ...
    def get_task_start_time(self):
        ...
        if type(start_time) == volbf.obj.NativeType and type(start_time.v()) == long:
...
class volbfLinuxIntelValidAS(obj.volbfMagic):
...
class volbfLinuxARMValidAS(obj.volbfMagic):
    """An object to check that an address space is a valid Arm Paged space"""
    def generate_suggestions(self):
...
class LinuxObjectClasses(obj.ProfileModification):
    ...
    def modification(self, profile):
        profile.object_classes.update({
            ...
            'volbfLinuxIntelValidAS' : volbfLinuxIntelValidAS,
            'volbfLinuxARMValidAS' : volbfLinuxARMValidAS,
