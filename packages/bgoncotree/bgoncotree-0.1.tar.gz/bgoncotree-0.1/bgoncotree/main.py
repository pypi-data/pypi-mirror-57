    ...
    def to_json(self, descending=True, limit=None, _current=0):
        ...
        if descending:
            if limit is None or _current < limit-1:
                data['descendents'] = [n.to_json(descending=True, limit=limit, _current=_current + 1) for n in self.children]
            else:
                data['descendents'] = [n.id for n in self.children]

        else:
            if limit is None or _current < limit-1:
                data['parent'] = self.parent.to_json(descending=False, limit=limit, _current=_current + 1) if self.parent is not None else None
            else:
...
class BGOncoTree(dict):
    ...
    def __init__(self, file=None, sep=None):
        ...
        values_sorted_by_node_id = sorted(nodes_map.values(), key=lambda pair: pair[0].id)
        for node, parent_name in values_sorted_by_node_id:
            parent = None if parent_name is None else nodes_map[parent_name][0]
            node.parent = parent
            self[node.id] = node
            if parent:
    ...
    def iter_from(self, node, level=0, descending=True):
        """Iterate descendents from a node (including itself)"""
        yield node, level
        if descending:
    ...
    def _find(self, name):
        ...
        if name_upper in self:
            ...
            for n, _ in self.iter_all():
                if name == n.name or name_ in n.synonyms:  # search as node name or as a synonym
                    node = n
                    break
        return node
