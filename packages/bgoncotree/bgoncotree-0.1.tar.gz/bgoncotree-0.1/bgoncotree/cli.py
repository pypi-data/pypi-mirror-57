...
def cli(verbose):
    logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%H:%M:%S', level=logging.DEBUG if verbose else logging.INFO)
    if not verbose:
...
@click.option('--json', 'as_json', is_flag=True, default=False, help='Use json format for the output')
...
def descendents(name, as_json, file, limit):
