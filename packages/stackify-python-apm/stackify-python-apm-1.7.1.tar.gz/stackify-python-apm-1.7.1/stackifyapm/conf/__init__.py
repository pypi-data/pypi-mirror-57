...
class Config(_ConfigBase):
    ...
    environment = _ConfigValue("ENVIRONMENT", default=constants.DEFAULT_ENVIRONMENT)
    ...
    rum_enabled = _BoolConfigValue("RUM_ENABLED", default=constants.DEFAULT_RUM_ENABLED)
    rum_auto_injection = _BoolConfigValue("RUM_AUTO_INJECTION", default=constants.DEFAULT_RUM_AUTO_INJECTION)
    socket_url = _ConfigValue("SOCKET_URL", default=constants.DEFAULT_TRANSPORT_SOCKET_FILE)
    transport = _ConfigValue("TRANSPORT", env_key="STACKIFY_TRANSPORT", default=constants.DEFAULT_TRANSPORT)
    ...
    try:
        for logger_name in ["stackifyapm", "stackifyapm.traces", "stackifyapm.errors", "stackifyapm.instrument"]:
            ...
            if "error" in logger_name:
                _logger.addHandler(logging.FileHandler("{}{}.err".format(constants.LOG_PATH, logger_name)))
            else:
                _logger.addHandler(logging.FileHandler("{}{}.out".format(constants.LOG_PATH, logger_name)))
    except Exception:
        pass
...
def setup_stackifyapm_logging(client):
    ...
    filename = "{}{}#{}-1.log".format(constants.LOG_PATH, host_name, process_id)

    def namer(name):
        num = int(name.split("/")[-1].split(".")[-1] or 0) + 1
        return "{}{}#{}-{}.log".format(constants.LOG_PATH, host_name, process_id, num)
