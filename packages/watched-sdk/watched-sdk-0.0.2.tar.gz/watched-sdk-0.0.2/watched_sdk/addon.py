...
class Addon(object):
    ...
    def __init__(self):
        ...
        if not props.get('name'):
            props['name'] = config.repository.name
        if not props.get('version'):
            props['version'] = config.repository.version
        if not props.get('homepage'):
            props['homepage'] = config.repository.homepage
...
def setup_repository(props):
    new_props = {'mirrors': [props['homepage']] if props.get('homepage') else []}
