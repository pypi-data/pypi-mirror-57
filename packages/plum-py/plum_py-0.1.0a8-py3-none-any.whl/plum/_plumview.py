...
from ._plum import _pack, unpack_from, pack
...
class PlumView:
    ...
    def __repr__(self):
        return f'<view at 0x{self.__offset__:x}: {self.get()!r}>'
...
class NumberView(PlumView):
    ...
    def __repr__(self):
        return (f'<view at 0x{self.__offset__:x}: '
                f'{self.__type__.__name__}({self.get()})>')
