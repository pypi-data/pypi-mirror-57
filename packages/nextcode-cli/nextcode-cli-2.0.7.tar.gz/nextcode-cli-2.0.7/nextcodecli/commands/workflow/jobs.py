...
@option('--completed', 'is_completed', is_flag=True, help='Only show jobs in failed/cancelled/completed state')
...
@pass_context
def jobs(ctx, is_mine, user_name, status, project, pipeline, is_page, is_running, is_completed, num, output, is_raw):
    svc = ctx.obj.service
    if is_mine:
        try:
    ...
    if is_running:
        ...
        state = "completed"
    try:
        jobs = svc.get_jobs(user_name, status, project=project, pipeline=pipeline, limit=num, state=state)
    ...
    if is_wide:
        fields.extend(['desc', 'complete_date', 'pod', '#proc'])
    jobs_list = []
    for job in jobs:
        job_list = []
        for f in fields:
