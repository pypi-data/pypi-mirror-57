...
@option('--finished', 'is_finished', is_flag=True, help='Only show jobs in failed/cancelled/finished state')
...
@pass_context
def jobs(ctx, is_mine, user_name, status, project, pipeline, is_page, is_running, is_finished, num, output, is_raw):
    svc = ctx.obj.service
    if is_mine:
        try:
