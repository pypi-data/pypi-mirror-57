...
def getModel(model, saveDir):
    ...
    if(os.path.exists(modelAbsPath)):
        ...
        choice = input(Fore.GREEN + Style.BRIGHT+"Loading Failed! Do you want to download "+model+" models from remote repository ? [y/n]"+'\033[39m')
        if choice == 'y':
...
def get_sequence_features(input_file,model):
	if model == "mouse":
		hex_file = "./ezLncPred/models/PredLnc_GFStack/src/GFStack_mouse/Mouse_features_Hexamer.tsv"
	else:
		hex_file = "./ezLncPred/models/PredLnc_GFStack/src/GFStack_human/Human_features_Hexamer.tsv"
...
def get_special_features(input_file,transcript_length):
	...
	for seq in SeqIO.parse(input_file,'fasta'):
			 ...
			 os.system("./ezLncPred/models/PredLnc_GFStack/src/txCdsPredict "+tmp_fa+" "+tmp_cds)
...
def predict(model):
	...
	Selected_features = []
	if model == 'mouse':
			temp = "./ezLncPred/models/PredLnc_GFStack/src/GFStack_mouse/"
			filename = temp+"mouse"+"_selected_features.csv"
	else:
			temp = "./ezLncPred/models/PredLnc_GFStack/src/GFStack_human/"
