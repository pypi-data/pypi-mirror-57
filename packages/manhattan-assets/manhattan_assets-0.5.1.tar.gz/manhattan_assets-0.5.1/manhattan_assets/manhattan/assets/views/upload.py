...
def store_temporary_asset(state):
    ...
    if blueprint_config and field_name:
        analyzers = blueprint_config.asset_analyzers.get(field_name, None)
        variations = blueprint_config.asset_variations.get(field_name, None)
    ...
    if variations:
        ...
        # Store the thumbnail transforms so the variation can be regenerated
        # outside of this view (draft variations are not regenerated as a
        # rule).
