...
class MediaWikiSite(CmdBaseFW):
    ...
    def _upload_files(self, input, file_list, rewrite=False, desc='', ignore=False):
        ...
        for _file in file_list:
            ...
            _upload_name = ''
            _desc = ''
            if len(_file_info) > 1 and _file_info[1] != '':
                _upload_name = _file_info[1]
            else:
                _upload_name = _filename
            if len(_file_info) > 2 and _file_info[2] != '':
                _desc = _file_info[2]
            else:
                _desc = desc
    ...
    def _edit_pages(self, input, file_list, rewrite=False, summary='', encoding=None,
        ...
        for _file in file_list:
            ...
            if len(_file_info) > 2 and _file_info[2] != '':
                _summary = _file_info[2]
            else:
                _summary = summary

            if len(_file_info) > 3 and _file_info[3] != '':
                if _file_info[3] == 'true':
                    _upload_files = True
                else:
                    _upload_files = False
