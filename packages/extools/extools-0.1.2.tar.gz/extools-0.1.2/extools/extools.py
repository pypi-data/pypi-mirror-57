...
class ExToolsError(RuntimeError): pass

def all_records_in_datasource(ds):
...
def all_records_in_view(viewid):
...
def all_records_in(viewlike):
    """Generator that yields all records in a view-like object.

    :param viewlike: an instance of a view-like object.
    """
    viewlike.recordClear()
    viewlike.browse("")

    while viewlike.fetch() == 0:
        yield viewlike

def optional_field_exists_in(viewlike, field):
    """Check if an optional field exists.

    After checking for the field, if the field exists,
    the viewlike will be seeked to the field record.

    :param viewlike: an instance of a view-like object.
    :param field: Optional field name (OPTFIELD).
    :rtype: bool
    """
    viewlike.recordClear()
    viewlike.put("OPTFIELD", field)
    if success(viewlike.read()):
        return True
    return False

def insert_or_update_optional_field(viewlike, field, value):
    """Check if an optional field exists, if so update, otherwise insert.

    :param viewlike: an instance of a view-like object to search for the
                     optional field.
    :param field: Optional field name (OPTFIELD).
    :rtype: bool
    """
    ...
    if _optional_field_exists_in(viewlike, field):
        _pv = viewlike.put("VALUE", value)
        if not success(_pv, viewlike.update()):
            return False
    else:
        _rg = viewlike.recordGenerate()
        _pf = viewlike.put("FIELD")
        _pv = viewlike.put("VALUE", value)
        if not success (_rg, _pf, _pv, viewlike.insert()):
            return False
