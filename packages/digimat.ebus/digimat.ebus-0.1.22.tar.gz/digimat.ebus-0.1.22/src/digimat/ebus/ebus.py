...
class EBusChannel(object):
    ...
    def connect(self):
        try:
            ...
            self.logger.exception("EBusChannel::connect Error!")
            self._timeoutInhibit=time.time()+5
    ...
    def write(self, data):
        try:
            ...
            self.logger.exception("EBusChannel::write error!")
            self.disconnect()
...
class EBusItem(object):
    ...
    def getXmlConfig(self, station, label, labelLow='', labelNormal='', labelHigh='', decimals=1, modeImport=True, defaultValue=None, initValue=0, timeout=15, unit=0xff):
        sxml='<BaseObject category="RSBEBI" station="%d" number="%d">\n' % (int(station), int(self.index))

        sxml+='<General mainLabel="%s" mainLabelLow="%s" mainLabelNormal="%s" mainLabelHigh="%s" decimals="%d"/>\n' % (label or '',
            labelLow or '',
            labelNormal or '',
            labelHigh or '',
            int(decimals))
        ...
        sxml+='<RSCBEBI installed="1" wIdPeer="0" import="%d" applyDefaultValue="%d" fDefaultValue="%f" fInitValue="%f" chTimeout="%d" ' % (cfgImport,
            cfgDefaultValue, defaultValue, initValue, int(timeout))
        if modeImport:
            sxml+='chUnit="%d"' % int(unit)
        sxml+='/>\n'
    ...
    def __repr__(self):
        return 'EbusItem(index=%d, name=%s, value=%s)' % (self.index, self.name, self.valuestr())
...
class EBusInputItem(EBusItem):
    ...
    def __repr__(self):
        return 'EbusInputItem(index=%d, name=%s, value=%s)' % (self.index, self.name, self.valuestr())
...
class EBusOutputItem(EBusItem):
    ...
    def __repr__(self):
        return 'EbusOutputItem(index=%d, name=%s, value=%s)' % (self.index, self.name, self.valuestr())
...
class EBusItems(object):
    ...
    def getFromName(self, name):
        try:
            return self._itemsIndexByName[name.lower()]
        except:
            pass
    ...
    def createInput(self, index, name, signalChangeDelta=0):
        with self._lock:
            ...
            if not item:
                ...
                if name:
                    self._itemsIndexByName[name.lower()]=item
...
class EBus(object):
    ...
    def onBrowseItemResponse(self, up):
        ...
        if not item:
            if typeImport:
                self.logger.warning('found unmapped peer i%d[id=%d, data=%d, timeout=%d]' % (index, pid, data, timeout))
                item=self.createOutput(index)
            else:
                self.logger.warning('found unmapped peer o%d[id=%d, data=%d, timeout=%d]' % (index, pid, data, timeout))
    ...
    def input(self, index, name='', signalChangeDelta=0):
        return self.createInput(index, name, signalChangeDelta)

    def createOutput(self, index, name='', signalChangeDelta=0, unit=0xFF):
        if not name:
            name='o%d' % index
        return self._items.createOutput(index, name, signalChangeDelta, self.guessUnit(unit))
    ...
    def output(self, index, name='', signalChangeDelta=0, unit=0xFF):
        return self.createOutput(index, name, signalChangeDelta, unit)
    ...
    def itemFromName(self, name):
        return self._items.getFromName(name)
