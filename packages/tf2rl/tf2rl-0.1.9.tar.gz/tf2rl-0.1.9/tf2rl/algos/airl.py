...
class Discriminator(tf.keras.Model):
    def __init__(self, state_shape, action_dim, units=[32, 32],
                 enable_sn=False, output_activation="linear",
                 name="Discriminator"):
        super().__init__(name=name)

        DenseClass = SNDense if enable_sn else Dense
        self.l1_rew = DenseClass(units[0], name="L1", activation="relu")
        self.l2_rew = DenseClass(units[1], name="L2", activation="relu")
        self.l3_rew = DenseClass(1, name="L3", activation=output_activation)

        self.l1_val = DenseClass(units[0], name="L4", activation="relu")
        self.l2_val = DenseClass(units[1], name="L2", activation="relu")
        self.l3_val = DenseClass(1, name="L3", activation=output_activation)
    ...
    def call(self, inputs):
        ...
        features_rew = self.l1_rew(features)
        features_rew = self.l2_rew(features_rew)
        features_val = self.l1_val(features)
        features_val = self.l2_val(features_val)
        return self.l3_rew(features_rew), self.l3_val(features_val)
