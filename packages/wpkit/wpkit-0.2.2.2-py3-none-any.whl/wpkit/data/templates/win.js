...
winjs=function () {
    ...
    content(cont){
        if(cont){this.inner.html(cont)}
        else{return this.inner.html();}
    }
    init_js(){
        var self=this;
        simpleMakeResizable(this.el[0]);
        makeDraggable(this.el[0], this.drag_head[0]);
        this.el.find('.window-close').click(function () {self.el.hide()});
    ...
    init_style() {
        ...
        var els=Object.keys(this.init_styles);
        for(var i in els){
            var el=els[i];
            var sty=this.init_styles[el];
