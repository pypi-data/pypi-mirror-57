...
class Node(metaclass=NodeMetaClass):
    ...
    def print_structure(self):
        print(self.get_structure_string())
    def get_structure_string(self,depth=0,new_line=True):
        def to_string(ch,depth=0):
           return ch.get_structure_string(depth=depth,new_line=new_line) if isinstance(ch,Node) else '\n'*new_line+self.__indent__*depth+str(ch)
        attrs = ' '.join(['%s="%s"' % ('class' if k == '_class' else k, v.get_structure_string(new_line=False) if isinstance(v,Node) else str(v)) for k, v in self.attrs().items()])
        attrs = ' ' + attrs if len(attrs) else attrs
...
class Var(Node):
    def __init__(self, varname=None, vardefault=T.NOT_GIVEN,**kwargs):
        ...
        if vardefault!=T.NOT_GIVEN :self.seta(default=vardefault)
    def to_string(self, depth=0):
        d=self.geta('default',T.NOT_FOUND)
        if d==T.NOT_FOUND:return self.get_string(depth=depth)
        if not d:return ''
...
class NodeList(Node,deque):
    def __init__(self,*args):
        ...
        Node.__init__(self)
    def to_string(self, depth=0):
        return ''.join([item.to_string(depth=depth) for item in self])
