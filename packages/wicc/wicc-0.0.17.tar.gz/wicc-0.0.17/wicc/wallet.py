...
class Wallet(object):
    ...
    @classmethod
    def private_key_from_mnemonic(cls, mnemonic, main_net=False):
        coin = WaykiCoin(testnet=not main_net)
        w = coin.wallet(mnemonic)
        address = w.new_receiving_address()
        print(address)
        return w.privkey(address)

    @classmethod
    def generate_private_key(cls, main_net=False):
        coin = WaykiCoin(testnet=not main_net)
        w = coin.wallet(random_words())
        return w.privkey(w.new_receiving_address())
...
if __name__ == '__main__':
    ...
    print(mnemonic)
    print(Wallet.private_key_from_mnemonic("recycle borrow divorce panel myth edit race inner buddy pig maid faith", main_net=True))

    #prefer chief trouble old liar void margin street invest trap slight galaxy
    #YBEFZECRus9y6tNUxM6oo87VRKx5EsH8XknuZdL5iPPQ9PYqTiTK"