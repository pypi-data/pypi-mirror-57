...
class TaskScheduler(object):
    ...
    def check_if_logging_tables_exists(self):
        ...
        new_table_query = 'CREATE TABLE IF NOT EXISTS `task_execution_steps`(' \
                          ...
                          '`category`    enum(\'Idle\', \'Initialize\', \'Extract\', \'Store\', \'Transform\', \'Transfer\', \'Inform\')  DEFAULT \'Idle\' NOT NULL,' \
                          'PRIMARY KEY (`id`),' \
    ...
    def create_task_execution_steps(self, step_details: list):
        ...
        Example: step_details = [
                                    {'nr': 1, 'description': 'test', 'category': 'Idle'},
                                    {'nr': 2, 'description': 'test2', 'category': 'Initialize'}
                                ]
        :return: error (str) or response of mysql
        """

        # Check if the required fiels are available in the given list
        required_fields = ['nr', 'description', 'category']
        ...
        values = ','.join(str((self.task_id, step['nr'], step['description'], step['category'])) for step in step_details)
        response = self.mysql.raw_query("INSERT INTO task_execution_steps (`task_id`, `nr`, `description`, `category`) "
                                        "VALUES {step_values} ON DUPLICATE KEY UPDATE `description` = VALUES(description), `category` = VALUES(category)".format(step_values=values), insert=True)
        return response
    ...
    def finish_task(self, reload_instant=False):
        ...
        if self.error_count > 0:
            self.mysql.update('task_scheduler', ['status', 'last_reload', 'last_error_message', 'step_nr'],
                              ['IDLE', datetime.datetime.now(), 'FinishedFail', 0],
                              'WHERE `id` = {}'.format(self.task_id))
            self.mysql.raw_query(
