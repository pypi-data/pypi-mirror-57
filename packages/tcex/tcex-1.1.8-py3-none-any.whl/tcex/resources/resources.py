...
class Resources(object):
    ...
    def _apply_filters(self):
        ...
        self.tcex.log.debug('filters: {}'.format(filters))
    ...
    def _request_bulk(self, response):
        ...
        try:
            ...
            self.tcex.log.debug('temp json file: {}'.format(temp_file))
    ...
    def _request_process(self, response):
        ...
        if response.status_code in self._status_codes[self._http_method]:
            ...
            status = 'Failure'
            err = 'Failed Request {}: Status Code ({}) not in {}. API Response: "{}".'.format(
                self._name,
    ...
    def owner(self, data):
        ...
        if data is not None:
            ...
            self.tcex.log.warn('Provided owner was invalid. ({})'.format(data))
    ...
    def paginate(self):
        ...
        if results.get('status') == 'Success':
            ...
            self.tcex.log.debug('Result Count: {}'.format(self._result_count))
            ...
            self.tcex.log.debug('Resource Count: {}'.format(len(resources)))
    ...
    def request(self):
        ...
        self._apply_filters()
        self.tcex.log.debug('Resource URL: ({})'.format(self._request.url))
    ...
    def __str__(self):
        ...
        if self._request.headers:
            printable_string += '\n{0!s:40}\n'.format('Headers')
            for k, v in list(self._request.headers.items()):
                printable_string += '  {0!s:<29}{1!s:<50}\n'.format(k, v)

        # payload
        if self._request.payload:
            printable_string += '\n{0!s:40}\n'.format('Payload')
            for k, v in list(self._request.payload.items()):
                printable_string += '  {0!s:<29}{1!s:<50}\n'.format(k, v)
...
class Address(Indicator):
    ...
    def indicator(self, data):
        ...
        try:
            ...
            ip = ipaddress.ip_address('{}'.format(data))
        if ip.version == 6:
            data = ip.exploded
            sections = []
