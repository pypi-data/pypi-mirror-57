...
def run_subproc(
    ...
    try:
        for (prefix, pipe, buf, postfix) in [
            ("%s%s" % (prefix, err_prefix), proc.stderr, stderr, err_postfix),
            ("%s%s" % (prefix, out_prefix), proc.stdout, stdout, out_postfix),
        ]:
            ...
            thread = threading.Thread(
                name=prefix,
                target=LinePipe(
                    prefix, pipe, buf, output_func, postfix, capture_output
                ).flow,
            )
