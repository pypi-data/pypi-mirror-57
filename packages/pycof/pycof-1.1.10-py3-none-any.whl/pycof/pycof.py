...
def remote_execute_sql(sql_query="", query_type="SELECT", table="", data={}, credentials={}, verbose=True, autofill_nan=True, useIAM=False, is_psycopg=False):
    ...
    Args:
        ...
        is_psycopg (bool): Force the function to use psycopg2 library (for redshift, postgreSQl, ...) (defauls False).
    ...
    if (useIAM) & (secret_key != "") & (access_key != ""):
        # If credentials are provided, try to setup a session with them
        try:
            ...
            raise ValueError('Cannot create boto3 session, please check your user\'s permission.')
    elif (useIAM):
        # If no IAM credentials, use the default boto3 behavior: use IAM roles
        try:
            session = boto3.Session()
        except:
            raise ValueError('Cannot create boto3 session, please make sure you attached IAM role to you instance.')
    else:
        raise ValueError('Cannot setup the boto3 session. Please check your IAM roles or IAM user credentials.')
    ...
    is_psy = True if (is_psycopg) or 'redshift' in hostname.split('.') else False
