...
def delegator(output: str, script_list: List[str], parametrized: bool):
    ...
    for script, spec in valid_scripts(script_list):
        ...
        print(os.path.join(output, script.meta['title'] + '.xml'))
...
def handle_gap_questions(script, spec, instances: int) -> Iterator[Dict]:
    ...
    for _ in range(instances):
        ...
        yield {
            'gap_list': parser.gap_parser(pre_process_text(script.task)),
            'author': script.meta['author'],
            'title': script.meta['title'],
            'shuffle': script.meta['shuffle'] if 'shuffle' in script.meta else True,
            'feedback': markdown(pre_process_text(script.feedback)),
...
def handle_choice_questions(script, spec, instances: int) -> Iterator[Dict]:
    ...
    for _ in range(instances):
        spec.loader.exec_module(script) # reload the script to get new instance
        yield {
            'question_text': markdown(pre_process_text(script.task)),
            'author': script.meta['author'],
            'title': script.meta['title'],
            'shuffle': script.meta['shuffle'] if 'shuffle' in script.meta else True,
...
def handle_javaScript_questions(script, spec, instances: int) -> Iterator[Dict]:
    """
    ...
    for _ in range(instances):
        spec.loader.exec_module(script) # reload the script to get new instance
        JS, imports, task = pre_process_js(script.javaScriptCode, script.task);
        yield {
            'question_text': markdown(pre_process_text(task)),
            'author': script.meta['author'],
            'title': script.meta['title'],
            'correctAnswers': markdown(script.correctAnswers),
...
def handle_new_script(name, qtype, author, points):
    ...
    if qtype == 'multiple choice' or qtype == 'single choice':
        ...
        scaffolding = templates.javaScript.format(author, base)
    else:
