...
def get_max_tries():
    if piano_sdk.max_tries:
        return piano_sdk.max_tries
    else:
        return 3
...
def backoff_logger(details):
    """
    Handler to take care of differential logging based onnumber of tries.

    details is defined by the `backoff` library:
    ...
    :param details: Dict containing function parameters and retry details
    :return:
    """

    logger.info('[{args[1]}:{args[2]}] - Try {tries}. Waiting {wait:0.1f}. '
                'Params: {kwargs[params]} Error: "{err[1]}"'.format(
                    err=sys.exc_info(), **details))
