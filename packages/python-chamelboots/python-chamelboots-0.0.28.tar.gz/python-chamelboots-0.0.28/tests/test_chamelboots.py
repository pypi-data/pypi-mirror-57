...
from chamelboots.constants import JoinWith
...
def test_multiple_equal_tals():
    FOO, BAR = "foo", "bar"
    div = ChameleonTemplate(
        "div",
        (
            TalStatement("define", JoinWith.SPACES((FOO, FOO))),
            TalStatement("define", JoinWith.SPACES((BAR, BAR))),
        ),
    )
    print(div)
    assert all(item in div.html_string for item in (FOO, BAR))
