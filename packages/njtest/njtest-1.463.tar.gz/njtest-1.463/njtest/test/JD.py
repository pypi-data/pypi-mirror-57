...
class JDTest(object):
    ...
    def find_wifi(self, wifi_name, wifi_pwd):
        ...
        if System.find_wifi(self.d, wifi_name, wifi_pwd):  # 切换wifi
    ...
    def find_activity(self):
        ...
        try:
            ...
            # print(activity)
            time.sleep(1.5)
            if not self.d(text='我的家').wait(timeout=1):
                self.d.press("back")
                self.find_activity()
        ...
        while True:
            try:
                if time.time() - start_time > times:
                    return False
                elif texts in self.d(resourceId='com.jd.smart:id/tv_action').get_text(timeout=2):
                    return True
            except Exception as err:
                pass
    ...
    def direct_init(self, device_name):
        ...
        '''
        try:
            threading.Thread(target=self.find_text, args=('重新打开应用程序', 2)).start()
            threading.Thread(target=self.find_text, args=('稍后提醒', 6)).start()
            ...
            if self.d(text=device_name).wait(timeout=9):  # 发现并删除设备
                ...
                time.sleep(1)
                while self.d(text='删除设备').wait(timeout=1):
                    self.d(text='删除设备').click(timeout=3)
                    self.d(text='确定').wait(timeout=7)
                    self.d(text='确定').click(timeout=3)
                    if self.d(resourceId='com.jd.smart:id/iv_right').wait(timeout=6):  # 如果发现下一个元素跳出循环
                        break
    ...
    def smap_network(self, device_name, wifi_name, wifi_pwd, ser, cmd):
        ...
        '''
        try:
            if not self.find_wifi(wifi_name, wifi_pwd):  # 切换wifi
                return False
    ...
    def ap_network(self, device_name, wifi_name, wifi_pwd, ser, cmd, ap_wifi, ap_pwd):
        ...
        try:
            ...
            if wifi_pwd is not None:
                ...
                self.d(resourceId='com.jd.smart:id/tv_pwd').set_text("")  # 输入wifi密码
                self.d(text='请选择设备要加入的Wi-Fi').click(timeout=2)  # 隐藏输入键盘
                self.d(text='下一步').click(timeout=6)
                self.d(text='确定').click(timeout=3)
            self.d(resourceId='com.jd.smart:id/id_cb_tip').click(timeout=6)
            ...
            if self.d(text="去连接").wait(timeout=25):
                ...
                if self.result_text('完成', 25):
                    ...
                    if not self.find_wifi(wifi_name, wifi_pwd):  # 切回wifi
                        return False
            else:
    ...
    def add_device(self):
        ...
        for i in range(11000000):
            ...
            #                   'JDKongTiao0039', '12345678')
            a = jd.smap_network('雷士-LED台灯', 'BL_huawei', 'nj12345678', ser, 'A5 A5 5A 5A 98 C1 E8 03 00 00 00 00')
            if a:
                count += 1
            print(count, sum)
