...
from gea.models import Antecedente, Catastro, CatastroLocal, Circunscripcion, \
    ...
    ExpedientePartida, ExpedientePersona, ExpedienteProfesional, Lugar, \
    Objeto, Pago, Partida, PartidaDominio, Persona, Presupuesto, Profesional, \
    Titulo, Zona
...
class CatastroAdmin(admin.ModelAdmin):
    list_filter = ['zona', 'seccion', 'poligono', 'manzana', 'parcela',
                   'subparcela']
    search_fields = ['zona', 'seccion', 'poligono', 'manzana', 'parcela',
                     'subparcela']
    actions_on_bottom = True
    list_per_page = 20
    save_on_top = True
    ordering = ['zona', 'seccion', 'poligono', 'manzana', 'parcela',
                'subparcela']
...
class CatastroLocalAdmin(NestedModelAdmin):
    list_display = ('seccion', 'manzana', 'parcela', 'subparcela', 'suburbana',
                    'poligono')
    ...
    list_per_page = 20
    save_on_top = True
    ordering = ['seccion', 'manzana', 'parcela', 'subparcela', 'suburbana',
                'poligono']
...
class ExpedienteAdmin(NestedModelAdmin):
    ...
    def ver_plano2(self, obj):
        if obj.plano != '' and obj.plano is not None:
            return '<a href="%s">%s</a>' % (obj.plano.url,
                                            obj.inscripcion_numero)
        else:
            return None
    ver_plano2.allow_tags = True
    ver_plano2.short_description = 'Ver plano'
    ver_plano2.admin_order_field = 'plano'

    def ver_plano(self, obj):
        p = obj.partida__pii
        if p != '' and p is not None:
            scit_url = 'http://scit.santafe.gov.ar/mycv2/l_muestra_grafica2.php?cla='
            return '<a href="%s%s">Visor SCIT</a>' % (scit_url, p)
        else:
            return None
    ver_plano.allow_tags = True
    ver_plano.short_description = 'Ver SCIT'
    ver_plano.admin_order_field = 'id'
...
class PresupuestoAdmin(admin.ModelAdmin):
    ...
    inlines = [PagoInline]
    list_display = ('expediente', 'monto', 'fecha', 'porcentaje_cancelado',
                    'obs')
