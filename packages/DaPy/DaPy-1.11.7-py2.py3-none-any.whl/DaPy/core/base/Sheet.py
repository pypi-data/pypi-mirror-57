...
class SeriesSet(BaseSheet):
    ...
    def show(self, lines=None, max_row_length=75):
        ...
        assert isinstance(max_row_length, int) and max_row_length > 0, '`max_row_length` must be an integer'
        ...
        str_series = []
        for key, arr in temp.iter_items():
            arr = arr.apply(str)
            arr.insert(0, key)
            str_series.append(arr)
        
        col_size = [max(col.apply(count_str_printed_length)) for col in str_series]
        frame = u''
        titles_, col_size_, values_ = [], [], []
        for size, value in zip(col_size, str_series):
            col_size_.append(size)
            values_.append(value)
            max_len = sum(col_size_) + 3 * len(col_size_) - 1
            if max_len > max_row_length:
                frame += self._show(col_size_, zip(*values_), lines, max_len, omit) + '\n'
                titles_, col_size_, values_ = [], [], []
        frame += self._show(col_size_, zip(*values_), lines, max_len, omit) + '\n'
        print(frame)
