...
class Serializer:
    ...
    * any object having ``to_msgpack``, ``from_msgpack`` (or ``to_json`` and ``from_json``)
    ...
    def _msgpack_encode(self, obj):
        ...
        if hasattr(obj, 'to_msgpack'):
            ...
            try:
                ...
                logger.warning('cannot pack %s (%s) (skip)', obj, type(obj))
                return
    ...
    def from_msgpack(self, data, remote_globals=None):
        """
        ...
        if remote_globals:
            globals().update(remote_globals)
            logger.debug('updating globals with %s', remote_globals)
        try:
    ...
    def from_json(self, txt):
        ...
        """
        self.__setstate__(json.loads(txt, object_hook=self._json_decode))
