#!/usr/bin/env python
# -*- coding: utf-8 -*-

"""Tests for `numtools.serializer` package."""

import pytest

import numpy as np
from numtools.serializer import Serializer, set_loglevel


def test_dic1_msgpack():
    from collections import UserDict

    class MyData(UserDict, Serializer):
        def __init__(self):
            super().__init__()
            self.a = 5

    mydata = MyData()
    mydata['5'] = 'hello'
    mydata[5] = 'world!'
    mydata['s'] = set((1, 2, 3))

    md1 = MyData()
    md1.from_msgpack(mydata.to_msgpack())
    assert md1['5'] == 'hello'
    assert md1[5] == 'world!'
    assert md1['s'] == set((1, 2, 3))


from collections import UserDict
class MyData(UserDict, Serializer):
    def __init__(self):
        super().__init__()
        self.a = 5

    def __repr__(self):
        return 'MyData'


class MyStruc(Serializer):
    def __init__(self):
        self.data = {'a': True, 'mydata': MyData(), 'processed': set()}

    def __repr__(self):
        return 'MyStruc'


def test_dic2_msgpack():
    set_loglevel('info')

    mystruc = MyStruc()
    mystruc.data['mydata']['5'] = 'hello'
    mystruc.data['mydata'][5] = 'world!'
    mystruc.data['mydata']['s'] = set((1, 2, 3))

    md1 = MyStruc()
    md1.from_msgpack(mystruc.to_msgpack(), remote_globals=globals())
    assert md1.data['mydata']['5'] == 'hello'
    assert md1.data['mydata'][5] == 'world!'
    assert md1.data['mydata']['s'] == set((1, 2, 3))
