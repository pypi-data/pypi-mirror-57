    ...
    for step_i in range(2):
        ...
        for clusi in range(len(orderedClusters)):
            ...
            if swap_model_after_translation != None:
                ...
                # NOTE CM: Change for Eli, 15 Nov 2019, just write the models but continue with the fragments
                # So I need to avoid overwriting of convNames, CluAll, RotClu, encn, nfixfr, and read from that sum
                # later on

                if not os.path.exists(os.path.join(current_directory, "./7_SWAP/" + str(i) + "/")):

                    file_check_rmsd = open('checking_rmsds.txt', 'w')
                    del file_check_rmsd
                    ...
                    list_path_models_to_swap = [os.path.join(path_swap_folder, ele)
                                                for ele in os.listdir(path_swap_folder)]
                    list_swapped_models = []

                    # Now I need to superimpose the swap_model_after_translation to all solutions that survived the packing
                    strutemp = Bioinformatics3.get_structure('toswap', swap_model_after_translation)
                    ...
                    PDBTEMP = Bioinformatics3.get_pdb_from_list_of_atoms(new_list_atoms)[0]
                    # SHERLOCK this block is just for uncommenting if we need to check the superposition makes sense
                    ...
                    for file_to_swap in list_path_models_to_swap:
                        ...
                        try:
                            (rmsT, nref, ncom,
                             allAtoms, compStru, pda) = Bioinformatics.getSuperimp(PDBTRANS,
                                                                                    PDBTEMP,
                                                                                    "PDBSTRINGBM_RESIDUES_CONSERVED",
                                                                                    algorithm="nigels-core2",
                                                                                    backbone=True, superpose_exclude=1,
                                                                                    n_iter=None, onlyCA=True)
                            # I should move to ALEPH superposition but not sure is going to produce the same results
                            # and also how to call the function because it has different arguments
                            #  ALEPH.perform_superposition()
                        except:
                        ...
                        file_c.write(pda[0])
                        file_c.close()
                        # CLAUDIA: CHECKING DIFFERENCES BETWEEN ALGORITHMS IN SUPERPOSITION
                        file_check_rmsd = open('checking_rmsds.txt','a')
                        file_check_rmsd.write(os.path.basename(file_to_swap)+'\t'+str(rmsT)+'\n')
                        del file_check_rmsd
                        # CLAUDIA: CHECKING DIFFERENCES BETWEEN ALGORITHMS IN SUPERPOSITION
                        list_swapped_models.append(os.path.basename(file_to_swap))

                        # Commenting to avoid usage later on
                        # for key in convNames.keys():
                ...
                # else:
                #     convNames, CluAll, RotClu, encn, nfixfr = SELSLIB2.readClustersFromSUMToDB(DicParameters,
                # os.path.join(current_directory, "./7_SWAP/" + str(i) + "/swapped.sum"),'ROTSOL',
                #                                                                                LIMIT_CLUSTER=i,
                #                                                                                give_fixed_frags=True)
            # Added this to be sure the objects contain the latest information required from the packing
            # after having written down the files of the swap
            convNames, CluAll, RotClu, encn, nfixfr = SELSLIB2.readClustersFromSUMToDB(DicParameters,
                                                                                       os.path.join(
                                                                                           current_directory,
                                                                                           "./7.5_PACK_Library/" + str(
                ...
                if USE_PACKING:
                    if swap_model_after_translation==None:
                        ...
                        convNames, CluAll, RotClu, encn, nfixfr = SELSLIB2.readClustersFromSUMToDB(DicParameters,
                                                                                               ...
                                                                                               LIMIT_CLUSTER=i,
                                                                                               give_fixed_frags=True)
                    #     # NOTE CM: comment to avoid usage of the swap models later on
            ...
            if topExp_run is not None:
                ...
                if alixe:
                    ...
                    if alixe_mode == 'one_step':
                        ali_confdict['alixe_mode'] = {}
                        ali_confdict['alixe_mode'] = 'one_step'
                        clust_fold_root = os.path.join(current_directory, "./11.5_CLUSTERING/")
                        al.check_dir_or_make_it(clust_fold_root, remove=False)
                        count_clu = evaluated_clusters.index(i)
                        clust_fold_rotclu = os.path.abspath(os.path.join(clust_fold_root, str(i)))
                        # NOTE CM: ali_confdict must have all the settings ready. Temporary discuss defaults
                        ali_confdict['minchunk'] = 100
                        ali_confdict['limit_sol_per_rotclu'] = 1000 # hard limit solutions to take from each rotclu
                        ali_confdict['max_non_clust_events'] = 20
                        if ent:
                        ...
                        # These ones are related with the analysis of the phase clusters
                        ali_confdict['fusedcoord'] = False
                        ali_confdict['ccfromphi'] = False
                        ali_confdict[clust_fold_rotclu] = {'compute_phs': False}
                        ali_confdict['plots'] = True
                        # These are for setting chescat runs
                        ali_confdict["tolerance_first_round"] = 60
                        ali_confdict["resolution_comparison"] = 4.0
                        ali_confdict["seed"] = 1 # this is only for the sequential part
                        ali_confdict["cycles"] = 3
                        ali_confdict["origin_search"] = 'sxosfft'
                        ali_confdict["weight"] = 'e'
                        ali_confdict["oricheck"] = True
                        ali_confdict["map_cc"] = True
                        ali_confdict['resolution_merging'] = 2.0
                        tuple_phi = al.startALIXEforARCIMBOLDO_OneStep(ali_confdict, input_mode, current_directory,
                                                                       clust_fold_rotclu,
                                                                       runstring)
                        ...
                        if tuple_phi[1] != []:  # If any phi, call SELSLIB2.shelxe_cycle_BORGES
                            ...
                            # TODO CM: we whould not send more phase clusters than normal solutions to expand
                            # TODO CM: but we should filter this before producing them, by only attempting topExp references
                            # TODO CM: I think this will be more the case for BORGES with libraries
                            #if len(tuple_phi[1]) > topExp:
...
""")
        print(colored("""#""", 'blue') + colored('\tNOTE', 'yellow') + colored(
            ': only if you want to use patterson correlation refinement for helical folds', 'blue'), end=' ')
        print(colored("""
#mtz_p1_path: %(working_directory)s/dataP1.mtz
""", 'blue'))
        print("""
...
[LOCAL]""", end=' ')
        print(colored("""
# Third party software paths
# Requires PHASER 2.7.x""", 'blue'), end=' ')
        print("""
path_local_phaser: phenix.phaser
path_local_shelxe: shelxe
path_local_arcimboldo: ARCIMBOLDO_BORGES
