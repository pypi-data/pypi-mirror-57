...
class DKCloudCommandRunner(object):
    ...
    def _merge_file(dk_api, kitchen_name, recipe_name, recipe_path, folder_name, file_info):
        ...
        if six.PY3 or (six.PY2 and isinstance(local_contents, six.text_type)):  # if unicode
            the_bytes = local_contents.encode('utf-8')
        else:
            the_bytes = local_contents
            ...
            for conflict in folder_contents:
                ...
                if 'conflict_tags' in conflict:
                    with open(file_fullpath, 'w') as conflict_file:
                        if six.PY3:
                            conflict_file.write(
                                base64.b64decode(conflict['conflict_tags']).decode('utf-8')
                            )
                        else:
    ...
    def _print_merge_success(payload, dk_api=None):
        ...
        x.left_padding_width = 1
        for recipe_key, recipe in merge_info['recipes'].items():
            for recipe_name, recipe_folders in recipe.items():
        ...
        if order_run and DKCloudCommandRunner.LOGS in order_run:
            ...
            for log in logs:
                if not DKCloudCommandRunner._show_log(log, showable_log_types):
                    continue
                line = list(map(lambda x: log.get(x, ''), items))
                log_line = ' | '.join(line)
