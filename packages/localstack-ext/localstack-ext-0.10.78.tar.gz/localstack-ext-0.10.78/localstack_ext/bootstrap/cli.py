...
def import_auth():
 try:
  ...
  raise BXHzo('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=BXHzj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=BXHzD)
def get_params(args,names=[],mandatory=BXHzj):
 result=()
 for name in names:
  value=BXHzO
  if name in args:
  ...
  if not value and mandatory:
   raise BXHzo('Please provide %s=... parameter'%name)
  result+=(value)
 if BXHzF(result)==1:
  return result[0]
 return result
