...
def import_auth():
 try:
  ...
  raise VdkNF('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=VdkNp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=VdkNc)
def get_params(args,names=[],mandatory=VdkNp):
 result=()
 for name in names:
  value=VdkNb
  if name in args:
  ...
  if not value and mandatory:
   raise VdkNF('Please provide %s=... parameter'%name)
  result+=(value)
 if VdkNa(result)==1:
  return result[0]
 return result
