...
def import_auth():
 try:
  ...
  raise AavBL('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=AavBX)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=AavBT)
def get_params(args,names=[],mandatory=AavBX):
 result=()
 for name in names:
  value=AavBW
  if name in args:
  ...
  if not value and mandatory:
   raise AavBL('Please provide %s=... parameter'%name)
  result+=(value)
 if AavBd(result)==1:
  return result[0]
 return result
