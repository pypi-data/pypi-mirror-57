...
def import_auth():
 try:
  ...
  raise usAFe('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=usAFk)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=usAFL)
def get_params(args,names=[],mandatory=usAFk):
 result=()
 for name in names:
  value=usAFy
  if name in args:
  ...
  if not value and mandatory:
   raise usAFe('Please provide %s=... parameter'%name)
  result+=(value)
 if usAFr(result)==1:
  return result[0]
 return result
