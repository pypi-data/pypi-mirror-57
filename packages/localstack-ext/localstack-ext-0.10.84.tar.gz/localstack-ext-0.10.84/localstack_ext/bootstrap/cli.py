...
def import_auth():
 try:
  ...
  raise rSGuw('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rSGun)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rSGuF)
def get_params(args,names=[],mandatory=rSGun):
 result=()
 for name in names:
  value=rSGuz
  if name in args:
  ...
  if not value and mandatory:
   raise rSGuw('Please provide %s=... parameter'%name)
  result+=(value)
 if rSGuj(result)==1:
  return result[0]
 return result
