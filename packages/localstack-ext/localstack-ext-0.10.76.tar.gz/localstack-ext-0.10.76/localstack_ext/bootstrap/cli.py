...
def import_auth():
 try:
  ...
  raise pDaMY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=pDaMd)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=pDaMI)
def get_params(args,names=[],mandatory=pDaMd):
 result=()
 for name in names:
  value=pDaMJ
  if name in args:
  ...
  if not value and mandatory:
   raise pDaMY('Please provide %s=... parameter'%name)
  result+=(value)
 if pDaMr(result)==1:
  return result[0]
 return result
