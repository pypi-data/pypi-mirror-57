...
def import_auth():
 try:
  ...
  raise Wdylr('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=WdylC)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=WdylK)
def get_params(args,names=[],mandatory=WdylC):
 result=()
 for name in names:
  value=Wdylk
  if name in args:
  ...
  if not value and mandatory:
   raise Wdylr('Please provide %s=... parameter'%name)
  result+=(value)
 if Wdylx(result)==1:
  return result[0]
 return result
