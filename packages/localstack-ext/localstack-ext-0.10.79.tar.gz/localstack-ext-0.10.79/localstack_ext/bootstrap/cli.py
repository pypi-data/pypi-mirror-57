...
def import_auth():
 try:
  ...
  raise okqfb('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=okqfr)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=okqfc)
def get_params(args,names=[],mandatory=okqfr):
 result=()
 for name in names:
  value=okqfy
  if name in args:
  ...
  if not value and mandatory:
   raise okqfb('Please provide %s=... parameter'%name)
  result+=(value)
 if okqfv(result)==1:
  return result[0]
 return result
