...
def import_auth():
 try:
  ...
  raise WEJyi('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=WEJyf)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=WEJyc)
def get_params(args,names=[],mandatory=WEJyf):
 result=()
 for name in names:
  value=WEJyK
  if name in args:
  ...
  if not value and mandatory:
   raise WEJyi('Please provide %s=... parameter'%name)
  result+=(value)
 if WEJyX(result)==1:
  return result[0]
 return result
