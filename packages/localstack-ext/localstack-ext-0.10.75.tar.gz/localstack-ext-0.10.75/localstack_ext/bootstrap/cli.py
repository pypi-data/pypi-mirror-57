...
def import_auth():
 try:
  ...
  raise aRvJI('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=aRvJf)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=aRvJh)
def get_params(args,names=[],mandatory=aRvJf):
 result=()
 for name in names:
  value=aRvJk
  if name in args:
  ...
  if not value and mandatory:
   raise aRvJI('Please provide %s=... parameter'%name)
  result+=(value)
 if aRvJG(result)==1:
  return result[0]
 return result
