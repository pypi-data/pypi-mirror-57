...
class TestWebPage(unittest.TestCase):
    ...
    def check_connection(self):   # pragma: no cover
        try:
...
<body>
<param valuetype='ref' value='index.html'>
<div style="background: url('img/background.png');">
<img srcset="img/img_20x14.png 1, img/img_40x14.png 2x">
<img src="img/img2.png" alt="" style="">
...
class TestWebPageElements(unittest.TestCase):
    ...
    def test_auto_parsing_by_iter_method(self):
        for e in self.w.__iter__():
            self.assertEqual(e.base_url, self.url)

    def test_auto_parsing_by_elements_property(self):
        self.assertEqual(len(self.w.elements), 9)

    def test_element_property_sync_check(self):
        self.w._parseComplete = True
        self.w.root = None
        with self.assertRaises(RuntimeError):
            _e = self.w.elements

    def test_element_property_parse_complete_flag(self):
        self.w.set_source(io.BytesIO(b''))
        with self.assertRaises(pywebcopy.exceptions.ParseError):
            getattr(self.w, 'elements')
...
class TestCrawler(unittest.TestCase):
    ...
    def test_parsing(self):
        self.w.parse()
        self.assertEqual(self.w.__len__(), 8)
