...
from six.moves.urllib.parse import urlsplit, urlparse, urljoin
...
class TestConfigHandlerBasic(unittest.TestCase):
    ...
    def test_len(self):
        ...
        self.assertEqual(self.c.__len__(), 1)
    ...
    def test_setup_path_bad_data(self):
        with self.assertRaises(TypeError):
            self.c.setup_paths(int, int)
        with self.assertRaises(TypeError):
            self.c.setup_paths(self.path, None)

    def test_setup_path_invalid_path(self):
        with self.assertRaises(TypeError):
            self.c.setup_paths('not_a_path')
    ...
    def test_setup_path_logger_setup(self):
        pre = len(logging.root.handlers)
        self.c.setup_paths(self.path, self.name)
        post = len(logging.root.handlers)
        self.assertEqual(post, pre + 1)

    def test_setup_paths2(self):
        self.c.setup_paths(self.path, self.name)
        self.assertEqual(self.name, self.c.get('project_name'))
        self.assertEqual(os.path.join(self.path, self.name), self.c.get('project_folder'))
...
class TestUrl2(unittest.TestCase):
    ...
    def test_url_parsing_2(self):
        ...
        self.assertEqual(self.utx._url, self.url)
...
class TestUrl3(unittest.TestCase):
    ...
    def setUp(self):
        self.url = 'http://test-site.com/index.aspx'
        self.path = tempfile.mkdtemp()
        self.file_name = 'index.html'
        self.utx = pywebcopy.urls.URLTransformer(
            self.url, base_path=self.path, default_fn=self.file_name)
    ...
    def test_url_split(self):
        self.assertEqual(urlsplit(self.url), self.utx.parsed_url)
    ...
    def test_utx_file_name(self):
        self.assertEqual('index.aspx', self.utx.file_name)

    def test_url_path(self):
        self.assertEqual(urlparse(self.url).path, self.utx.url_path)

    def test_utx_to_path(self):
        fn, _ = self.utx.get_filename_and_pos(self.url)
        path = os.path.join(self.path, self.utx.hostname + self.utx.url_path.replace(fn, self.utx.file_name))
        self.assertEqual(os.path.normpath(path), self.utx.to_path)

    def test_utx_file_path1(self):
        path, _ = self.utx._refactor_filename(self.utx.url_path, self.utx._hex(), '__')
        self.assertEqual(os.path.join(self.utx.base_path, url2pathname(self.utx.hostname + path)),
                         self.utx.file_path)

    def test_utx_file_path2(self):
        self.utx.enforce_suffix = True
        path, _ = self.utx._refactor_filename(self.utx.url_path, self.utx._hex(), '__')
        self.assertEqual(os.path.join(self.utx.base_path, url2pathname(self.utx.hostname + path)),
                         self.utx.file_path)
        self.utx.enforce_suffix = False
