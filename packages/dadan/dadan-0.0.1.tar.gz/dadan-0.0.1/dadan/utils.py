...
def cut_filter_words(cutted_sentences, stopwords, use_stopwords=False):
    ...
    for sent in cutted_sentences:
        ...
        if use_stopwords:
            sents.append([word for word in dadan.seg(sent) if word and word not in stopwords])  # 把句子分成词语
        else:
            sents.append([word for word in dadan.seg(sent) if word])
