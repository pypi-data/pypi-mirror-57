...
class NYT(scrapy.Spider, ArticleSource):
    ...
    def ask_for_query(self):
        ...
        return list(self.CANDIDATE_DICT.keys()), yesterday, today
    ...
    def start_requests(self):
        ...
        if self.interactive:
            ...
            for q in query:
                for p in range(10):
                    api_url = self.form_query(query=q, page=p, begin_date=begin_date, end_date=end_date)
                    urls, info = self.make_api_call(api_url)
...
def get_recent_articles():
    start_process(NYT, interactive=False)
